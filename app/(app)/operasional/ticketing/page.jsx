// Operasional > Ticketing — gabungan:
//  (1) MONITOR TICKETING (sama spt Dashboard Manager, via getTicketingMonitor)
//  (2) PNR Inventory (sama spt di Finance > PNR) — reuse PnrRow + rute add/edit /finance/pnr
// ADITIF: halaman baru, tidak mengubah finance/pnr maupun manager-dashboard.
// Path: app/(app)/operasional/ticketing/page.jsx
...
import TicketingMonitor from '@/components/operasional/TicketingMonitor';
import { getTicketingMonitor } from '@/lib/actions/ticketing-monitor';
...
export default async function TicketingPage() {
  ...
  // Monitor ticketing (jangan blokir halaman kalau gagal).
  let monitor = null;
  try { const r = await getTicketingMonitor(); if (r && !r.error) monitor = r; } catch {}
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold text-brand-700">🎫 Ticketing</h1>
          <p className="mt-1 text-slate-600">Monitoring tiket + PNR Inventory (deposit maskapai, harga tiket, vendor, deadline pelunasan).</p>
        </div>
      ...
      {/* MONITOR TICKETING — sama spt Dashboard Manager */}
      {monitor && <TicketingMonitor data={monitor} />}
