...
export default function AccountingForm({ trips = [] }) {
  ...
  return (
    <form action={handleSubmit} className="space-y-5">
      <div>
        ...
        <div className="flex gap-2">
          <label className="flex-1 cursor-pointer">
            ...
            <div className={`p-3 text-center rounded-lg border-2 font-bold transition-colors ${type === 'in' ? 'bg-green-500 border-green-500 text-white' : 'bg-white border-slate-300 text-slate-700 hover:border-green-300'}`}>
              ⬆ Cash IN
            </div>
          </label>
          <label className="flex-1 cursor-pointer">
            <input type="radio" name="type" value="out" checked={type === 'out'} onChange={() => setType('out')} className="sr-only" />
            <div className={`p-3 text-center rounded-lg border-2 font-bold transition-colors ${type === 'out' ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-300 text-slate-700 hover:border-amber-300'}`}>
              ⬇ Cash OUT
            </div>
          </label>
        </div>
      ...
      <button type="submit" disabled={pending} className={`w-full py-3 disabled:opacity-50 text-white font-semibold rounded-lg shadow-card transition-colors ${type === 'in' ? 'bg-green-500 hover:bg-green-600' : 'bg-amber-500 hover:bg-amber-600'}`}>
        {pending ? 'Menyimpan...' : `Tambah Cash ${type === 'in' ? 'In' : 'Out'}`}
      </button>
    </form>
  );
}
