'use client';
// Panel input self-report peserta yang dibawa TL (nama + trip + bukti foto). Untuk penilaian TL.
...
export default function TlReferralPanel({ tripOptions = [], initialReferrals = [] }) {
  ...
  function submit(e) {
    ...
    if (!proof) { setErr('Bukti foto wajib diupload'); return; }
    start(async () => {
      const r = await addTlReferral(nama.trim(), trip.trim(), proof);
      if (r?.ok) { setNama(''); setTrip(''); setProof(''); setOk('Tersimpan'); router.refresh(); setTimeout(() => setOk(''), 2000); }
    ...
    <section className="bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-200 bg-emerald-50">
        ...
        <p className="text-xs text-slate-500 mt-0.5">
          Catat peserta yang berhasil kamu ajak daftar + lampirkan bukti foto - untuk penilaian performa TL. Bulan ini: <b>{initialReferrals.length} peserta</b>.
          <br />Ini hanya perhitungan TL, bukan data pendaftaran resmi (peserta tetap diinput CS).
        </p>
      </div>

      <form onSubmit={submit} className="px-5 py-4 space-y-3 border-b border-slate-100">
        ...
        <div>
          <label className="block text-[11px] font-bold text-slate-600 mb-1">Bukti Foto (wajib)</label>
          <FileUploadInput tripId="referral" subfolder="referral-proof" value={proof} onChange={setProof}
            accept="image/*" label="Upload bukti (foto/screenshot chat)" maxSizeMB={10} disabled={pending} required />
        </div>
        ...
        <ul className="divide-y divide-slate-100">
          {initialReferrals.map((r) => (
            <li key={r.id} className="px-5 py-2.5 flex items-center gap-3 text-sm">
              ...
              {r.proof_url && <a href={r.proof_url} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-600 hover:underline shrink-0">Bukti</a>}
