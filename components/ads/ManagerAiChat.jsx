...
import { askManagerAI } from '@/lib/actions/manager-ai';
...
const SUGGESTED = [
  'Platform ads mana paling efisien bulan ini?',
  'CPL kita sehat ga? Budget mau dialihkan kemana?',
  'Kanal mana yang paling banyak menghasilkan closing?',
  'Analisa omzet & conversion bulan ini.',
  'Trip mana yang perlu digenjot iklannya?',
];
...
export default function ManagerAiChat() {
  ...
  async function send(text) {
    ...
    try {
      const r = await askManagerAI(q, history);
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden mt-6">
      <div className="bg-indigo-700 text-white px-5 py-3 flex items-center gap-2">
        <span className="text-lg">🤖</span>
        <div>
          <p className="font-semibold leading-tight">Asisten Manager AI</p>
          <p className="text-[11px] text-indigo-200">Diskusi data perusahaan & marketing — performa ads, omzet, closing, conversion. Jawaban berbasis data real.</p>
        </div>
      </div>
      <div className="px-5 py-4">
        ...
        <div className="space-y-3 max-h-[460px] overflow-y-auto">
          ...
          {sending && (
            <div className="flex justify-start">
              <div className="bg-slate-100 text-slate-500 rounded-2xl px-3.5 py-2.5 text-sm animate-pulse">Asisten Manager AI mengetik…</div>
            </div>
          )}
          <div ref={endRef} />
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); send(); }} className="mt-3 flex gap-2">
          <input value={input} onChange={(e) => setInput(e.target.value)} disabled={sending}
            placeholder="Diskusi apa saja: performa ads, budget, CPL, omzet, closing, target…"
