      ...
      <div className="space-y-3">
        ...
        <button type="button" onClick={() => setMethod('manual')}
          ...
          <span className="flex-1">
            ...
            <span className="block text-xs text-slate-500">Transfer ke rekening {(Array.isArray(bank.accounts) && bank.accounts.length > 1) ? 'bank (pilih salah satu)' : (bank.bank_name || 'BCA')} lalu upload bukti</span>
          </span>
          <span className="text-slate-400">›</span>
        </button>
      </div>
