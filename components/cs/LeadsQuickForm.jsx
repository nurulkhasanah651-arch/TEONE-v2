...
// Round 73: tambah section ADS (Meta/Google/TikTok) di leads harian
...
export default function LeadsQuickForm({ initial = {} }) {
  ...
  const hasToday = initial.tanggal && (initial.leads_ig != null || initial.leads_ads_meta != null);
  ...
  return (
    <form action={handleSubmit} className="border border-brand-200 rounded-xl p-4 bg-brand-50/30 space-y-3">
      ...
      <input type="text" name="notes" defaultValue={initial.notes || ''} placeholder="Catatan (opsional)..." className="w-full px-3 py-1.5 border border-slate-300 rounded-lg text-sm bg-white" />
