...
// R215m: Visa Workflow Panel — per-peserta cost, Request DP, Send WA, Upload hasil
// ADDITIVE — render di bawah VisaMatrix existing (gak nyentuh matrix)
...
export default function VisaWorkflowPanel({ trip, passengers = [] }) {
  ...
  function handleBulkSend() {
    ...
    startTransition(async () => {
      ...
      showMsg(`✓ Bulk send: ${r.sent} sukses, ${r.failed} gagal${r.family_aware ? ` · Family-aware (${r.family_count} family groups)` : ''}`);
    ...
    <div className="bg-white rounded-xl border-2 border-amber-300 shadow-card overflow-hidden">
      <div className="px-5 py-3 bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-200">
        ...
        <p className="text-[11px] text-slate-600 mt-0.5">
          Per peserta: input biaya biometrik & visa · Request DP ke Finance · Send WA template · Upload hasil
        </p>
      </div>
      ...
      <div className="px-5 py-3 bg-blue-50 border-b border-blue-200 space-y-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <label className="flex items-center gap-1 text-xs cursor-pointer">
            <input
              type="checkbox"
              checked={bulkFamilyAware}
              onChange={(e) => setBulkFamilyAware(e.target.checked)}
            />
        ...
        <div className="flex flex-wrap gap-2 items-end">
          ...
          {bulkTemplate === 'doc_kurang' && (
            <div className="flex-1 min-w-[200px]">
              ...
              <input
                type="text"
                value={bulkCustomDokKurang}
                onChange={(e) => setBulkCustomDokKurang(e.target.value)}
                placeholder="- Pas foto belum jelas&#10;- Rekening koran kurang saldo"
          ...
          <button
            ...
            className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-bold rounded"
          >
...
function PassengerWorkflowRow({ passenger, trip, isSelected, onToggleSelect, showMsg, pending, startTransition, router }) {
  ...
  function handleRequestDP(type) {
    const amount = type === 'biometric' ? biometricCost : visaCost;
    if (!amount || amount <= 0) {
      showMsg(`Set cost ${type} dulu`, 'error');
      return;
    }
    startTransition(async () => {
  ...
  function handleGenerateToken() {
    startTransition(async () => {
      ...
      if (r?.error) showMsg(r.error, 'error');
        ...
        showMsg(`✓ Token ter-generate. URL upload sudah di-copy: ${r.upload_url}`);
        router.refresh();
      }
    });
  }
  ...
  function handleSaveResult() {
    ...
    startTransition(async () => {
      ...
      if (resultType === 'rejected') {
        if (resultRejReason) extras.rejection_reason = resultRejReason;
      }
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {p.visa_last_wa_template && (
                <span className="text-[10px] text-slate-500" title={`Last WA: ${p.visa_last_wa_sent_at}`}>
                  📨 {p.visa_last_wa_template}
                </span>
              )}
            </div>
          ...
          <div className="mt-1 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
            ...
            <div className="bg-blue-50 rounded px-2 py-1">
              <p className="text-[10px] text-blue-700 font-semibold">📤 Upload Token</p>
          ...
          {/* EXPANDED — Cost, Request DP, WA, Upload */}
          {expanded && (
            <div className="mt-3 space-y-3">
              ...
              <div className="p-3 bg-amber-50 rounded border border-amber-200 space-y-2">
                  ...
                  <div>
                    ...
                    <div className="flex gap-1">
                      <input
                        type="number"
                        value={biometricCost}
                        onChange={(e) => setBiometricCost(e.target.value)}
                ...
                <p className="text-[10px] text-slate-500">
                  💾 = save cost · 💰 DP = bikin HPP item + redirect ke Finance untuk Request DP ke accounting
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2 items-end">
                  ...
                  {singleTemplate === 'doc_kurang' && (
                    <div className="flex-1 min-w-[200px]">
                      <input
                        type="text"
                        placeholder="Dokumen kurang"
                        value={singleCustomVars.list_dokumen_kurang || ''}
                        onChange={(e) => setSingleCustomVars({ list_dokumen_kurang: e.target.value })}
                        className="w-full px-2 py-1 border border-slate-300 rounded text-sm"
                      />
                    </div>
                  )}
                  <button type="button" onClick={handleSendSingle} disabled={pending} className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded">
                    📨 Send
                  </button>
                </div>
              </div>

              {/* Upload portal (no biometric) */}
              {!trip.visa_needs_biometric && (
                <div className="p-3 bg-cyan-50 rounded border border-cyan-200">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    {p.visa_upload_token && (
                      <span className="text-[10px] font-mono text-slate-600">/visa/upload/{p.visa_upload_token}</span>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="p-3 bg-emerald-50 rounded border border-emerald-200">
                ...
                {showUploadResult && (
                  <div className="space-y-2">
                    ...
                    <div>
                      <label className="text-[10px] font-semibold text-slate-600 block">URL Foto Hasil (upload ke Supabase storage / drive)</label>
                      <input
                        type="url"
                        value={resultPhotoUrl}
                        onChange={(e) => setResultPhotoUrl(e.target.value)}
                        placeholder="https://..."
                        className="w-full px-2 py-1 border border-slate-300 rounded text-sm"
                      />
                    </div>
                    {resultType === 'approved' && (
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                        <input type="date" placeholder="Valid From" value={resultValidFrom} onChange={(e) => setResultValidFrom(e.target.value)} className="px-2 py-1 border border-slate-300 rounded text-sm" />
                        <input type="date" placeholder="Valid Until" value={resultValidUntil} onChange={(e) => setResultValidUntil(e.target.value)} className="px-2 py-1 border border-slate-300 rounded text-sm" />
                        ...
                        <input type="text" placeholder="Kurir (JNE/SiCepat)" value={resultReturnKurir} onChange={(e) => setResultReturnKurir(e.target.value)} className="px-2 py-1 border border-slate-300 rounded text-sm" />
                        <input type="text" placeholder="Nomor Resi" value={resultReturnResi} onChange={(e) => setResultReturnResi(e.target.value)} className="px-2 py-1 border border-slate-300 rounded text-sm col-span-2" />
                      </div>
                    )}
                    {resultType === 'rejected' && (
                      <textarea
                        value={resultRejReason}
                        onChange={(e) => setResultRejReason(e.target.value)}
                        placeholder="Alasan penolakan dari embassy"
                        rows="2"
                        className="w-full px-2 py-1 border border-slate-300 rounded text-sm"
                      />
                    )}
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-xs font-semibold">📨 Auto-send WA hasil ke peserta (dgn foto attached)</span>
                    </label>
