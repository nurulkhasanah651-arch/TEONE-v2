...
export default function ManualQueueClient({ pending = [], done = [] }) {
  return (
    <div className="space-y-6">
      <section>
        <h2 className="font-bold text-brand-700 mb-2">Menunggu dikirim ({pending.length})</h2>
        {pending.length === 0 ? (
          ...
          <div className="space-y-2">{pending.map((r) => <Row key={r.id} row={r} />)}</div>
        )}
      </section>
