            ...
            {tpl.map((t, i) => (
              <div key={i} className="flex items-center gap-2">
                <input type="text" value={t.category}
                  onChange={(e) => { const next = [...tpl]; next[i] = { ...t, category: e.target.value }; setTpl(next); }}
                  placeholder="Nama kategori" className={input + ' w-32'} />
                ...
                <button onClick={() => setTpl(tpl.filter((_, x) => x !== i))} className="text-red-500 text-sm px-2" title="Hapus">✕</button>
              </div>
            ))}
          </div>
          <div className="flex gap-2 mt-3">
            <button onClick={() => setTpl([...tpl, { category: '', fee: 0 }])} className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-semibold rounded">+ Tambah Kategori Custom</button>
            <button onClick={saveTpl} disabled={pending} className="px-4 py-2 bg-green-600 text-white text-sm font-bold rounded">💾 Simpan Template</button>
          </div>
        </div>
      ) : (
        <>
