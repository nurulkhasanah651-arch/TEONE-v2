...
// R215y: Payment Drive Sync Panel — pattern sama kayak visa
// Path: components/finance/PaymentDriveSyncPanel.jsx
...
export default function PaymentDriveSyncPanel({ trip }) {
  ...
  const hasSetup = !!trip?.payment_drive_trip_folder_id;
  const folderUrl = trip?.payment_drive_trip_folder_url;
  const lastSync = trip?.payment_drive_last_sync_at;
  ...
  function handleSetup() {
    ...
    startTransition(async () => {
      const r = await setPaymentDriveFolder(trip.id, folderInput.trim());
  ...
  function handleSync() {
    setSyncResult(null);
    startTransition(async () => {
      const r = await syncTripPaymentsToDrive(trip.id);
      if (r?.error) { showMsg(r.error, 'error'); return; }
      setSyncResult(r);
      if (r.message) showMsg(r.message);
        ...
        showMsg(`✓ Sync sukses: ${r.synced} bukti di-upload, ${r.skipped} skipped`);
      }
      router.refresh();
    });
  }
  ...
  function handleUnlink() {
    ...
    startTransition(async () => {
      const r = await unlinkPaymentDriveFolder(trip.id);
    ...
    <div className="bg-white rounded-xl border-2 border-emerald-300 shadow-card overflow-hidden">
      <div className="px-5 py-3 bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-emerald-200">
        ...
        <p className="text-[11px] text-slate-600 mt-0.5">
          Auto-upload semua bukti transfer peserta ke Google Drive, folder per peserta
        </p>
      </div>
          ...
          <div className="space-y-3">
            <div className="bg-emerald-50 border border-emerald-200 rounded p-3 text-xs text-emerald-800 space-y-2">
              ...
              <ol className="list-decimal list-inside space-y-1 ml-1">
                ...
                <li>Pakai **Shared Drive** yg sama dgn visa (e.g. "tes 2") atau bikin baru "TEONE Payment Proofs"</li>
                <li>Pastiin service account TE udah Manager/Content Manager di Shared Drive ROOT</li>
                <li>Copy URL Shared Drive → paste di bawah</li>
              </ol>
              <p className="text-[10px] mt-2 italic">
                ℹ Bisa pakai Shared Drive yg sama dgn visa — TEONE auto-create subfolder "PAYMENTS" terpisah dari "VISA"
              </p>
            </div>
            ...
            >
              {pending ? '⏳ Setting up...' : '🔗 Link & Auto-Create Payment Subfolder'}
            </button>
          </div>
        ) : (
          <div className="space-y-3">
                ...
                {syncResult.error_details?.length > 0 && (
                  <details className="mt-2" open>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1 italic">
                      💡 Tip: bukti transfer harus udah ter-upload dulu via invoice/portal peserta. Drive sync cuma copy file existing dari Supabase storage ke Drive.
                    </p>
                  </details>
                )}
              </div>
            ...
            <p className="text-[10px] text-slate-500 italic">
              ℹ File yg udah ke-sync di-skip biar gak duplicate. Untuk re-sync, clear field <code>drive_file_id</code> di DB.
            </p>
          </div>
        )}
      </div>
