...
// Round 137: Modal terpisah buat tambah peserta via Passport AI di master trip
// Path: components/trips/PassportAIAddModal.jsx
// Isolated component — gak ngubah ParticipantForm yg existing

import { useState, useTransition } from 'react';
import dynamic from 'next/dynamic';
import { addParticipant } from '@/lib/actions/participants';

// Dynamic import biar gak crash kalau PassportUploadAI missing
const PassportUploadAI = dynamic(
  () => import('@/components/cs/PassportUploadAI').catch(() => ({
    default: () => (
      <div className="p-3 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
        ⚠ PassportUploadAI belum ada. Upload <code>components/cs/PassportUploadAI.jsx</code> dulu.
      </div>
    ),
  })),
...
export default function PassportAIAddModal({ tripId, onClose }) {
  ...
  const [data, setData] = useState({
    ...
    passport_expiry: '', passport_photo_url: '', nationality: '',
    room_type: '', price_paid: '',
  });
  ...
  function handleAIExtracted(updates) {
    ...
    setData((d) => {
      const next = { ...d };
      if (updates.passport_given_names) next.first_name = updates.passport_given_names;
      if (updates.passport_surname) next.last_name = updates.passport_surname;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div
        className="bg-white rounded-xl max-w-3xl w-full my-8 max-h-[90vh] overflow-y-auto shadow-2xl"
        ...
        <div className="sticky top-0 bg-white z-10 px-5 py-3 border-b border-slate-200 flex items-center justify-between">
          <div>
            <h2 className="font-bold text-brand-700 flex items-center gap-2">
              🛂 Tambah Peserta via Passport AI
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">Upload foto passport → AI auto-fill semua data</p>
          </div>
          <button
            onClick={onClose}
            className="text-2xl text-slate-400 hover:text-slate-700 leading-none w-8 h-8 flex items-center justify-center rounded hover:bg-slate-100"
          >
            ×
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {success ? (
            <div className="p-6 bg-green-50 border-2 border-green-300 rounded-xl text-center">
              ...
              <p className="text-lg font-bold text-green-800">Peserta berhasil ditambahkan!</p>
              <p className="text-sm text-green-700 mt-1">Menutup modal...</p>
            </div>
          ) : (
            <>
              {/* PASSPORT AI */}
              ...
              <div className="flex gap-2 pt-2 sticky bottom-0 bg-white pb-2">
                <button
                  type="submit"
                  disabled={pending}
                  className="flex-1 py-2.5 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white text-sm font-bold rounded-lg transition-colors"
                >
                  {pending ? 'Menyimpan...' : '✓ Tambah Peserta ke Trip'}
                </button>
                <button
                  type="button"
                  onClick={onClose}
