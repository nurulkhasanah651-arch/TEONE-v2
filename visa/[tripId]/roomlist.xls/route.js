// /visa/[tripId]/roomlist.xls — Round 49: Excel-compatible HTML table
// Trick: Excel auto-opens HTML table dengan extension .xls
...
export async function GET(req, { params }) {
  ...
  for (const roomKey of Object.keys(byRoom).sort((a, b) => {
    ...
    // Room header row
    tableRows.push(`<tr style="background:#0570de;color:white;font-weight:bold">
      <td colspan="9">🛏 ${esc(roomKey)} (${list.length} pax)</td>
    </tr>`);
    for (const p of list) {
...
<head>
<meta charset="UTF-8">
<title>Roomlist ${esc(trip.kode_trip || tripId)}</title>
</head>
<body>
