...
// URL untuk LINK DI PESAN CUSTOMER (domain publik). Login/TL/staff tetap pakai siteUrlFor().
export function customerSiteUrlFor(code) {
  if (code === 'khasanah') {
    return process.env.NEXT_PUBLIC_CUSTOMER_URL_KHASANAH || 'https://khasanahtravel.app';
  }
  return process.env.NEXT_PUBLIC_CUSTOMER_URL || 'https://travelingeropa.com';
}

function _validUrl(u) {
  return (typeof u === 'string' && /^https?:\/\//i.test(u.trim())) ? u.trim().replace(/\/$/, '') : null;
}
export function siteUrlFor(code) {
  if (code === 'khasanah') {
    return _validUrl(process.env.NEXT_PUBLIC_SITE_URL_KHASANAH) || 'https://khasanahtravel.app';
  }
  return _validUrl(process.env.NEXT_PUBLIC_SITE_URL) || _validUrl(process.env.NEXT_PUBLIC_APP_URL) || 'https://travelingeropa.com';
}
