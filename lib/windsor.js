...
export async function fetchMetaAds(accountId, datePreset = 'last_30d') {
  const fields = [
...
export async function fetchInstagramOverview(accountId, datePreset = 'last_30d') {
  // 1) Profil (user_info) — ambil 1 baris terbaru
  const profileRows = await windsorFetch('instagram', {
  ...
  // 2) Reach 30 hari (user_insights_day) — jumlahkan
  let reach30 = null;
  try {
    const reachRows = await windsorFetch('instagram', {
  ...
  // 3) Media terbaru (media_info + media_insights)
  let media = [];
  try {
    const mediaRows = await windsorFetch('instagram', {
      fields: [
