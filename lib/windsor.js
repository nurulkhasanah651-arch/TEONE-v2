...
// Ekstrak kode trip dari nama campaign Meta (mis. "TE - 536 EXPLORE..." -> "536", "PR001").
// Ambil token kode PERTAMA: PR + angka, atau 3 digit angka.
export function extractTripCode(campaignName) {
  const m = String(campaignName || '').match(/\b(PR\d{2,3}|\d{3})\b/i);
...
export async function fetchMetaAdsAll(accountIds, datePreset = 'last_7d') {
  ...
  const rows = await windsorFetch('facebook', { fields, date_preset: datePreset, extra: { __timeout_ms: 50000 } });
