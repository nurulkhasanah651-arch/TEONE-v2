...
// Update: support lookup phone via tl_id dari tour_leaders
...
/**
 * Lookup TL dari tour_leaders kalau tlId disediakan
 */
...
/**
 * Kirim WA konfirmasi ke TL via Fonnte
 * FormData accepts:
 *   - tripId (required)
 *   - tlPhone (optional — kalau kosong, lookup via tlId)
 *   - tlName  (optional — kalau kosong, lookup via tlId)
 *   - tlId    (optional — untuk lookup phone dari tour_leaders)
 */
export async function sendTLAssignmentWA(formData) {
  try {
    ...
    // Kalau phone kosong tapi tlId ada, lookup dulu
    if (!tlPhone && tlId) {
    ...
    // Update trip dengan tl_phone + token
    ...
    const tripName = trip.name || trip.kode_trip || `Trip #${tripId}`;
    const tripDate = formatDate(trip.departure || trip.start_date || trip.departure_date);
    const tripEnd = formatDate(trip.arrival || trip.end_date || trip.return_date);
    const pax = trip.quota || trip.pax || trip.total_pax || '-';
