...
// Guard: hanya staf internal boleh kirim/reset assignment TL.
async function requireStaff() {
  const auth = createAuthClient();
  const { data: { user } } = await auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const g = await assertStaff(user); if (g.error) return { error: g.error };
  return { user };
}
...
// Ambil Fonnte token PIC Indri (TEONE) - semua WA plotting/penugasan TL dikirim
// dari nomor Indri. Kalau tak ketemu, fallback ke token CS env biar tak gagal kirim.
async function getIndriFonnteToken() {
  try {
    const c = serviceClientFor('teone');
    if (!c) return null;
    let { data } = await c.from('employees').select('fonnte_token').ilike('email', 'indri@teone.dev').maybeSingle();
    if (!data?.fonnte_token) {
      const r = await c.from('employees').select('fonnte_token').eq('role', 'pic').ilike('full_name', '%indri%').maybeSingle();
      data = r.data;
    }
    const tok = (data?.fonnte_token || '').trim();
    return tok || null;
  } catch { return null; }
}

async function sendViaFonnte(target, message, senderToken) {
  const token = senderToken || process.env.FONNTE_TOKEN_CS || process.env.FONNTE_TOKEN || process.env.FONNTE_API_KEY;
  if (!token) {
  ...
  try {
    ...
    if (!res.ok || !data || data.status === false) {
      const _werr = data?.reason || data?.message || `Fonnte error ${res.status}`;
      // R227: catat ke History WA (wa_log) — sebelumnya jalur TL-assign tidak tercatat
      try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'cs', phone: target, message, kind: 'tl_assign', status: 'failed', state: 'failed', reason: _werr, senderToken: token }); } catch {}
      return {
        ok: false,
        error: _werr,
        raw: data,
    ...
    try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'cs', phone: target, message, kind: 'tl_assign', status: 'sent', state: 'sent', fonnteId: _wfid, senderToken: token }); } catch {}
 ...
 */
function buildWAMessage({ greetingName, tripName, kodeTrip, tripStart, tripEnd, pax, destination, approveUrl, rejectUrl, brandLabel = 'Traveling Eropa' }) {
  const lines = [];
  ...
  lines.push(`_travelingeropa & khasanahtravel team_`);
...
export async function sendTLAssignmentWA(formData) {
  try {
    { const _g = await requireStaff(); if (_g.error) return { error: _g.error }; }
    ...
    // Link konfirmasi HARUS ikut brand trip (TE -> travelingeropa, Khasanah -> khasanahtravel)
    // supaya /api/tl-confirm mencari token di DB brand yg benar (kalau salah domain -> 'kadaluarsa').
    let _brand = ''; try { _brand = currentBrandCode(); } catch {}
    const baseUrl = siteUrlFor(_brand) || process.env.NEXT_PUBLIC_APP_URL || 'https://teone.dev';
    const fullBase = baseUrl.startsWith('http') ? baseUrl : `https://${baseUrl}`;
    const brandLabel = _brand === 'khasanah' ? 'Khasanah Travel' : 'Traveling Eropa';
    ...
    // MANUAL: tidak auto-kirim. Balikin template + link wa.me supaya siapa pun bisa
    // kirim dari WhatsApp yang sedang login (web/app). Token approve/reject tetap dibuat di atas,
    // jadi status assignment tetap tercatat pending + tl_assignment_sent_at terisi.
...
export async function resetTLAssignment(formData) {
  try {
    ...
    const { error } = await supabase
      .from('trips')
      .update({
        // Reset = LEPAS TL sepenuhnya, biar trip hilang dari dashboard TL lama.
        tl_id: null,
        tl_name: null,
        tl_phone: null,
        tl_email: null,
