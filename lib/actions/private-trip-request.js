...
function getServiceClient() {
  // Brand-aware: di khasanahtravel.com -> DB khasanah; teone -> DB teone (bukan hardcode)
...
export async function submitPrivateTripRequest(formData) {
  ...
  // Anti-spam: rem submit beruntun dari No HP yang sama (< 60 detik)
  try {
    const { data: recentR } = await supabase.from('private_trip_requests')
      .select('id').eq('phone', phone)
      .gt('created_at', new Date(Date.now() - 60000).toISOString())
      .limit(1).maybeSingle();
    if (recentR) return { error: 'Request kamu baru saja terkirim. Mohon tunggu sebentar sebelum mengirim lagi.' };
  } catch {}
