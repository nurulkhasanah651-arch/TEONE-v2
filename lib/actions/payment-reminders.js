...
export async function getPaymentDeadlineAlerts({ soonDays = 7, picFilter = '', tripIds = null } = {}) {
  ...
  // Kalau pemanggil sudah kirim daftar trip eksplisit (mis. Dashboard PIC yang sudah
  // menghitung scope-nya sendiri, termasuk SPV yang lihat semua), percayai itu &
  // JANGAN terapkan auto-scope role 'pic' (yang akan mempersempit balik ke trip sendiri).
  const explicitSet0 = Array.isArray(tripIds) ? new Set(tripIds) : null;
  let ownTripIds = null;
  if (role === 'pic' && !explicitSet0) {
  ...
  // Scope eksplisit (mis. dari Dashboard PIC) — batasi ke set trip tertentu.
  const explicitSet = explicitSet0;
  const picOf = (tripId) => tripMap[tripId]?.pic || '';
  const allowed = (tripId) => {
    if (ownTripIds && !ownTripIds.has(tripId)) return false;
    if (explicitSet && !explicitSet.has(tripId)) return false;
