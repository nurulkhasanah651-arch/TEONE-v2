...
function salamWord() {
  ...
  return c === 'khasanah' ? "Assalamu'alaikum" : 'Hai';
}
function financeIntro(picName, isFirst) {
  ...
  // Khasanah: perkenalan PIC HANYA di chat pertama (DP). Selanjutnya tanpa perkenalan.
  if (code === 'khasanah') return (isFirst && picName) ? `Perkenalkan, saya ${picName} dari ${brand} \ud83d\ude4f` : '';
...
function invoiceNumbers(pax, breakdown, payments) {
  ...
  const _isLandTour = _rt.includes('land');
  ...
  const flight = (_isInfant || _isLandTour) ? 0 : Number(bd.domestic_flight || 0);
  const baggage = (_isInfant || _isLandTour) ? 0 : Number(bd.domestic_baggage || 0);
  const baseFee = (_isInfant || _isLandTour) ? 0 : Number(bd.harga_jual_base || 0);
...
export async function sendInvoiceWA(passengerId, previewOnly = false) {
...
export async function sendPaymentReceivedWA(passengerId, previewOnly = false) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  ...
  try {
    ...
    const message = [
      ...
      ``,
      `${salamWord()} *${customer.name}*,`,
      financeIntro(await getPicNameForTrip(supabase, pax.trip_id), /dp/i.test(String(lastPayment?.type || ''))),
      isLunas
