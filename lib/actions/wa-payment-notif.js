...
//  key     = employees.waba_api_key (override) ATAU key akun dari env (cfg.accountKey)
//  phoneId = employees.waba_phone_id (nomor pengirim PIC ini di akun)
// PIC dianggap "pakai WABA" hanya kalau dia punya key sendiri ATAU phone_id — jadi
// PIC lain yang belum siap tetap jatuh ke manual/Fonnte.
async function getPicWabaKey(supabase, tripId, accountKey) {
  try {
    ...
    if (!key) return null;
    // Opt-in: PIC harus punya key sendiri atau phone_id supaya dianggap aktif WABA.
    if (!ownKey && !phoneId) return null;
...
export async function tryWabaApproval(passengerId) {
  try {
    ...
    const picKey = await getPicWabaKey(supabase, pax.trip_id, cfg.accountKey);
