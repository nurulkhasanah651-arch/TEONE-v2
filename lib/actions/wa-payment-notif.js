...
async function queueManualWA(supabase, { phone, message, kind }) {
  try {
...
export async function sendInvoiceWA(passengerId, previewOnly = false, opts = {}) {
  ...
  try {
    ...
    if (!opts.forceSend && await isPicWaManualForTrip(supabase, pax.trip_id)) {
      if (opts.queueManual) await queueManualWA(supabase, { phone: normalizePhone(phone), message, kind: opts.queueKind });
