...
export async function sendPaymentReceivedWA(passengerId, previewOnly = false, opts = {}) {
  ...
  try {
    ...
    // Nominal SINKRON dengan invoice yg di-link (family = total semua anggota, bukan 1 org).
    const _payAmount = (_invRow && Number(_invRow.amount) > 0) ? Number(_invRow.amount) : (Number(lastPayment.amount) || 0);
