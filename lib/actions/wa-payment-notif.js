...
export async function tryWabaApproval(passengerId) {
  try {
    if (!metaConfig() || !passengerId) return null; // null kalau Meta WABA tak aktif / bukan Khasanah -> fallback manual/Fonnte
...
export async function sendPaymentReceivedWA(passengerId, previewOnly = false, opts = {}) {
  ...
  try {
    ...
    if (!opts.forceSend && await isPicWaManualForTrip(supabase, pax.trip_id)) {
      const _waba = await trySendWabaForTrip(supabase, pax.trip_id, phone, message);
      if (_waba?.ok) {
