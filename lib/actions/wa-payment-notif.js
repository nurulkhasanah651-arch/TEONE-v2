...
import { wabaConfig, sendWabaText } from '@/lib/utils/waba';
...
// Cari API key WABA milik PIC trip ini (employees.waba_api_key). Null kalau tak ada.
async function getPicWabaKey(supabase, tripId) {
  try {
    ...
    let emp = null;
    if (email) {
      const r = await supabase.from('employees').select('waba_api_key, waba_phone_id, full_name').ilike('email', email).maybeSingle();
      emp = r.data;
    }
    if (!emp && name) {
      let r = await supabase.from('employees').select('waba_api_key, waba_phone_id, full_name').ilike('full_name', name).maybeSingle();
      emp = r.data;
      if (!emp) { r = await supabase.from('employees').select('waba_api_key, waba_phone_id, full_name').ilike('nickname', name).maybeSingle(); emp = r.data; }
    }
    const key = emp?.waba_api_key ? String(emp.waba_api_key).trim() : '';
    const phoneId = emp?.waba_phone_id ? String(emp.waba_phone_id).trim() : '';
    return key ? { key, phoneId, pic: emp.full_name || name } : null;
  } catch { return null; }
}

export async function tryWabaApproval(passengerId) {
  try {
    const cfg = wabaConfig();               // null kalau bukan Khasanah -> TEONE aman
    if (!cfg || !passengerId) return null;
    ...
    // Key WABA milik PIC trip ini. Kalau PIC belum isi key -> null (jatuh ke manual).
    const picKey = await getPicWabaKey(supabase, pax.trip_id);
    if (!picKey) return null;

    // Reuse builder: ambil pesan + nomor persis seperti yang akan dikirim manual.
    const preview = await sendPaymentReceivedWA(passengerId, true);
    if (!preview?.message || !preview?.phone) return null;

    const r = await sendWabaText({ key: picKey.key, endpoint: cfg.endpoint, phoneId: picKey.phoneId }, preview.phone, preview.message);
    if (r.ok) {
      try {
        await supabase.from('wa_log').insert({
          brand: currentBrandCode(), context: 'finance', kind: 'waba_approval',
          trip_id: pax.trip_id, target_phone: normalizePhone(preview.phone),
          message: preview.message, status: 'sent', sender: picKey.pic,
        });
      } catch {}
      return { ok: true, phone: preview.phone, customer_name: preview.customerName };
    }
    console.error('[WABA khasanah gagal kirim]', r.error);
