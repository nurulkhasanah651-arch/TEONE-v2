...
export async function tryWabaApproval(passengerId) {
  try {
    ...
    if (!cfg || !cfg.pic || !passengerId) return null;
    ...
    if (!pax) return null;
    const picName = await getPicNameForTrip(supabase, pax.trip_id);
    if (String(picName || '').trim().toLowerCase() !== cfg.pic.toLowerCase()) return null;
    ...
    const r = await sendWabaText(cfg, preview.phone, preview.message);
    if (r.ok) {
      try {
        await supabase.from('wa_log').insert({
          ...
          message: preview.message, status: 'sent', sender: cfg.pic,
        });
      } catch {}
