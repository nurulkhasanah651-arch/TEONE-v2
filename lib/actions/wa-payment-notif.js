...
import { metaConfig } from '@/lib/utils/waba-meta';
...
function detailPaketLines(n) {
  ...
  if (n.tips > 0) L.push(`   • Tipping & Service: ${fmtRp(n.tips)}`);
...
export async function sendInvoiceWA(passengerId, previewOnly = false, opts = {}) {
  ...
  try {
    ...
    const _picNameInv = await getPicNameForTrip(supabase, pax.trip_id);

    const message = plainForBrand([
      `📋 *INVOICE PEMBAYARAN — ${brandLabel()}*`,
      ``,
      `${salamWord()} *${customer.name}*,`,
      financeIntro(_picNameInv, !(payments && payments.length)),
    ...
    const _tripLabelInv = `${trip?.kode_trip || ''} ${trip?.name || ''}`.trim();
    const invoiceParams = [(customer.name ? 'Kak ' + customer.name : 'Kak'), _picNameInv || '-', _tripLabelInv || '-', targetInvoice?.invoice_no || '-', targetInvoice?.milestone || '-', invoiceLink || '-'];

    if (previewOnly) return { ok: true, preview: true, message, phone: normalizePhone(phone), customerName: customer.name };
    // WABA (Api.co.id) SELALU dicoba dulu utk PIC Khasanah bernomor. No-op TEONE -> lanjut ke bawah.
    {
      const _tplInv = process.env.WABA_TPL_INVOICE_KHASANAH || 'invoice_khasanah';
      let _waba = await trySendWabaTemplateForTrip(supabase, pax.trip_id, phone, _tplInv, invoiceParams, { kind: 'waba_invoice' });
      if (!_waba?.ok) { const t = await trySendWabaForTrip(supabase, pax.trip_id, phone, message); if (t?.ok) _waba = t; }
      if (_waba?.ok) {
        revalidatePath(`/finance/payments/${pax.trip_id}`);
        return { ok: true, target: normalizePhone(phone), via: 'waba', link: invoiceLink };
      }
    }
    // URUTAN: WABA (di atas) → Fonnte → manual.
    ...
    if (_picTok) {
      ...
      if (!result.error) { revalidatePath(`/finance/payments/${pax.trip_id}`); return { ok: true, target: normalizePhone(phone), link: invoiceLink, via: 'fonnte' }; }
    }
    if (!opts.forceSend) {
    ...
    return { error: 'WA gagal terkirim (WABA & Fonnte)' };
  } catch (e) {
    return { error: 'Error: ' + (e?.message || 'unknown') };
  }
}

// ============ WABA resmi (Api.co.id) — coba kirim otomatis utk PIC bernomor WABA ============
// Balikan { ok:true } kalau terkirim via WABA, { ok:false } kalau gagal, atau null
// kalau WABA tidak aktif / bukan PIC pemilik nomor (mis. bukan Anis).
export async function tryWabaApproval(passengerId) {
  try {
    if ((!metaConfig() && !apicoidConfig()) || !passengerId) return null; // fallback manual/Fonnte kalau WABA (Api.co.id/Meta) tak aktif
    ...
    if (!pax) return null;
    // Ambil nomor + parameter template konfirmasi.
    const preview = await sendPaymentReceivedWA(passengerId, true);
    if (!preview?.phone || !preview?.wabaParams) return null;
    const tplName = process.env.WABA_TPL_KHASANAH || 'konfirmasi_payment_khasanah';
    // Kirim TEMPLATE dulu (bisa ke peserta yang belum chat). Kalau gagal -> teks
    // (kalau sudah dalam 24 jam window). Kalau dua-duanya gagal -> null (manual).
    let r = await trySendWabaTemplateForTrip(supabase, pax.trip_id, preview.phone, tplName, preview.wabaParams, { kind: 'waba_approval' });
    if (!r?.ok && preview.message) {
      const t = await trySendWabaForTrip(supabase, pax.trip_id, preview.phone, preview.message);
      if (t?.ok) r = t;
    }
    if (r?.ok) return { ok: true, phone: preview.phone, customer_name: preview.customerName };
    return r;
  } catch (e) {
    console.error('[tryWabaApproval]', e?.message);
    return null;
  }
}
...
export async function sendPaymentReceivedWA(passengerId, previewOnly = false, opts = {}) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user && !opts.system) return { error: 'Not authenticated' };
  ...
  try {
    const { data: pax } = await supabase
      .from('trip_passengers')
      .select('id, trip_id, customer_id, price_paid, room_type, discount_amount, age_type, include_visa, visa_ready, include_asuransi, include_evisa, visa_type')
      .eq('id', passengerId)
      .maybeSingle();
    if (!pax) return { error: 'Peserta gak ditemukan' };
    ...
    const appUrl = getAppUrl().replace(/\/$/, '');
    // Invoice yg di-link menentukan NOMINAL yg ditulis (_payAmount di bawah), jadi
    // harus invoice utk pembayaran INI — bukan sekadar yg terbaru. Kalau asal ambil
    // yg terbaru: peserta bayar P1 sementara invoice P1 belum dibuat -> kepilih
    // invoice DP -> pesan menulis "P1: <nominal DP>".
    // Urutan: milestone cocok persis -> cocok sebagian TAPI hanya bila tunggal
    // (>1 kandidat = ambigu, lebih baik jangan menebak) -> terbaru.
    const _pickByMilestone = (list, type) => {
      const t = String(type || '').trim().toLowerCase();
      if (!t || !Array.isArray(list) || !list.length) return null;
      const norm = (x) => String(x?.milestone || '').trim().toLowerCase();
      const exact = list.filter((x) => norm(x) === t);
      if (exact.length) return exact[0]; // list sudah urut created_at desc
      const partial = list.filter((x) => { const m = norm(x); return m && (m.includes(t) || t.includes(m)); });
      return partial.length === 1 ? partial[0] : null;
    };

    // Fallback: peserta tanpa invoice sendiri bisa tercakup di invoice KELUARGA.
    let _all = Array.isArray(invoices) ? [...invoices] : [];
    if (!_all.length) {
      try {
        const { data: famInv } = await supabase.from('invoices')
          .select('*')
          .eq('trip_id', pax.trip_id)
          .contains('covers_passenger_ids', JSON.stringify([pax.id]))
          .order('created_at', { ascending: false });
        if (famInv && famInv.length) _all = famInv;
      } catch {}
    }

    // DUA peran, sengaja dipisah:
    // _payInv  -> sumber NOMINAL. Hanya boleh invoice utk pembayaran INI. Kalau tak
    //             ada yg cocok, biarkan null: lebih baik jatuh ke nominal pembayaran
    //             asli drpd menulis angka invoice lain (dulu: bayar P1 -> kepilih
    //             invoice DP -> pesan menulis "P1: <nominal DP>").
    // _linkInv -> sumber LINK. Boleh invoice mana pun (peserta tetap terbantu
    //             melihat tagihannya), jadi link tak ikut hilang saat milestone
    //             belum punya invoice.
    const _payInv = _pickByMilestone(_all, lastPayment.type);
    const _linkInv = _payInv || _all[0] || null;
    const _invRow = _linkInv;
    const invoiceLink = _linkInv ? `${appUrl}/invoice/${_linkInv.public_token}` : null;
    ...
    if (_invRow && _invRow.is_family_invoice) {
      try {
        ...
        totalBiaya = (Number(bill.expectedTotal) || 0) + (Number(bill.visaExpected) || 0) + (Number(bill.asuransiExpected) || 0) + (Number(bill.evisaExpected) || 0);
    ...
    if (!(totalBiaya > 0)) {
      ...
      const _evisaExp = pax.include_evisa ? (Number(_bd.visa_evisa) || 0) : 0;
      totalBiaya = (Number(n.pokokNet) || 0) + _visaExp + _asrExp + _evisaExp;
    ...
    // Nominal dari invoice utk pembayaran INI (family = total semua anggota, bukan 1 org).
    // Sengaja _payInv, BUKAN _invRow: invoice yg di-link boleh beda milestone.
    const _payAmount = (_payInv && Number(_payInv.amount) > 0) ? Number(_payInv.amount) : (Number(lastPayment.amount) || 0);
    ...
    // Parameter utk template WABA (konfirmasi_payment_khasanah): {{1}}nama {{2}}pic
    // {{3}}jenis {{4}}trip {{5}}nominal(tanpa Rp) {{6}}link
    const _tripLabelW = `${trip?.kode_trip || ''} ${trip?.name || ''}`.trim();
    const _amountW = String(fmtRp(_payAmount)).replace(/^Rp\s*/, '');
    const wabaParams = [(customer.name ? 'Kak ' + customer.name : 'Kak'), _picName || '-', _payType || 'Pembayaran', _tripLabelW || '-', _amountW || '-', invoiceLink || '-'];

    if (previewOnly) return { ok: true, preview: true, message, phone: normalizePhone(phone), customerName: customer.name, wabaParams };
    // WABA (Api.co.id) SELALU dicoba dulu: template konfirmasi -> teks -> baru manual/Fonnte.
    {
      const _tplK = process.env.WABA_TPL_KHASANAH || 'konfirmasi_payment_khasanah';
      let _waba = await trySendWabaTemplateForTrip(supabase, pax.trip_id, phone, _tplK, wabaParams, { kind: 'waba_approval' });
      if (!_waba?.ok) { const t = await trySendWabaForTrip(supabase, pax.trip_id, phone, message); if (t?.ok) _waba = t; }
      if (_waba?.ok) {
        try { if (_invRow?.id) await supabase.from('invoices').update({ wa_sent: true }).eq('id', _invRow.id); } catch {}
        revalidatePath(`/finance/payments/${pax.trip_id}`);
        return { ok: true, target: normalizePhone(phone), via: 'waba' };
      }
    }
    // URUTAN: WABA (di atas) → Fonnte → manual. Coba Fonnte dulu; kalau gagal/PIC tak
    // punya token, baru masuk antrean manual (kecuali forceSend).
    const _picTok = await getPicFonnteTokenById(supabase, pax.trip_id);
    if (_picTok) {
      const result = await sendFonnte(phone, message, { context: 'finance', token: _picTok });
      if (!result.error) { revalidatePath(`/finance/payments/${pax.trip_id}`); return { ok: true, target: normalizePhone(phone), lunas: isLunas, via: 'fonnte' }; }
    }
    if (!opts.forceSend) {
      if (opts.queueManual) await queueManualWA(supabase, { phone: normalizePhone(phone), message, kind: opts.queueKind, tripId: pax.trip_id, meta: opts.meta || null });
      return { ok: true, wa_manual: true, wa_message: message, wa_phone: normalizePhone(phone), customer_name: customer.name };
