...
function roomKeyOf(rt) { return String(rt || '').toLowerCase().replace(/[^a-z_]/g, ''); }
...
function invoiceNumbers(pax, breakdown, payments) {
  ...
  if (pokokGross <= 0) pokokGross = roomPrice + tips + cityTax; // fallback kalau Harga Bayar belum diisi
  ...
  return { roomPrice, tips, cityTax, discount, pokokGross, pokokNet, pokokPaid, addonPaid, sisa, totalBayar };
}
function detailPaketLines(n) {
  const L = [];
  if (n.roomPrice > 0) L.push(`   • Paket kamar: ${fmtRp(n.roomPrice)}`);
