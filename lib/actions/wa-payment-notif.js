...
export async function sendInvoiceWA(passengerId) {
  ...
  try {
    ...
    const message = [
      ...
      `   Sudah Dibayar: ${fmtRp(totalBayar)}`,
      `   *Sisa: ${fmtRp(sisa)}*`,
      ``,
    ...
    const result = await sendFonnte(phone, message, { context: 'payment' });
...
export async function sendPaymentReceivedWA(passengerId) {
  ...
  try {
    ...
    const message = [
      ...
      `   Total Tagihan: ${fmtRp(pax.price_paid || 0)}`,
      `   Sudah Dibayar: ${fmtRp(totalBayar)}`,
      isLunas ? `   *Status: LUNAS ✅*` : `   *Sisa: ${fmtRp(sisa)}*`,
      ``,
    ...
    for (const p of paxList) {
      try {
        const { data: pays } = await supabase
          .from('participant_payments').select('amount').eq('passenger_id', p.id);
