...
async function getPicWabaKey(supabase, tripId) {
  try {
    ...
    let emp = null;
    if (email) {
      const r = await supabase.from('employees').select('waba_api_key, full_name').ilike('email', email).maybeSingle();
      emp = r.data;
    }
    if (!emp && name) {
      let r = await supabase.from('employees').select('waba_api_key, full_name').ilike('full_name', name).maybeSingle();
      emp = r.data;
      if (!emp) { r = await supabase.from('employees').select('waba_api_key, full_name').ilike('nickname', name).maybeSingle(); emp = r.data; }
    }
    const key = emp?.waba_api_key ? String(emp.waba_api_key).trim() : '';
    return key ? { key, pic: emp.full_name || name } : null;
  } catch { return null; }
}
...
export async function tryWabaApproval(passengerId) {
  try {
    ...
    const r = await sendWabaText({ key: picKey.key, endpoint: cfg.endpoint }, preview.phone, preview.message);
    if (r.ok) {
      try {
