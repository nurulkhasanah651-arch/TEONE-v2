...
export async function sendInvoiceWA(passengerId, previewOnly = false, opts = {}) {
  ...
  try {
    ...
    const invoiceParams = [customer.name || '-', _picNameInv || '-', _tripLabelInv || '-', targetInvoice?.invoice_no || '-', targetInvoice?.milestone || '-', invoiceLink || '-'];
...
export async function sendPaymentReceivedWA(passengerId, previewOnly = false, opts = {}) {
  ...
  try {
    ...
    const wabaParams = [customer.name || '-', _picName || '-', _payType || 'Pembayaran', _tripLabelW || '-', _amountW || '-', invoiceLink || '-'];
