...
export async function tryWabaApproval(passengerId) {
  try {
    ...
    if (!pax) return null;
    // Reuse builder konfirmasi: ambil pesan + nomor persis seperti yang akan dikirim.
    const preview = await sendPaymentReceivedWA(passengerId, true);
    if (!preview?.message || !preview?.phone) return null;
    const r = await trySendWabaForTrip(supabase, pax.trip_id, preview.phone, preview.message);
