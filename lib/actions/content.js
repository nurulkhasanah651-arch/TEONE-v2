...
export async function refreshIgData(force = false) {
  ...
  try {
    ...
    // 4) Insights per media (reach, saved, shares, views/impressions) — best effort.
    // Degradasi bertahap: kalau satu set metric ditolak akun, coba set lebih kecil,
    // supaya reach/saved TIDAK ikut hilang saat 'views'/'shares' tak didukung.
    await Promise.all(items.map(async (m) => {
      const sets = m.media_type === 'VIDEO'
        ? ['reach,saved,shares,views', 'reach,saved,shares,plays', 'reach,saved,plays', 'reach,saved']
        : ['reach,saved,shares,views', 'reach,saved,shares,impressions', 'reach,saved,impressions', 'reach,saved'];
      for (const metric of sets) {
        try {
          const r = await fetch(`${GRAPH}/${m.id}/insights?metric=${metric}&access_token=${encodeURIComponent(token)}`);
          const j = await r.json();
          if (j.error || !Array.isArray(j.data)) continue;
          for (const d of j.data) m[d.name] = d.values?.[0]?.value ?? null;
          break; // set berhasil → berhenti
        } catch {}
      }
      // Normalisasi: views = views | impressions | plays (biar kolom Views konsisten)
      if (m.views == null) m.views = (m.impressions ?? m.plays ?? null);
    }));

    // PENGAMAN: kalau daftar media balik KOSONG (API flaky / permission), JANGAN timpa post lama
    let finalItems = items;
    let mediaPreserved = false;
    if (items.length === 0) {
      const { data: prev } = await svc.from('ig_cache').select('data').eq('key', 'overview').maybeSingle();
      const pm = prev?.data?.media;
      if (Array.isArray(pm) && pm.length > 0) { finalItems = pm; mediaPreserved = true; }
    }
    ...
    const overview = {
      ...
      reach30,
      media: finalItems,
      media_preserved: mediaPreserved,
    };
    await svc.from('ig_cache').upsert({ key: 'overview', data: overview, fetched_at: new Date().toISOString() }, { onConflict: 'key' });
    revalidatePath('/content');
    return { ok: true, media_count: finalItems.length, media_preserved: mediaPreserved };
  } catch (e) {
