...
import { getFonnteToken, sendFonnte } from '@/lib/utils/fonnte';
...
async function sendFonnteMessage(phone, message) {
  // Kebijakan: WA assign TL SELALU dari nomor CS TEONE untuk SEMUA brand (TE & Khasanah).
  // Jadi brand dipaksa 'teone' → token FONNTE_TOKEN_CS (bukan *_KHASANAH).
  const r = await sendFonnte(phone, message, { context: 'cs', brand: 'teone' });
...
— Traveling Eropa One System`;
  ...
  return { ok: true, tlName: tl.name, phone, sentVia: result.sentVia || 'cs' };
}
