...
async function sendFonnteMessage(phone, message) {
  ...
  const { token } = getFonnteToken('cs', (() => { try { return currentBrandCode(); } catch { return ''; } })());
  if (!token) {
    return { error: 'Fonnte token CS belum di-set (FONNTE_TOKEN_CS / FONNTE_TOKEN_FINANCE / FONNTE_TOKEN).' };
  }
