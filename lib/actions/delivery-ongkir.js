...
// R209 + R210 + R212 v2: Ongkir + Cash Out + Family Invoice
// v2 FIX: pakai service client (bypass RLS) supaya Cash Out insert berhasil
...
/**
 * Insert Cash Out ongkir ke accounting_entries
 * Try multiple schemas — return detailed result so caller bisa surface error
 */
async function insertCashOut(supabase, tripId, passengerId, customerName, courier, resi, amount, createdBy, familyMemberCount = 1) {
  ...
  try {
    ...
    if (!error) return { ok: true, attempt: 'accounting_entries (full)', payload: payload1 };
    errors.push(`accounting_entries (full): ${error.message}`);
  } catch (e) {
    errors.push(`accounting_entries exception: ${e?.message || String(e)}`);
  }
  ...
  try {
    ...
    const { error } = await supabase.from('accounting_entries').insert(payload2);
    if (!error) return { ok: true, attempt: 'accounting_entries (no trip_id)', payload: payload2 };
    errors.push(`no trip_id: ${error.message}`);
  } catch (e) {
    errors.push(`no trip_id exception: ${e?.message || String(e)}`);
  }

  // Try 3: pakai direction='out'
  try {
    const payload3 = {
      date: today,
      direction: 'out',
    ...
    const { error } = await supabase.from('accounting_entries').insert(payload3);
    if (!error) return { ok: true, attempt: 'accounting_entries (direction)', payload: payload3 };
    errors.push(`direction: ${error.message}`);
  } catch (e) {
    errors.push(`direction exception: ${e?.message || String(e)}`);
  }

  // Try 4: minimal — cuma date + amount + description
  try {
    const payload4 = {
      date: today,
    ...
    const { error } = await supabase.from('accounting_entries').insert(payload4);
    if (!error) return { ok: true, attempt: 'accounting_entries (minimal)', payload: payload4 };
    errors.push(`minimal: ${error.message}`);
  } catch (e) {
    errors.push(`minimal exception: ${e?.message || String(e)}`);
  }
...
export async function createAndSendOngkirInvoice(passengerId, ongkirAmount, courier, resi) {
  ...
  // v2 FIX: pakai service client (bypass RLS untuk insert ke accounting_entries)
  ...
  if (!phone) {
    return {
      ...
      cash_out: cashOutResult.ok,
      cash_out_error: cashOutResult.reason || null,
      cash_out_attempt: cashOutResult.attempt || null,
      wa_sent: false,
      wa_error: 'No phone',
