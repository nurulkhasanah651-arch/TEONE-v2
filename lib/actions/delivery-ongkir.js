...
async function sendFonnte(phone, message, tokenOverride) {
  ...
  try {
    ...
    if (!res.ok || data.status === false) {
      ...
      try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'delivery', phone, message, status: 'failed', state: 'failed', reason: _werr }); } catch {}
      return { error: _werr };
    }
    const _wfid = Array.isArray(data.id) ? data.id[0] : (data.id || null);
    try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'delivery', phone, message, status: 'sent', state: 'sent', fonnteId: _wfid }); } catch {}
    return { ok: true };
  } catch (e) {
