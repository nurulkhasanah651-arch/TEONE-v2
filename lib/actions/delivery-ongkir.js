...
async function sendFonnte(phone, message, tokenOverride) {
  ...
  try {
    ...
    if (!res.ok || data.status === false) {
      ...
      try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'delivery', phone, message, status: 'failed', state: 'failed', reason: _werr, senderToken: token }); } catch {}
      return { error: _werr };
    }
    const _wfid = Array.isArray(data.id) ? data.id[0] : (data.id || null);
    try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'delivery', phone, message, status: 'sent', state: 'sent', fonnteId: _wfid, senderToken: token }); } catch {}
    return { ok: true };
  } catch (e) {
...
export async function createAndSendOngkirInvoice(passengerId, ongkirAmount, courier, resi) {
  ...
  const invoice_no = `${getBrandCodeSafe() === 'khasanah' ? 'KT' : 'TEONE'}-${kode}-${seq}`;
  ...
  const message = plainForBrand(`${_salamWord()} ${recipient},${_picNm ? `\nSaya *${_picNm}* dari Khasanah Travel \ud83d\ude4f` : ''}
...
${companyName}`);
  ...
  // PIC kirim manual -> jangan kirim & jangan antre; balikan template supaya
  // muncul modal copy-paste di panel Perlengkapan (sama seperti invoice).
  // WABA: template resmi dulu. Param ongkir_<brand>: {{1}}Kak nama {{2}}trip {{3}}jumlah {{4}}link (tanpa tombol).
  {
    const _codeO = getBrandCodeSafe();
    const _tplOngkir = process.env[`WABA_TPL_ONGKIR_${_codeO === 'khasanah' ? 'KHASANAH' : 'TEONE'}`] || `ongkir_${_codeO}`;
    const _tripLabelO = `${trip?.kode_trip || ''} ${trip?.name || ''}`.trim();
    const _paramsO = [(recipient ? 'Kak ' + recipient : 'Kak'), _tripLabelO || '-', fmtRupiah(amount), invoiceLink];
    const _wtplO = await trySendWabaTemplateForTrip(supabase, pax.trip_id, normalizePhone(phone), _tplOngkir, _paramsO, { kind: 'waba_ongkir' });
    if (_wtplO?.ok) {
      revalidatePath('/invoices');
      if (trip.id) revalidatePath(`/finance/payments/${trip.id}`);
      return {
        ok: true, via: 'waba', wa_phone: normalizePhone(phone),
        invoice_id: invData.id, invoice_no: invData.invoice_no,
        family_invoice: isFamilyInvoice, family_count: familyMemberCount,
        cash_out: cashOutResult?.ok || false, cash_out_error: cashOutResult?.reason || null,
      };
    }
  }
  const _wabaOngkir = await trySendWabaForTrip(supabase, pax.trip_id, normalizePhone(phone), message, { context: 'cs', kind: 'waba_ongkir' });
  if (_wabaOngkir?.ok) {
    revalidatePath('/invoices');
    if (trip.id) revalidatePath(`/finance/payments/${trip.id}`);
    return {
  ...
  if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
    ...
    if (trip.id) revalidatePath(`/finance/payments/${trip.id}`);
    return {
      ok: true, wa_manual: true, wa_message: message, wa_phone: normalizePhone(phone),
      customer_name: recipient || '',
      invoice_id: invData.id, invoice_no: invData.invoice_no,
      family_invoice: isFamilyInvoice, family_count: familyMemberCount,
      cash_out: cashOutResult?.ok || false,
      cash_out_error: cashOutResult?.reason || null,
    };
  }
