...
${companyName}`);
  ...
  // PIC kirim manual -> jangan kirim, antrekan + balikan template.
  if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
    await queueManualWA(supabase, { phone: normalizePhone(phone), message, kind: 'manual_pending_ongkir', context: 'finance', tripId: pax.trip_id });
    return { ok: true, wa_manual: true, wa_message: message, wa_phone: normalizePhone(phone) };
  }
