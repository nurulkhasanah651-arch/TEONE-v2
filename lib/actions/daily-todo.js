...
const ROLES_SEE_ALL = ['owner', 'manager', 'accounting']; // manajemen lihat semua laporan
...
export async function getDailyTodos(dateStr) {
  ...
  const { data: rows, error } = await db.from('daily_todos')
    .select('id, author_id, author_name, author_role, content, done, note, created_at')
    .eq('todo_date', date)
    .order('created_at', { ascending: true });
  if (error) return { error: error.message };

  const mapItem = (r) => ({ id: r.id, content: r.content, done: r.done === true, note: r.note || '', authorId: r.author_id });
...
export async function toggleTodo(id, done) {
  ...
  const { error } = await db.from('daily_todos').update({ done: !!done, updated_at: new Date().toISOString() }).eq('id', rid);
  if (error) return { error: error.message };
  revalidatePath('/daily-todo');
  return { ok: true };
}
