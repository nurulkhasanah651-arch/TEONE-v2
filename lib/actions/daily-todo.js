...
import { TODO_KIND_KEYS } from '@/lib/utils/daily-todo-kinds';
...
// Alur laporan to-do: siapa (viewer role) yang MENERIMA laporan to-do dari tiap AUTHOR role.
// - PIC / ops / cs (tim lapangan) → laporannya diterima manager, owner, accounting.
// - Manager → laporannya HANYA diterima owner.
// - Accounting → hanya owner.
// Owner selalu melihat semua laporan.
const REPORT_TO = {
  pic: ['owner', 'manager', 'accounting'],
  ops: ['owner', 'manager', 'accounting'],
  cs: ['owner', 'manager', 'accounting'],
  manager: ['owner'],
  accounting: ['owner'],
};
// Role yang punya panel "Laporan Tim" (menerima laporan dari role lain).
const ROLES_RECEIVE = ['owner', 'manager', 'accounting'];
function canReceive(viewerRole, authorRole) {
  if (viewerRole === 'owner') return true;
  return (REPORT_TO[authorRole] || []).includes(viewerRole);
}
...
export async function getDailyTodos(dateStr) {
  ...
  const canSeeAll = ROLES_RECEIVE.includes(role);

  const { data: rows, error } = await db.from('daily_todos')
    .select('id, author_id, author_name, author_role, content, done, note, created_at, done_at, durasi_text, kind')
    .eq('todo_date', date)
    .order('created_at', { ascending: true });
  if (error) return { error: error.message };

  const mapItem = (r) => ({ id: r.id, content: r.content, done: r.done === true, note: r.note || '', authorId: r.author_id, createdAt: r.created_at, durasi: r.durasi_text || '', kind: r.kind || '' });
  const mine = (rows || []).filter((r) => r.author_id === user.id).map(mapItem);

  // Laporan yang BOLEH diterima viewer ini, sesuai alur REPORT_TO (owner lihat semua).
  let others = [];
  if (canSeeAll) {
    const byAuthor = {};
    for (const r of (rows || [])) {
      if (r.author_id === user.id) continue;
      if (!canReceive(role, r.author_role)) continue;
...
export async function addTodo(dateStr, content) {
  ...
  if (error) return { error: error.message };
  revalidatePath('/manager-dashboard'); revalidatePath('/pic-dashboard');
  return { ok: true };
}
...
export async function updateTodo(id, fields) {
  ...
  if (typeof fields?.durasi === 'string') upd.durasi_text = fields.durasi.slice(0, 100);
  if (typeof fields?.kind === 'string') { const k = fields.kind.trim(); upd.kind = (k === '' || TODO_KIND_KEYS.includes(k)) ? (k || null) : null; }
