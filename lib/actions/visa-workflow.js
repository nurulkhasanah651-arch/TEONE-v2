...
export async function sendVisaWA({ tripId, passengerIds, templateKey, customVars = {}, familyAware = false }) {
  ...
  for (const pax of targetPaxList) {
    ...
    let _wabaSent = false;
    {
      ...
      let _tpl = null, _params = null, _suffix = null;
      if (effectiveTemplateKey === 'doc_collection') {
        _tpl = _envTpl('VISA_DOC') || `visa_doc_collection_${_code}`;
        _params = [cust.name || 'Kak', _intro, paxVars.country_name || 'Negara Tujuan', _tripLabel || '-', paxVars.deadline_dokumen || '-'];
        _suffix = pax.visa_upload_token || null;
      } else if (effectiveTemplateKey === 'doc_collection_no_biometric') {
        _tpl = _envTpl('VISA_DOCNB') || `visa_doc_nonbiometric_${_code}`;
        _params = [cust.name || 'Kak', _intro, paxVars.country_name || 'Negara Tujuan', _tripLabel || '-', paxVars.deadline_dokumen || '-'];
        _suffix = pax.visa_upload_token || null;
      } else if (effectiveTemplateKey === 'biometric_schedule_info') {
        _tpl = _envTpl('VISA_BIO') || `visa_biometric_schedule_${_code}`;
        _params = [cust.name || 'Kak', _intro, paxVars.country_name || 'Negara Tujuan', _fmtD(paxVars.tanggal_biometrik), paxVars.jam_biometrik || '-', paxVars.lokasi_biometrik || trip.visa_biometric_location || '-'];
        _suffix = null;
      }
      if (_tpl && (_suffix || effectiveTemplateKey === 'biometric_schedule_info')) {
        try {
          const _wr = await trySendWabaTemplateForTrip(supabase, tripId, toPhone, _tpl, _params, { kind: 'waba_visa', buttonUrlSuffix: _suffix });
