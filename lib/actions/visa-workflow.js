...
import { signedVisaResultUrl } from '@/lib/actions/visa-storage';
...
export async function sendVisaWA({ tripId, passengerIds, templateKey, customVars = {}, familyAware = false }) {
  ...
  for (const pax of targetPaxList) {
    ...
    // bucket private: buat link aman 7 hari utk foto hasil visa (kalau ada)
    const signedPhoto = pax.visa_result_photo_url ? await signedVisaResultUrl(pax.visa_result_photo_url, 7 * 24 * 3600) : null;
