...
async function fonnteSend(targetPhone, message, attachmentUrl = null) {
  // Pesan visa → nomor CS Visa (context 'visa'), brand-aware
  const { token } = getFonnteToken('visa', currentBrandCode());
  ...
  for (const pax of targetPaxList) {
    ...
    const paxVars = {
      ...
      visa_intro: (() => {
        ...
        const nm = (c === 'khasanah' ? process.env.WA_VISA_NAME_KHASANAH : process.env.WA_VISA_NAME) || 'Ibti';
    ...
    const sendRes = await fonnteSend(toPhone, message, attachmentUrl);
