...
async function fonnteSend(targetPhone, message, attachmentUrl = null, tokenOverride = null) {
  ...
  try {
    ...
    if (result.status === true || result.status === 'true') {
      ...
      try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'visa', phone: targetPhone, message, status: 'sent', state: 'sent', fonnteId: _wfid }); } catch {}
      return { ok: true, fonnte: result };
    }
    const _werr = 'Fonnte: ' + (result.reason || result.detail || 'unknown');
    try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'visa', phone: targetPhone, message, status: 'failed', state: 'failed', reason: _werr }); } catch {}
    return { error: _werr };
  } catch (e) {
