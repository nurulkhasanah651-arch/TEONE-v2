...
import { trySendWabaTemplateForTrip } from '@/lib/utils/waba-send';
import { getFonnteToken } from '@/lib/utils/fonnte';
import { syaratLinksFor, fillableDocsFor, syaratCountryKey } from '@/lib/utils/visa-syarat-docs';
...
async function fonnteSend(targetPhone, message, attachmentUrl = null, tokenOverride = null) {
  ...
  try {
    ...
    if (result.status === true || result.status === 'true') {
      const _wfid = Array.isArray(result.id) ? result.id[0] : (result.id || null);
      try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'visa', phone: targetPhone, message, status: 'sent', state: 'sent', fonnteId: _wfid, senderToken: token }); } catch {}
      return { ok: true, fonnte: result };
    }
    const _werr = 'Fonnte: ' + (result.reason || result.detail || 'unknown');
    try { const _wm = await import('@/lib/wa-outbox-log'); await _wm.logWA({ context: 'visa', phone: targetPhone, message, status: 'failed', state: 'failed', reason: _werr, senderToken: token }); } catch {}
    return { error: _werr };
  } catch (e) {
...
// ============================================================
export async function sendVisaWA({ tripId, passengerIds, templateKey, customVars = {}, familyAware = false, resultDocId = null }) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const supabase = getServiceClient() || authClient;
  ...
  const syaratDocs = syaratLinksFor(trip.visa_country, siteBase);
  const fillableDocs = fillableDocsFor(trip.visa_country, siteBase);
  const results = [];
  for (const pax of targetPaxList) {
    ...
    // Visa tambahan (multi): kalau kirim untuk 1 dok tertentu (resultDocId),
    // pakai FOTO & DETAIL dok itu (bukan hasil primary). URL foto = signed 7 hari.
    const _docOv = resultDocId
      ? (Array.isArray(pax.visa_result_docs) ? pax.visa_result_docs : []).find((x) => x && x.id === resultDocId)
      : null;
    if (_docOv) {
      signedPhoto = null;
      if (_docOv.file_path) {
        try {
          const { data: _su } = await supabase.storage.from('visa-results').createSignedUrl(_docOv.file_path, 60 * 60 * 24 * 7);
          signedPhoto = _su?.signedUrl || null;
    ...
    const paxVars = {
      ...baseVars,
      syarat_docs: syaratDocs,
      fillable_docs: fillableDocs,
      ...
      visa_valid_from: _docOv ? _docOv.valid_from : pax.visa_valid_from,
      visa_valid_until: _docOv ? _docOv.valid_until : pax.visa_valid_until,
      visa_entry_type: _docOv ? _docOv.entry_type : pax.visa_entry_type,
      return_kurir: pax.visa_return_kurir,
      return_resi: pax.visa_return_resi,
      rejection_reason: _docOv ? _docOv.rejection_reason : pax.visa_rejection_reason,
      rejection_solution: _docOv ? _docOv.rejection_solution : pax.visa_rejection_solution,
      refund_amount: _docOv ? _docOv.refund_amount : pax.visa_refund_amount,
      country_name: _docOv ? (_docOv.label || baseVars.country_name) : baseVars.country_name,
      ...
      visa_intro: (() => {
        ...
        return _picName ? `Perkenalkan, saya *${_picName}*, PIC trip kamu di ${b} \ud83d\ude4f` : `Perkenalkan, kami tim ${b} \ud83d\ude4f`;
      })(),
      ...
      upload_portal_url: pax.visa_upload_token
        ...
        : '(belum di-generate)',
      // Link isi alamat pengiriman paspor (auto-sinkron ke Finance checklist)
      delivery_link: pax.delivery_token ? `${siteBase}/delivery/${pax.delivery_token}` : null,
    };

    // R215o: Render dgn trip param untuk pakai override
    let message = renderTemplate(effectiveTemplateKey, paxVars, trip);
    // SAFETY: kalau template (custom) menaruh link upload dgn placeholder rusak
    // (mis. /visa/upload/{{AKAN_AUTO_GENERATE}}), ganti dgn token asli peserta.
    if (pax.visa_upload_token) {
      message = message.replace(/\/visa\/upload\/\{\{[^}]*\}\}/g, `/visa/upload/${pax.visa_upload_token}`);
    }
    // Hapus sisa placeholder {{...}} yg tak dikenal supaya tak muncul mentah di WA
    ...
    // WABA: kalau PIC trip pakai nomor WABA & ada template resmi utk jenis ini, coba template dulu
    // (mendukung kirim ke peserta yg belum chat / di luar 24 jam). Kalau tak ada nomor WABA
    // (trySendWabaTemplateForTrip -> null) atau template gagal, jatuh ke Fonnte spt biasa (additive).
    let _wabaSent = false;
    {
      const _code = (() => { try { return currentBrandCode(); } catch { return 'teone'; } })();
      const _brandName = _code === 'khasanah' ? 'Khasanah Travel' : 'Traveling Eropa';
      const _intro = _picName ? `Saya ${_picName}, PIC trip kamu di ${_brandName}.` : `Kami tim ${_brandName}.`;
      const _tripLabel = `${trip.kode_trip ? trip.kode_trip + ' \u2014 ' : ''}${trip.name || ''}`.trim();
      const _envTpl = (v) => process.env[`WABA_TPL_${v}_${_code === 'khasanah' ? 'KHASANAH' : 'TEONE'}`];
      const _fmtD = (d) => { try { return d ? new Date(d).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : '-'; } catch { return String(d || '-'); } };
      // Gabung daftar (dokumen/anggota/kekurangan) jadi 1 baris dipisah koma (Meta tolak multi-baris).
      const _csv = (s) => String(s || '').split('\n').map((x) => x.replace(/^[\s\-•☐📸📝]+/, '').trim()).filter(Boolean).join(', ');
      const _nama = cust.name || 'Kak';
      const _neg = paxVars.country_name || 'Negara Tujuan';
      const _deadline = paxVars.deadline_dokumen || customVars.deadline_dokumen || '-';
      const _lokasi = paxVars.lokasi_biometrik || trip.visa_biometric_location || '-';
      const _countryKey = (() => { try { return syaratCountryKey(trip.visa_country); } catch { return null; } })();
      let _tpl = null, _params = null, _suffix = null, _kind = null, _hdr = null, _ctry = null;
      switch (effectiveTemplateKey) {
        case 'doc_collection':
          _kind = 'waba_visa_dokumen'; _tpl = `visa_dokumen_${_code}`; _ctry = _countryKey;
          _params = [_nama, _picName || 'Tim', trip.name || _tripLabel || '-', _fmtD(trip.departure), _deadline, paxVars.upload_portal_url || '-'];
          break;
        case 'doc_collection_no_biometric':
          _kind = 'waba_visa_nonbio'; _tpl = `visa_nonbio_${_code}`;
          // Non-biometrik: 1 template generik. Negara = assigned (variabel), TANPA link syarat,
          // daftar dokumen = template dokumen tersimpan (CSV). 8 var:
          // {{1}}nama {{2}}PIC {{3}}negara {{4}}trip {{5}}tgl berangkat {{6}}daftar dokumen {{7}}link upload {{8}}deadline
          _params = [_nama, _picName || 'Tim', _neg, trip.name || _tripLabel || '-', _fmtD(trip.departure), _csv(paxVars.list_dokumen) || '-', paxVars.upload_portal_url || '-', _deadline];
          break;
        case 'doc_received':
          _kind = 'waba_visa_diterima'; _tpl = `visa_diterima_${_code}`;
          _params = [_nama, _tripLabel || '-']; break;
        case 'doc_kurang':
          _kind = 'waba_visa_kurang'; _tpl = `visa_kurang_${_code}`; _ctry = _countryKey;
          _params = [_nama, _intro, _neg, _csv(paxVars.list_dokumen_kurang) || '-', _deadline]; _suffix = pax.visa_upload_token || null; break;
        case 'biometric_schedule_info':
          _kind = 'waba_visa_jadwalbio'; _tpl = `visa_jadwalbio_${_code}`;
          _params = [_nama, _intro, _neg, _fmtD(paxVars.tanggal_biometrik), paxVars.jam_biometrik || '-', _lokasi]; break;
        case 'biometric_reminder':
          _kind = 'waba_visa_reminderbio'; _tpl = `visa_reminderbio_${_code}`;
          _params = [_nama, _intro, _neg, _fmtD(paxVars.tanggal_biometrik), paxVars.jam_biometrik || '-', _lokasi]; break;
        case 'visa_approved':
          _kind = 'waba_visa_approved'; _tpl = `visa_approved_${_code}`;
          _params = [_nama, _neg, _fmtD(paxVars.visa_valid_from), _fmtD(paxVars.visa_valid_until), paxVars.visa_entry_type || '-'];
          _suffix = pax.delivery_token || null; _hdr = signedPhoto || null; break;
        case 'visa_rejected':
          _kind = 'waba_visa_rejected'; _tpl = `visa_rejected_${_code}`;
          _params = [_nama, _neg, _tripLabel || '-', paxVars.rejection_reason || '-', paxVars.rejection_solution || '-'];
          _suffix = pax.delivery_token || null; _hdr = signedPhoto || null; break;
        case 'family_announcement':
          _kind = 'waba_visa_family'; _tpl = `visa_family_${_code}`;
          _params = [_nama, _intro, _tripLabel || '-', _csv(paxVars.list_nama_anggota_family) || _nama, trip.visa_pickup_address || '-', _deadline];
          _suffix = pax.visa_upload_token || null; break;
        default: break;
      }
      if (_kind) {
        try {
          const _wr = await trySendWabaTemplateForTrip(supabase, tripId, toPhone, _tpl, _params, { kind: _kind, buttonUrlSuffix: _suffix, headerImageUrl: _hdr, countrySuffix: _ctry });
          if (_wr?.ok) _wabaSent = true;
        } catch (e) { console.error('[sendVisaWA WABA template]', e?.message); }
      }
    }
    const sendRes = _wabaSent ? { ok: true, via: 'waba' } : await fonnteSend(toPhone, message, attachmentUrl, _picTok);
...
export async function uploadVisaResult(passengerId, fileUrl, result, extras = {}) {
  ...
  if (result === 'rejected') {
    if (extras.rejection_reason) update.visa_rejection_reason = extras.rejection_reason;
    if (extras.rejection_solution !== undefined) update.visa_rejection_solution = extras.rejection_solution || null;
    if (extras.refund_amount !== undefined) update.visa_refund_amount = (Number(extras.refund_amount) > 0 ? Number(extras.refund_amount) : null);
  }
...
// ============================================================
// VISA RESULT DOCS (multi) - 1 peserta bisa >1 visa (mis. Schengen + UK)
// Additive: disimpan di trip_passengers.visa_result_docs (jsonb array).
// Tidak mengganggu hasil primary (kolom tunggal) yg dipakai kirim WA.
// ============================================================
export async function addVisaResultDoc(passengerId, entry = {}) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const supabase = getServiceClient() || authClient;
  ...
  if (!['approved', 'rejected'].includes(result)) return { error: 'Result harus approved atau rejected' };
  if (!entry.file_path) return { error: 'File visa wajib diupload dulu' };

  const { data: pax } = await supabase.from('trip_passengers').select('trip_id, visa_result_docs').eq('id', passengerId).maybeSingle();
  if (!pax) return { error: 'Peserta gak ketemu' };

  const list = Array.isArray(pax.visa_result_docs) ? pax.visa_result_docs : [];
  const doc = {
    id: `vrd_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    label: String(entry.label || '').trim() || 'Visa',
    result,
    file_path: entry.file_path,
    valid_from: result === 'approved' ? (entry.valid_from || null) : null,
    valid_until: result === 'approved' ? (entry.valid_until || null) : null,
    entry_type: result === 'approved' ? (entry.entry_type || null) : null,
    rejection_reason: result === 'rejected' ? (entry.rejection_reason || null) : null,
    rejection_solution: result === 'rejected' ? (entry.rejection_solution || null) : null,
    refund_amount: result === 'rejected' ? (Number(entry.refund_amount) > 0 ? Number(entry.refund_amount) : null) : null,
    uploaded_at: new Date().toISOString(),
    uploaded_by: user.user_metadata?.full_name || user.email || 'staff',
  };
  const next = [...list, doc];
  const { error } = await supabase.from('trip_passengers').update({ visa_result_docs: next }).eq('id', passengerId);
  if (error) return { error: 'Update failed: ' + error.message };
  revalidatePath(`/visa/${pax.trip_id}`);
  return { ok: true, doc };
}

export async function deleteVisaResultDoc(passengerId, docId) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const supabase = getServiceClient() || authClient;
  ...
  if (!pax) return { error: 'Peserta gak ketemu' };
  const list = Array.isArray(pax.visa_result_docs) ? pax.visa_result_docs : [];
  const next = list.filter((d) => d && d.id !== docId);
  const { error } = await supabase.from('trip_passengers').update({ visa_result_docs: next }).eq('id', passengerId);
  if (error) return { error: 'Update failed: ' + error.message };
  revalidatePath(`/visa/${pax.trip_id}`);
  return { ok: true };
