...
import { syaratLinksFor, fillableDocsFor } from '@/lib/utils/visa-syarat-docs';
...
export async function sendVisaWA({ tripId, passengerIds, templateKey, customVars = {}, familyAware = false }) {
  ...
  for (const pax of targetPaxList) {
    ...
    let _wabaSent = false;
    {
      ...
      let _tpl = null, _params = null, _suffix = null, _kind = null, _hdr = null;
      switch (effectiveTemplateKey) {
        case 'doc_collection':
          _kind = 'waba_visa_dokumen'; _tpl = `visa_dokumen_${_code}`;
          _params = [_nama, _intro, _neg, _tripLabel || '-', _deadline]; _suffix = pax.visa_upload_token || null; break;
        ...
        case 'doc_kurang':
          _kind = 'waba_visa_kurang'; _tpl = `visa_kurang_${_code}`;
      ...
      if (_kind) {
        try {
          const _wr = await trySendWabaTemplateForTrip(supabase, tripId, toPhone, _tpl, _params, { kind: _kind, buttonUrlSuffix: _suffix, headerImageUrl: _hdr });
