...
export async function requestVisaCostToFinance(passengerId, type, amount, vendor, notes) {
  ...
  for (const status of PAYMENT_STATUS_CANDIDATES) {
    ...
    const { data, error } = await supabase.from('trip_finance_items').insert(payload).select().maybeSingle();
  ...
  return { ok: true, hpp_item: inserted, message: `HPP item dibuat — klik Request DP di Finance` };
}
...
async function fonnteSend(targetPhone, message, attachmentUrl = null) {
  const token = getFonnteToken('visa').token;
  if (!token) return { error: 'Token Fonnte belum di-set (cek env FONNTE_TOKEN_*)' };
