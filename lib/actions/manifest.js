...
export async function getRoomlistRows(tripId) {
  ...
  try {
    ...
    let cmap = {};
    if (ids.length > 0) {
      const { data: cust } = await supabase.from('customers').select('id, name, gender, sex').in('id', ids);
      cmap = Object.fromEntries((cust || []).map((c) => [c.id, c]));
    }
    ...
    return { ok: true, trip: { name: trip.name, kode_trip: trip.kode_trip, isFinal: Array.isArray(saved) && saved.length > 0 }, rooms };
  } catch (e) {
    return { error: e?.message || 'gagal' };
  }
}
