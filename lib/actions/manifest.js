...
// Detail 1 crew (TL/Tim) utk baris roomlist. m = member tersimpan (opsional, utk gender override).
function crewToRoomMember(c, m = {}) {
  ...
  const _rmk = `${c.role || 'Tour Leader'}${c.notes ? ' — ' + c.notes : ''}`;
  return {
    name: c.name || first || '', first_name: first, surname: last, title,
    gender: (m && m.gender) || g,
...
async function _guardTripAccess(trip) {
  try {
...
export async function getManifestRows(tripId) {
  ...
  try {
    const { data: trip } = await supabase
      .from('trips').select('id, name, kode_trip, departure, return_date, arrival, tl_id, tl_email, tl_phone').eq('id', tripId).maybeSingle();
    if (!trip) return { error: 'Trip tidak ditemukan' };
    if (!(await _guardTripAccess(trip))) return { error: 'Tidak berizin' };
...
function buildRmDetail(c = {}, pax = {}) {
  ...
  return {
    ...
    // Field manifest (dipakai roomlist PDF format-manifest). Additive — tak memengaruhi kolom lama.
    issue_date: fmtDate(c.passport_issued_date || c.issue_date),
    issuing_office: c.passport_issued_at || c.issuing_office || '',
    expiry_date: fmtDate(c.passport_expiry || c.expiry_date),
    phone: c.phone || c.whatsapp || '',
...
export async function getRoomlistRows(tripId) {
  ...
  try {
    const { data: trip } = await supabase
      .from('trips').select('id, name, kode_trip, departure, return_date, arrival, final_roomlist, tl_id, tl_email, tl_phone').eq('id', tripId).maybeSingle();
    if (!trip) return { error: 'Trip tidak ditemukan' };
    if (!(await _guardTripAccess(trip))) return { error: 'Tidak berizin' };
    ...
    // Crew (TL/Tim) — ambil dulu supaya bisa: (a) resolve passport kalau crew ditaruh
    // manual di kamar (final_roomlist), (b) tambah section "TL & Tim" utk yg belum ditaruh.
    const crew = await fetchTripCrewResolved(tripId);
    const crewByName = {}; for (const c of crew) { const nm = String(c.name || '').toLowerCase(); if (nm) crewByName[nm] = c; }
    ...
    if (Array.isArray(saved) && saved.length > 0) {
      rooms = saved.map((r, i) => ({
        ...
        is_family: r.is_family, is_crew: r.is_crew || false, note: r.note || '',
        members: (r.members || []).map((m) => {
          const nm = String(m.name || '').toLowerCase();
          // Kalau member ini crew (ditaruh manual di kamar) -> pakai data passport crew.
          if (crewByName[nm]) return crewToRoomMember(crewByName[nm], m);
    ...
    // Tambah section "TL & TIM" HANYA utk crew yang BELUM ditaruh di kamar mana pun
    // (dedup by name) -> tidak dobel dgn crew yg sudah dipindah manual ke kamar peserta.
    const _present = new Set();
    for (const r of rooms) for (const m of (r.members || [])) _present.add(String(m.name || '').toLowerCase());
    const _remainingCrew = crew.filter((c) => c.name && !_present.has(String(c.name).toLowerCase()));
    if (_remainingCrew.length) {
      rooms = [...rooms, {
        room_no: rooms.length + 1, room_type: 'TL & TIM', label: 'TL & Tim',
        is_family: false, is_crew: true, note: '',
        members: _remainingCrew.map((c) => crewToRoomMember(c)),
      }];
    }
