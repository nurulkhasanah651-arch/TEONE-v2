...
// Bayar SEMUA sekaligus dari halaman invoice: pelunasan pokok + visa + asuransi (all-in)
export async function startInvoiceAllInPayment(invoiceToken, method) {
  const code = currentBrandCode();
  if (!midtransConfigured(code)) return { error: 'Pembayaran online belum aktif. Silakan transfer manual.' };
  const db = svc();
  if (!db) return { error: 'Server belum siap' };
  ...
  const out = await invoiceAllInOutstanding(inv.id);
  const amount = Number(out.total) || 0;
  if (amount <= 0) return { error: 'Tidak ada sisa tagihan untuk dilunasi.' };

  const headPid = inv.passenger_id || (out.members[0] && out.members[0].pid);
  let cust = null;
  if (headPid) { const { data: pax } = await db.from('trip_passengers').select('customer_id').eq('id', headPid).maybeSingle(); if (pax?.customer_id) { const r = await db.from('customers').select('name, phone, whatsapp, email').eq('id', pax.customer_id).maybeSingle(); cust = r.data; } }

  const fee = paymentFee(method, amount);
  const base = customerSiteUrlFor(code);
  const orderId = `INVALLIN-${code === 'khasanah' ? 'KH' : 'TE'}-${inv.id}-${Date.now().toString(36)}`;
  const r = await createSnapTransaction(code, {
    orderId, grossAmount: amount + fee, fee, enabledPayments: payMethod(method)?.enabled,
    customer: { name: cust?.name, email: cust?.email, phone: cust?.whatsapp || cust?.phone },
    itemName: `Pelunasan All-in (pokok + visa + asuransi)`,
    finishUrl: `${base}/invoice/${invoiceToken}`,
  });
  if (r?.error) return { error: r.error };
  try { await db.from('invoices').update({ midtrans_order_id: orderId }).eq('id', inv.id); } catch {}
  return { ok: true, redirect_url: r.redirect_url, amount: amount + fee };
}
