...
export async function startPayment(bookingId, method) {
  ...
  const finishUrl = `${base}/order/${b.id}`;  // setelah bayar → halaman order (publik, tampil 'Pembayaran Berhasil', tak butuh login)
  ...
  // b.amount sudah termasuk admin DP web (ADMIN_FEE). Un-bake → hitung fee sesuai metode:
  // non-CC: tetap Rp13.000 (DP web). CC: 3% dari pokok DP.
  const dpBaseAmt = Math.max((Number(b.amount) || 0) - ADMIN_FEE, 0);
...
export async function startInvoicePayment(invoiceToken, method) {
  ...
  const fee = paymentFee(method, amount); // non-CC Rp6.000, CC 3%
  const base = siteUrlFor(code);
  const orderId = `INVID-${inv.id}-${Date.now().toString(36)}`;
  const r = await createSnapTransaction(code, {
    orderId, grossAmount: amount + fee, fee, enabledPayments: payMethod(method)?.enabled,
