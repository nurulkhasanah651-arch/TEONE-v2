...
export async function startPayment(bookingId, method) {
  ...
  const fee = 0; // DP awal: biaya admin Rp13.000 sudah termasuk di b.amount (checkout)
  const r = await createSnapTransaction(code, {
    orderId,
    grossAmount: (Number(b.amount) || 0) + fee,
    fee,
...
export async function startMilestonePayment(bookingId, milestoneType, method) {
  ...
  const fee = ADMIN_FEE_ONLINE; // biaya admin online flat Rp6.000
  const base = siteUrlFor(code);
