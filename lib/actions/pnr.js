...
// R195i: PNR actions — FIX field name mismatch
// Form pakai: seats, ticket_price, deposit_total, payoff_amount, payoff_date, payoff_due_date, route, vendor_notes
// Action baca: pax, price_per_pax, deposit_total, payoff_amount, payoff_deadline, payoff_due_date, routes, notes
// Fix: read BOTH names (form name OR DB name) sebagai fallback
...
// R195i: read field with fallback name (form name OR action name)
...
// R195i: build PNR payload — handle multiple field name variants
function buildPnrPayload(formData) {
  // Form sends: seats, ticket_price, deposit_total, payoff_amount, payoff_date, payoff_due_date, route, vendor_notes
  // DB columns: pax, price_per_pax, deposit_total, payoff_amount, payoff_deadline, payoff_due_date, routes, notes

  const routeStr = getField(formData, 'routes', 'route') || '';

  return {
    pnr: (getField(formData, 'pnr') || '').trim() || null,
    airline: getField(formData, 'airline'),
    vendor: getField(formData, 'vendor'),
    pax: toInt(getField(formData, 'pax', 'seats')),                        // ← form: 'seats'
    price_per_pax: toInt(getField(formData, 'price_per_pax', 'ticket_price')), // ← form: 'ticket_price'
    deposit_total: toInt(getField(formData, 'deposit_total')),
    payoff_amount: toInt(getField(formData, 'payoff_amount')),
    routes: String(routeStr).split('\n').map((r) => r.trim()).filter(Boolean),
    departure_date: getField(formData, 'departure_date'),
    payoff_deadline: getField(formData, 'payoff_deadline', 'payoff_date'),
    payoff_due_date: getField(formData, 'payoff_due_date'),
    notes: getField(formData, 'notes', 'vendor_notes'),
  };
}
...
async function syncPnrToHPP(supabase, pnrId, tripId, userEmail) {
  ...
  // DELETE-FIRST: hapus semua matching dulu (prevent double)
  try {
    await withTimeout(
...
// ============================================================
// CREATE PNR — R195i: pakai buildPnrPayload (handle form field names)
// ============================================================
export async function createPnr(formData) {
  ...
  const payload = buildPnrPayload(formData);
  ...
  if (createdData?.trip_id) {
    await syncPnrToHPP(supabase, createdData.id, createdData.trip_id, user.email);
  }
...
export async function updatePnr(id, formData) {
  ...
  const updates = buildPnrPayload(formData);
