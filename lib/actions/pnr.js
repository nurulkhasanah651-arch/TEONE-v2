...
function parsePnrFields(formData) {
  ...
  // Tiket FIT: tipe + harga total + sambung ke trip
  const ticketType = (formData.get('ticket_type') || 'group').trim();
  fields.ticket_type = ticketType === 'fit' ? 'fit' : 'group';
  if (fields.ticket_type === 'fit') {
    ...
    // FIT dibeli & dibayar penuh; pastikan ada kode (pakai placeholder kalau kosong)
...
async function syncPnrToHPP(supabase, pnrId, tripId) {
  ...
  const payload = {
    ...
    component: isFit ? 'Tiket FIT' : 'Tiket Maskapai',
