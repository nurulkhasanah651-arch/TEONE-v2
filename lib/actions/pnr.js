...
export async function syncPnrToHPP(supabase, pnrId, tripId) {
  ...
  // Trip yg sudah dilink tiket PNR -> Status Operasional "Tiket" = confirmed.
  // Jangan turunkan kalau sudah 'issued' / 'confirmed'.
  try {
    const { data: _tr } = await supabase.from('trips').select('ticket_status').eq('id', tripId).maybeSingle();
    const _cur = String(_tr?.ticket_status || '').toLowerCase();
    if (_cur !== 'confirmed' && _cur !== 'issued') {
      await supabase.from('trips').update({ ticket_status: 'confirmed' }).eq('id', tripId);
    }
  } catch { /* best-effort */ }
