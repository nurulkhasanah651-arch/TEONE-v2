...
export async function buildCsRecap() {
 try {
  ...
  const totalClosing = sum('total_terjual_hari_ini');
  const newAds = sum('closing_ads');
  const newIg = sum('from_instagram');
  const alumni = sum('closing_alumni');
  const mitra = sum('closing_mitra');
  const newCust = sum('closing_new_customer');
