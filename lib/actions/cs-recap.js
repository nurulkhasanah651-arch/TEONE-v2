...
export async function buildCsRecap() {
 try {
  ...
  // SEMUA BRAND (Khasanah & TEONE): trip yang sudah CLOSE SELLING (di-close manual /
  // quota penuh) tidak muncul lagi di rekap harian CS — biar daftar bersih, cuma trip
  // yang masih dijual. Total closing tetap dihitung dari semua peserta.
  const activeTrips = (trips || []).filter((t) => {
    if (t.status === 'completed' || t.status === 'cancelled' || !t.departure) return false;
    const liveSold = liveCnt[t.id] || 0;
    if (effectiveSellingStatus({ ...t, _soldReal: liveSold }) === 'closed selling') return false;
