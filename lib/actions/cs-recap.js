...
export async function buildCsRecap() {
 try {
  ...
  const [{ data: trips }, { data: updates }, { data: leads }, paxRowsAll] = await Promise.all([
    ...
    fetchAll(() => supabase.from('trip_passengers').select('trip_id, transfer_status, refund_status, joined_at, lead_source')),
  ]);
  ...
  // CLOSING per orang = peserta AKTUAL yg masuk dalam window bisnis hari ini (per trip).
  // Family 5 orang = 5 closing. Web + CS + Tambah Peserta semua otomatis terhitung (tak perlu bump manual).
  const _win = businessWindow(today);
  const _ws = new Date(_win.startISO), _we = new Date(_win.endISO);
  const closeTodayByTrip = {};
  let paxCloseTotal = 0;
  for (const p of (paxRowsAll || [])) {
    if (p.transfer_status === 'transferred') continue;
    if (p.refund_status === 'refunded' || p.refund_status === 'partial_refund') continue;
    if (p.lead_source === 'master') continue; // ditambah via Master Trip -> bukan closing
    const j = p.joined_at ? new Date(p.joined_at) : null;
    if (j && j >= _ws && j < _we) {
      closeTodayByTrip[p.trip_id] = (closeTodayByTrip[p.trip_id] || 0) + 1;
      paxCloseTotal++;
    }
  }

  // KHASANAH: trip yang sudah CLOSE SELLING (di-close manual / penuh) tidak muncul lagi
  // di rekap harian CS (biar daftar bersih, cuma trip yang masih dijual).
  let _brand = ''; try { _brand = currentBrandCode(); } catch {}
  const _isKh = String(_brand || '').toLowerCase() === 'khasanah';
  const activeTrips = (trips || []).filter((t) => {
    if (t.status === 'completed' || t.status === 'cancelled' || !t.departure) return false;
    const liveSold = liveCnt[t.id] || 0;
    if (_isKh && effectiveSellingStatus({ ...t, _soldReal: liveSold }) === 'closed selling') return false;
  ...
  // Total closing = jumlah PESERTA aktual masuk hari ini (per orang). Rincian channel di bawah
  // adalah atribusi input CS (bisa berbeda dgn total peserta bila channel tak diisi lengkap).
  const totalClosing = paxCloseTotal;
  ...
  for (const mk of sortedMonths) {
    ...
    for (const t of byMonth[mk]) {
      ...
      txt += `\n`; // spasi antar trip biar enak dibaca
    }
  }
  ...
  txt += `\nMitra 🤝 : ${dash(mitra)}`;
  txt += `\nWeb 🌐 : ${dash(web)}`;
  const _lainnya = totalClosing - (num(ig) + num(wa) + num(offline) + num(alumni) + num(mitra) + num(web) + num(newAds));
  if (_lainnya > 0) txt += `\nLainnya 🧾 : ${_lainnya}`;
  txt += `\n*Total closing : ${totalClosing}*`;

  return { ok: true, text: txt, totalClosing, leadsTotal: totalChat, leadsOrganic: organic, leadsAds: adsLeads, tripAktif: (updates || []).length };
 } catch (e) {
