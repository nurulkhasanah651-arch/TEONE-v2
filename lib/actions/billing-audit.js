...
export async function getBillingAudit(opts = {}) {
  ...
  const { data: pax } = await db.from('trip_passengers')
    .select('id, trip_id, customer_id, room_type, age_type, price_paid, discount_amount, include_visa, visa_ready, include_asuransi, include_evisa, visa_type')
    .in('trip_id', tripIds);
  ...
  for (const t of aktif) {
    ...
    const baris = anggota.map((p) => {
      ...
      // E-Visa = include terpisah (independen dari visa biasa). Canada: bisa bareng visa biasa.
      const evisaOpt = (!isKh && (p.include_evisa || bayarTypes.has('E-Visa'))) ? n(bd.visa_evisa) : 0;
      ...
      const seharusnya = Math.max(hargaMaster + visaOpt + asrOpt + evisaOpt + addon - diskon, 0);
      ...
      const dipakaiSistem = Math.max(pokokGross + visaOpt + asrOpt + evisaOpt + addon - diskon, 0);
