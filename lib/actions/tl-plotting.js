...
export async function finalPlotTl(brand, tripId) {
  ...
  let tl = null;
  try { const { data } = await db.from('tour_leaders').select('id, phone, email').ilike('name', nm).maybeSingle(); tl = data || null; } catch {}
  const patch = {
    tl_name: nm,
    tl_id: tl?.id || null,
...
export async function resendTlAssignmentWA(brand, tripId) {
  ...
  return runWithBrand(code, async () => {
    ...
    // Kalau tak ada tl_id (nama manual), baru pakai nomor tersimpan.
    if (trip.tl_id) fd.set('tlId', String(trip.tl_id));
    else if (trip.tl_phone) fd.set('tlPhone', String(trip.tl_phone));
