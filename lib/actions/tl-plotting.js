...
const COLS = 'id, kode_trip, name, destination, departure, return_date, quota, sold, seat_left, status, tl_name, tl_id, tl_plan, tl_assignment_status, tl_gaji70, tl_gaji30, tl_konten_approved';
    ...
    try {
      ...
      return (data || [])
        ...
        .map((t) => ({
          ...
          gaji70: Boolean(t.tl_gaji70),
          gaji30: Boolean(t.tl_gaji30),
          konten_approved: Boolean(t.tl_konten_approved),
          status: t.status || '',
        }));
    } catch { return []; }
  }
...
// Toggle penanda gaji TL: '70' (gaji 70% H-1), '30' (gaji 30%), 'konten' (konten sudah approval).
// ATURAN: gaji 30% HANYA boleh dicentang kalau konten SUDAH approval. Kalau approval dicabut,
// centang 30% ikut direset (tanpa approval, 30% tidak ditransfer).
export async function setTlPayFlag(brand, tripId, field, value) {
  const g = await guard(); if (g.error) return g;
  const db = brandClient(brand); if (!db) return { error: 'DB tidak tersedia' };
  const val = Boolean(value);

  const { data: cur } = await db.from('trips')
    .select('tl_gaji70, tl_gaji30, tl_konten_approved').eq('id', tripId).maybeSingle();
  const kontenOk = Boolean(cur?.tl_konten_approved);

  const patch = {};
  if (field === '70') patch.tl_gaji70 = val;
  else if (field === 'konten') {
    patch.tl_konten_approved = val;
    if (!val) patch.tl_gaji30 = false;   // approval dicabut → 30% otomatis batal
  } else if (field === '30') {
    if (val && !kontenOk) return { error: 'Konten belum di-approve — 30% belum bisa ditransfer.' };
    patch.tl_gaji30 = val;
  } else return { error: 'field tidak dikenal' };
