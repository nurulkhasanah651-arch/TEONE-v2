...
import { getPicFonnteTokenForTrip, isPicWaManualForTrip } from '@/lib/auth/pic-scope';
...
export async function approveInvoicePayment(paymentId) {
  ...
  if (passengerId) {
    ...
    if (await isPicWaManualForTrip(supabase, invoice.trip_id)) {
      // Coba kirim otomatis via WABA resmi dulu (PIC yang nomornya sudah WABA, mis. Anis).
      const _waba = await tryWabaApproval(passengerId);
      if (_waba?.ok) {
        revalidateAll(invoice.trip_id);
        return {
          ok: true, checklist_synced: syncResult.ok, is_family: syncResult.is_family,
          member_count: syncResult.member_count, invoice_status: newInvoiceStatus,
          wa_sent: true, via: 'waba',
        };
      }
      let tpl = null;
      try { tpl = await sendPaymentReceivedWA(passengerId, true); } catch {}
      revalidateAll(invoice.trip_id);
      return {
        ok: true, checklist_synced: syncResult.ok, is_family: syncResult.is_family,
        member_count: syncResult.member_count, invoice_status: newInvoiceStatus,
        wa_manual: true, wa_sent: false,
        wa_message: tpl?.message || null, wa_phone: tpl?.phone || null, customer_name: tpl?.customerName || null,
      };
    }
