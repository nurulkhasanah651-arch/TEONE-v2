...
export async function applyTemplateToTrip(targetTripId, sourceTripId) {
  ...
  try {
    const { data: t } = await db.from('trips')
      .select('cover_image_url, gallery_images, itinerary, highlights, description, included, excluded, syarat_visa, syarat_ketentuan, price_breakdown, dp_amount, web_payment_schedule')
