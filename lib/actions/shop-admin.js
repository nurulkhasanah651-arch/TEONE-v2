...
export async function updateTripPublicContent(tripId, formData) {
  ...
  { const _g = await assertStaff(user, '/trips'); if (_g.error) return { error: _g.error }; }
  ...
  const update = {
    ...
    web_poster_url: (formData.get('web_poster_url') || '').toString().trim() || null,
    ...
    public_title: (formData.get('public_title') || '').toString().trim() || null,
    ...
    web_reasons: (formData.get('web_reasons') || '').toString().trim() || null,
  };
  ...
  // Hotel (Khasanah) — foto + nama + fasilitas singkat (web_hotels). Hanya di-set kalau
  // panel mengirim hotels_json (khusus Khasanah), jadi tak menyentuh trip brand lain.
  const hotelsRaw = (formData.get('hotels_json') || '').toString();
  if (hotelsRaw) {
    try {
      const arr = JSON.parse(hotelsRaw);
      if (Array.isArray(arr)) {
        update.web_hotels = arr
          .map((h) => {
            const imgs = Array.isArray(h?.images) ? h.images.map((u) => String(u || '').trim()).filter(Boolean)
              : (h?.image ? [String(h.image).trim()] : []);
            return { name: String(h?.name || '').trim(), facilities: String(h?.facilities || '').trim(), images: imgs };
          })
          .filter((h) => h.name || h.facilities || h.images.length);
      }
    } catch { /* abaikan kalau bukan JSON valid */ }
  }

  // Skema cicilan KHUSUS tampilan web publish trip (web_payment_schedule).
  // TIDAK menyentuh payment_template / payment checklist finance.
  const schedRaw = (formData.get('payment_schedule_json') || '').toString();
  if (schedRaw) {
    try {
      const arr = JSON.parse(schedRaw);
      if (Array.isArray(arr)) {
        update.web_payment_schedule = arr
          .map((r) => ({
            type: String(r?.type || '').trim(),
            amount: Number(r?.amount) || 0,
            due: (r?.due || '').toString().trim(),
            // Keterangan bebas, mis. "dibayar setelah trip full seat" — dipakai kalau tanggal tak pasti.
            note: (r?.note || '').toString().trim().slice(0, 200),
          }))
          .filter((r) => r.type && (r.amount > 0 || r.due || r.note || r.type === 'Pelunasan'));
      }
    } catch { /* abaikan kalau bukan JSON valid */ }
  }

  // Catatan: price_breakdown TIDAK disentuh dari panel storefront (harga = data Master Trip / via Pakai Template).
...
export async function applyTemplateToTrip(targetTripId, sourceTripId) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const _g = await assertStaff(user, '/trips'); if (_g.error) return { error: _g.error }; }
  if (!targetTripId || !sourceTripId) return { error: 'sumber/target kosong' };
  const db = svc() || supabase;
  try {
    const { data: t } = await db.from('trips')
      .select('cover_image_url, gallery_images, itinerary, highlights, description, included, excluded, syarat_visa, syarat_ketentuan, price_breakdown, dp_amount, web_payment_schedule, web_hotels')
      .eq('id', sourceTripId).maybeSingle();
    if (!t) return { error: 'Trip sumber tidak ditemukan' };
    // Hanya salin field yang SUMBERNYA ADA isinya — supaya template tak menimpa data target jadi kosong.
    const upd = {};
    if (t.cover_image_url) upd.cover_image_url = t.cover_image_url;
    if (Array.isArray(t.gallery_images) && t.gallery_images.length) upd.gallery_images = t.gallery_images;
    if (Array.isArray(t.itinerary) && t.itinerary.length) upd.itinerary = t.itinerary;
    if (t.highlights) upd.highlights = t.highlights;
    if (t.description) upd.description = t.description;
    if (t.included) upd.included = t.included;
    if (t.excluded) upd.excluded = t.excluded;
    if (t.syarat_visa) upd.syarat_visa = t.syarat_visa;
    if (t.syarat_ketentuan) upd.syarat_ketentuan = t.syarat_ketentuan;
    if (t.price_breakdown && typeof t.price_breakdown === 'object' && Object.keys(t.price_breakdown).length > 0) upd.price_breakdown = t.price_breakdown;
    if (t.dp_amount) upd.dp_amount = t.dp_amount;
    if (Array.isArray(t.web_payment_schedule) && t.web_payment_schedule.length) upd.web_payment_schedule = t.web_payment_schedule;
    if (Array.isArray(t.web_hotels) && t.web_hotels.length) upd.web_hotels = t.web_hotels;
    if (Object.keys(upd).length === 0) return { error: 'Trip sumber belum punya konten/harga untuk disalin.' };
    const { error } = await db.from('trips').update(upd).eq('id', targetTripId);
    if (error) return { error: 'Gagal terapkan: ' + error.message };
    revalidatePath(`/trips/${targetTripId}/edit`);
    return { ok: true };
  } catch (e) { return { error: e?.message || 'gagal' }; }
}
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        price_breakdown: (t.price_breakdown && typeof t.price_breakdown === 'object') ? t.price_breakdown : {},
        dp_amount: t.dp_amount || '',
        web_payment_schedule: Array.isArray(t.web_payment_schedule) ? t.web_payment_schedule : [],
        web_hotels: Array.isArray(t.web_hotels) ? t.web_hotels : [],
      },
    };
  } catch (e) {
    return { error: e?.message || 'gagal' };
