...
export async function updateTripPublicContent(tripId, formData) {
  ...
  // Skema cicilan diisi CS: [{type,label,amount,due}] -> payment_template (nominal) + payment_deadlines (tanggal)
    ...
    try {
      ...
      if (Array.isArray(arr)) {
        ...
        for (const r of arr) {
          ...
          if (!type || type === 'DP') { // DP nominal tetap dari field dp_amount; simpan tanggalnya kalau ada
            const dueDp = (r?.due || '').toString().trim();
            if (type === 'DP' && dueDp) dl.DP = dueDp;
            continue;
          }
          const amt = Number(r?.amount) || 0;
          const due = (r?.due || '').toString().trim();
          if (amt > 0) tpl[type] = amt;
          if (due) dl[type] = due;
        }
        update.payment_template = tpl;
        update.payment_deadlines = dl;
      }
    } catch { /* abaikan kalau bukan JSON valid */ }
  }
