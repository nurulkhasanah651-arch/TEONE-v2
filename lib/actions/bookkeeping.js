...
export async function buildTripInvoices(tripId) {
  ...
  let company = {};
  try { const { data } = await db.from('company_settings').select('*').limit(1).maybeSingle(); company = data || {}; } catch {}
  if (!company.company_name) { try { const { data } = await db.from('brands').select('name, logo_url').limit(1).maybeSingle(); if (data) { company.company_name = data.name; company.company_logo_url = company.company_logo_url || data.logo_url; } } catch {} }
  ...
  return {
    ok: true,
    company: {
      name: company.company_name || (isKh ? 'Khasanah Travel' : 'Traveling Eropa'),
