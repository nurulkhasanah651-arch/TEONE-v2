// R215d + R215g + R215k: Hotel HPP server actions
// R215k FIX: payment_status pakai 'belum lunas' (bukan 'belum' yg violate CHECK constraint)
//            + fallback chain kalau masih fail
...
// R215k — list status values yg dicoba secara berurutan (fallback chain)
const PAYMENT_STATUS_CANDIDATES = [
  'belum lunas',  // most likely valid
  'pending',
  'belum bayar',
  'unpaid',
  'open',
  null,            // last resort: tanpa payment_status
];
...
export async function saveHotelHPP(data) {
  ...
  // R215k — build payload tanpa payment_status dulu (akan dicoba per-iteration)
  ...
  // R215k — Fallback chain: try each candidate payment_status, then without extras, then without status
  let inserted = null;
  let warningMsg = null;
  let lastErrorMsg = null;

  for (const status of PAYMENT_STATUS_CANDIDATES) {
    const corePayload = { ...baseCorePayload };
    if (status !== null) corePayload.payment_status = status;

    // Try WITH extras (R215d fields)
    ...
    lastErrorMsg = insErr1?.message || 'unknown';

    // If extras failed, try WITHOUT extras
    ...
    if (!insErr2 && ins2) {
      inserted = ins2;
      warningMsg = 'Kolom extra (is_hotel, room_type, dll) belum ada di DB — item ke-save dgn data dasar.';
      break;
    }

    lastErrorMsg = insErr2?.message || 'unknown';

    // Kalau error bukan CHECK constraint payment_status, gak perlu retry pakai status lain
    if (!/payment_status/i.test(lastErrorMsg) && !/check.*constraint/i.test(lastErrorMsg)) {
      // Error bukan dari payment_status — gak akan beda walau ganti status
      break;
    }
    // Else: retry dgn payment_status candidate berikutnya
  }

  if (!inserted) {
    return {
      error: `Insert failed: ${lastErrorMsg}. Sudah coba payment_status candidates: ${PAYMENT_STATUS_CANDIDATES.join(', ')}`
    };
  }
