...
async function getOrCreateMyEmployee(db, user) {
  ...
  // PENTING KEAMANAN: role TIDAK BOLEH diambil dari user_metadata (bisa diubah sendiri
  // dari browser → eskalasi ke owner). Auto-create selalu role non-istimewa 'other';
  // role staf sebenarnya hanya di-set admin lewat master employees.
  const name = user.user_metadata?.name || user.user_metadata?.full_name || (user.email || '').split('@')[0];
  const ins = await db.from('employees').insert({
    user_id: user.id, email: (user.email || '').toLowerCase(), full_name: name,
    role: 'other', status: 'active', employment_type: 'full_time',
  }).select('*').single();
