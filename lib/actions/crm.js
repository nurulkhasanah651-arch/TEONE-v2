...
// Kirim WA ke satu customer via Fonnte (ultah / reminder paspor / dll)
export async function sendCustomerWA(phone, message) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  if (!phone) return { error: 'Nomor HP kosong' };
  if (!message || !message.trim()) return { error: 'Pesan kosong' };
  try {
    const res = await sendFonnte(normalizePhone(phone), message, { context: 'finance' });
    if (res?.error) return { error: res.error };
    return { ok: true };
  } catch (e) {
    return { error: e?.message || 'gagal kirim' };
  }
}
