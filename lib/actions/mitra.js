...
export async function getMitraStats() {
  ...
  try {
    ...
    // Sumber atribusi: CS Daily (closing_mitra + mitra_id) — sesuai input harian CS
    const { data: cs } = await supabase
      .from('cs_daily_updates')
      .select('trip_id, mitra_id, closing_mitra, tanggal')
      .not('mitra_id', 'is', null);
    const tripIds = [...new Set((cs || []).map((p) => p.trip_id).filter(Boolean))];
    let tripMap = {};
    if (tripIds.length) {
    ...
    const stats = (mitras || []).map((m) => {
      const myRows = (cs || []).filter((r) => r.mitra_id === m.id);
      const trips = {};
      let feeEarned = 0, sold = 0;
      for (const r of myRows) {
        const n = Number(r.closing_mitra) || 0;
        if (n <= 0) continue;
        sold += n;
        const t = tripMap[r.trip_id];
        const cat = t?.fee_category || 'Lainnya';
        feeEarned += (feeByCat[cat] || 0) * n;
        const key = r.trip_id;
