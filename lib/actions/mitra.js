...
import { tripPrice } from '@/lib/shop/data';
...
// Hitung jumlah peserta AKTIF (bukan transfer/refund) per trip untuk daftar id tsb.
async function activePaxMap(client, ids) {
  const map = {};
  const list = Array.isArray(ids) ? ids : [];
  for (let i = 0; i < list.length; i += 100) {
    const batch = list.slice(i, i + 100);
    // Paginasi per-batch via .range() supaya tidak kena cap 1000 baris PostgREST.
    // Tanpa ini, saat total peserta > 1000 sold ke-undercount -> sisa seat ke-overcount
    // (jadi beda dengan Master Trip yang pakai fetchAll penuh).
    for (let from = 0; ; from += 1000) {
      const { data: pax, error } = await client.from('trip_passengers')
        .select('trip_id, transfer_status, refund_status')
        .in('trip_id', batch).range(from, from + 999);
      if (error) break;
      for (const p of (pax || [])) {
        if (p.transfer_status === 'transferred' || p.refund_status === 'refunded' || p.refund_status === 'partial_refund') continue;
        map[p.trip_id] = (map[p.trip_id] || 0) + 1;
      }
...
// Trip dianggap "open selling" OTOMATIS bila status efektifnya open selling — yaitu
// sudah ada peserta aktif (sold>0) ATAU status tersimpan 'open selling', dan BELUM
// penuh/closed/ongoing/completed/cancelled. Konsisten dengan Master Trip.
function isEffectivelyOpen(trip, activeSold) {
  return effectiveSellingStatus({ ...trip, _soldReal: activeSold }) === 'open selling';
}
...
async function openSellingForBrand(code) {
  ...
  const { data: allTrips } = await c.from('trips')
    .select('id, name, kode_trip, slug, departure, return_date, harga_jual, price, public_price, price_breakdown, quota, status, trip_docs_link, visa_pdf_syarat_url')
    .order('departure', { ascending: true });
  // Hitung peserta aktif utk SEMUA trip, lalu pilih yang status efektifnya open selling
  // (termasuk trip yg auto-open karena sudah ada peserta walau status tersimpan masih prepare).
  const soldMap = await activePaxMap(c, (allTrips || []).map((t) => t.id));
  const open = (allTrips || []).filter((t) => isEffectivelyOpen(t, soldMap[t.id] || 0));
  return open.map((t) => ({
    id: t.id, brand: code, kode_trip: t.kode_trip || '', name: t.name || '', departure: t.departure || null,
    // Harga "mulai dari": Khasanah simpan harga di price_breakdown (tipe kamar), bukan di
    // kolom flat -> pakai tripPrice() spy sama dgn card web (cegah tampil Rp 0). Fallback ke
    // harga_jual/price utk trip TEONE yg masih pakai kolom lama.
    price: tripPrice(t) || Number(t.harga_jual) || Number(t.price) || 0,
...
export async function getOpenSellingTrips() {
  ...
  const { data: allTrips } = await svc.from('trips')
    .select('id, name, kode_trip, slug, departure, return_date, harga_jual, price, public_price, quota, status, trip_docs_link, visa_pdf_syarat_url')
    .order('departure', { ascending: true });
  const soldMap = await activePaxMap(svc, (allTrips || []).map((t) => t.id));
  const open = (allTrips || []).filter((t) => isEffectivelyOpen(t, soldMap[t.id] || 0));
...
export async function getOpenTripsForMitra() {
  ...
  try {
    const { data: trips } = await supabase
      .from('trips')
      .select('id, name, kode_trip, departure, return_date, harga_jual, price, quota, seat_left, status, visa_pdf_syarat_url, trip_docs_link, fee_category')
      .order('departure', { ascending: true });
    // hitung peserta aktif utk SEMUA trip, lalu pilih yg status efektifnya open selling
    const soldMap = await activePaxMap(supabase, (trips || []).map((t) => t.id));
    const open = (trips || []).filter((t) => isEffectivelyOpen(t, soldMap[t.id] || 0));
