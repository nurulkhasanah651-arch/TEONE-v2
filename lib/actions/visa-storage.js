...
// Ambil PATH storage dari nilai tersimpan (bisa URL publik lama ATAU path baru)
export function visaResultPath(stored) {
  if (!stored) return null;
  const str = String(stored);
  const marker = `/${BUCKET}/`;
  const idx = str.indexOf(marker);
  if (idx >= 0) return str.slice(idx + marker.length).split('?')[0];
  return str.replace(/^\/+/, '');   // sudah berupa path
}

// Buat signed URL (berlaku sementara) dari nilai tersimpan — bucket private
export async function signedVisaResultUrl(stored, expirySec = 7 * 24 * 3600) {
  const path = visaResultPath(stored);
  if (!path) return null;
  const supabase = getServiceClient();
  if (!supabase) return null;
  try {
    const { data } = await supabase.storage.from(BUCKET).createSignedUrl(path, expirySec);
...
export async function uploadVisaResultFile(passengerId, formData) {
  ...
  try {
    ...
    // Bucket PRIVATE: simpan PATH (bukan URL publik). Beri signed URL utk preview admin (1 jam).
    const { data: signed } = await supabase.storage.from(BUCKET).createSignedUrl(filePath, 60 * 60);

    revalidatePath(`/visa/${pax.trip_id}`);
    return { ok: true, file_url: signed?.signedUrl || null, file_path: filePath };
  } catch (e) {
