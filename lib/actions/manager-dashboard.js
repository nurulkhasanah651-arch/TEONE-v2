...
const FOLLOWUP_DAYS = 3; // follow-up menyembunyikan item selama 3 hari; setelah itu muncul lagi kalau belum dikerjakan tim
...
export async function getManagerDashboard() {
  ...
  // ── Follow-up aktif (≤3 hari) → sembunyikan item terkait ──
  const fset = new Set();
  try {
    const cutoff = new Date(Date.now() - FOLLOWUP_DAYS * 86400 * 1000).toISOString();
    const { data: fups } = await db.from('manager_followups').select('section, trip_id, created_at').gte('created_at', cutoff);
...
const VALID_SECTIONS = new Set([
  ...
  'visa.notProcessed', 'visa.paidUnscheduled', 'visa.fullH5',
  'operation.newRelease', 'operation.fullNoOffering', 'operation.estimateNotUpdated',
  'selling.slowSelling', 'selling.almostFull',
]);

// Tandai "sudah follow up" → item hilang 3 hari; muncul lagi kalau belum dikerjakan tim.
