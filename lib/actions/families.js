...
// Round 100d: Family Group actions — pakai SERVICE_ROLE untuk bypass RLS
// Karena error "violates RLS" tetep muncul walau RLS sudah disable + grant
// (kemungkinan cache supabase client / quirk Supabase), kita pakai
// service role key langsung untuk INSERT/UPDATE family table.
// Service role bypass SEMUA RLS check.
...
export async function createFamilyGroup({ trip_id, name, head_passenger_id, member_passenger_ids = [] }) {
  // Auth via session (verify user logged in)
  ...
  // SERVICE ROLE for INSERTs (bypass RLS)
