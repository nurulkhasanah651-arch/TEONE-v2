...
import { roomTypeToKey, khMandatoryForKey } from '@/lib/utils/price-breakdown';
...
export async function createBooking(formData) {
  ...
  const includeVisa = (formData.get('include_visa') || '').toString() === '1';
  const visaReady = (formData.get('visa_ready') || '').toString() === '1';
  const visaType = (formData.get('visa_type') || '').toString().trim() || null;
  const includeAsuransi = (formData.get('include_asuransi') || '').toString() === '1';
  const includeEvisa = (formData.get('include_evisa') || '').toString() === '1';
  ...
  // KHASANAH: harga pokok per tipe kamar SUDAH termasuk biaya wajib (visa/asuransi/handling dll),
  // sama seperti invoice. TEONE tetap pakai pokokPriceForKey (main addon saja).
  const _isKhCheckout = currentBrandCode() === 'khasanah';
  const priceForKey = (k) => pokokPriceForKey(bd, k) + (_isKhCheckout ? khMandatoryForKey(bd, k) : 0);
  ...
  if (paxListRaw.length) {
    ...
    const unit = (rk ? priceForKey(rk) : 0) || Number(trip.public_price || trip.price || 0);
  ...
  // Biaya admin TIDAK di-bake ke amount → hanya ditambah saat bayar ONLINE (Midtrans).
  // Transfer manual = tanpa biaya admin.
  const amount = base;
  ...
  try {
    if (authCustomerId) {
      // Akun login: JANGAN timpa nama akun dgn nama booking (bisa beda-beda tiap checkout).
      // Isi nama hanya kalau profil akun masih kosong. Email/HP tetap diperbarui.
      const { data: cur } = await db.from('customers').select('name').eq('id', authCustomerId).maybeSingle();
      const patch = { email: email || null, whatsapp: np || null };
      if (!cur?.name || !String(cur.name).trim()) patch.name = name;
      await db.from('customers').update(patch).eq('id', authCustomerId);
    } else {
  ...
  const { data: booking, error } = await db.from('bookings').insert({
    ...
    notes: JSON.stringify({ admin_fee: 0, subtotal: subtotalFull, payment_base: base, composition: compStore, pax_list: paxList, include_visa: includeVisa, visa_ready: visaReady, include_asuransi: includeAsuransi, include_evisa: includeEvisa, visa_type: visaType }),
