...
export async function createBooking(formData) {
  ...
  // Komposisi (kamar + kategori khusus). Hitung ulang di server (jangan percaya harga klien).
  let composition = [];
  try { const c = JSON.parse((formData.get('composition') || '[]').toString()); if (Array.isArray(c)) composition = c; } catch {}

  let paxCount = 0, subtotalFull = 0, roomType = null, compStore = [];
  ...
  const { data: booking, error } = await db.from('bookings').insert({
    ...
    notes: JSON.stringify({ admin_fee: adminFee, subtotal: subtotalFull, payment_base: base, composition: compStore }),
