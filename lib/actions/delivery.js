...
export async function sendDeliveryLink(passengerId) {
  ...
  try {
    ...
    // PIC kirim manual: JANGAN antrekan ke WA Manual (tab itu khusus pembayaran
    // online). Balikan template supaya panel Perlengkapan memunculkan modal
    // salin nomor / salin pesan / buka WhatsApp.
    // WABA: template resmi dulu (bisa ke peserta yg belum chat / di luar 24 jam).
    // Param pengiriman_alamat_<brand>: {{1}}Kak nama {{2}}trip {{3}}link. Template tanpa tombol -> link di body.
    {
      const _codeD = (() => { try { return currentBrandCode(); } catch { return 'teone'; } })();
      const _tplAlamat = process.env[`WABA_TPL_PENGIRIMAN_${_codeD === 'khasanah' ? 'KHASANAH' : 'TEONE'}`] || `pengiriman_alamat_${_codeD}`;
      const _tripLabelD = `${trip?.kode_trip || ''} ${trip?.name || ''}`.trim();
      const _paramsD = [(customer?.name ? 'Kak ' + customer.name : 'Kak'), _tripLabelD || '-', link];
      const _wtpl = await trySendWabaTemplateForTrip(supabase, pax.trip_id, phone, _tplAlamat, _paramsD, { kind: 'waba_pengiriman' });
      if (_wtpl?.ok) {
        await supabase.from('trip_passengers').update({ delivery_link_sent_at: new Date().toISOString() }).eq('id', passengerId);
        revalidateAll(pax.trip_id);
        return { ok: true, via: 'waba', wa_phone: phone, target: normalizePhone(phone), link };
      }
    }
    if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
      const _waba = await trySendWabaForTrip(supabase, pax.trip_id, phone, message, { context: 'cs', kind: 'waba_ongkir' });
      if (_waba?.ok) return { ok: true, via: 'waba', wa_phone: phone };
      return { ok: true, wa_manual: true, wa_message: message, wa_phone: phone, customer_name: customer?.name || '' };
    }
...
export async function bulkSendDeliveryLinks(tripId) {
  ...
  try {
    ...
    let sent = 0, failed = 0, manual = 0, errors = [];
    ...
    return {
      ...
      errors,
      message: manual > 0
        ? `📨 ${sent}/${paxList.length} link terkirim${failed > 0 ? ` · ${failed} gagal` : ''} · ${manual} peserta perlu dikirim manual (nomor PIC belum tersambung) — pakai tombol "Kirim link" satu per satu`
        : `📨 ${sent}/${paxList.length} link terkirim${failed > 0 ? ` · ${failed} gagal` : ''}`,
    };
  } catch (e) {
    return { error: 'Error: ' + (e?.message || 'unknown') };
  }
  ...
  try {
    ...
    try {
      ...
      if (phone) {
        ...
        const _waba = await trySendWabaForTrip(supabase, pax.trip_id, phone, msg, { context: 'cs', kind: 'waba_ongkir' });
        if (_waba?.ok) {
          _manual = {};
        } else if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
          _manual = { wa_manual: true, wa_message: msg, wa_phone: phone, customer_name: customer?.name || '' };
        } else {
