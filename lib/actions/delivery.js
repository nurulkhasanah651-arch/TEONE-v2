...
export async function sendDeliveryLink(passengerId) {
  ...
  try {
    ...
    if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
      await queueManualWA(supabase, { phone, message, kind: 'manual_pending_ongkir', context: 'finance' });
  ...
  try {
    ...
    try {
      ...
      if (phone) {
        ...
        if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
          await queueManualWA(supabase, { phone, message: msg, kind: 'manual_pending_ongkir', context: 'finance' });
        } else {
