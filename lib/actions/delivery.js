...
export async function sendDeliveryLink(passengerId, opts = {}) {
  ...
  try {
    ...
    if (await isPicWaManualForTrip(supabase, pax.trip_id)) {
      // Blast massal tetap diantrekan (banyak pesan sekaligus). Kirim satuan dari
      // panel Perlengkapan langsung memunculkan template copy-paste, tidak diantre.
      if (opts.queueManual) {
...
export async function bulkSendDeliveryLinks(tripId) {
  ...
  try {
    ...
    for (const p of paxList) {
      try {
        const r = await sendDeliveryLink(p.id, { queueManual: true });
