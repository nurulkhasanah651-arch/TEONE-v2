...
import { flightLinesFromInventory } from '@/lib/utils/pnr-flights';
import { randomUUID } from 'crypto';

// Bersihkan struktur general info -> [{title, items:[...]}]
function _cleanGeneralInfo(arr) {
...
function buildDefault(trip, crew, fiRows) {
  ...
  return {
    ...
    detail_flight: flightLinesFromInventory(fiRows) || '',  // sinkron dari catatan PNR
    itinerary,
    hotels: [],
    general_info: TC_GENERAL_INFO.map((s) => ({ title: s.title, items: [...s.items] })),
  };
}
...
export async function getTourConfirmation(tripId) {
  ...
  const g = await assertStaff(user, '/operasional/tour-confirmation'); if (g.error) return { error: g.error };
  ...
  let fiRows = [];
  try { const { data: fi } = await db.from('flight_inventory').select('airline, route_from, route_to, departure_date, return_date, routes, notes').eq('trip_id', tripId); fiRows = fi || []; } catch {}

  const { data: existing } = await db.from('tour_confirmations').select('*').eq('trip_id', tripId).maybeSingle();
  const def = buildDefault(trip, crew, fiRows);
  ...
  const tc = existing ? {
    ...
    // Flight ikut catatan PNR bila belum pernah diisi manual di TC.
    detail_flight: (existing.detail_flight && existing.detail_flight.trim()) ? existing.detail_flight : def.detail_flight,
    ...
    general_info: Array.isArray(existing.general_info) && existing.general_info.length ? existing.general_info : def.general_info,
...
export async function saveTourConfirmation(tripId, data = {}) {
  ...
  const general_info = _cleanGeneralInfo(data.general_info);
