...
// Kirim TC ke semua peserta aktif trip dari nomor PIC (Fonnte). 1 pesan per keluarga.
export async function sendTourConfirmation(tripId, customMessage) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const g = await assertStaff(user, '/operasional'); if (g.error) return { error: g.error };
  if (!tripId) return { error: 'Trip belum dipilih.' };
  ...
  // Peserta aktif -> group per nomor (keluarga).
