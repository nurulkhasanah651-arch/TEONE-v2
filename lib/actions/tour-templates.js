...
export async function getPpnPerGroup() {
  ...
  const rows = ids.map((id) => ({
    ...
    coBase: map[id].coBase || 0, ppnCompany: map[id].ppnCompany || 0,
  ...
  const totalCoBase = rows.reduce((s, r) => s + r.coBase, 0);
  const totalCompany = rows.reduce((s, r) => s + r.ppnCompany, 0);
  return { ok: true, rows, totalExpected, totalCollected, totalTour, totalCoBase, totalCompany };
}
