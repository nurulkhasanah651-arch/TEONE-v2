...
// R216f: FINAL FIX field name — sesuai diagnostic actual DB
// CONFIRMED dari sample customer existing:
//   - surname ✓ (bukan last_name)
//   - birthday ✓ (bukan birthdate)
//   - city ✓ (bukan place_of_birth)
//   - gender ✓ (value: L/P)
// SEMUA logic R216c/d (family grouping, defensive insert) TETAP UTUH
...
import { normalizeRoomType } from '@/lib/utils/room-pricing';
...
export async function previewExcelImport(tripId, formData) {
  ...
  // R216f: pakai field name confirmed dari DB
  const { data: allCustomers } = await supabase
    .from('customers').select('id, name, first_name, surname, phone, whatsapp, passport_no');
  ...
  const previewRows = validRows.map((r, idx) => {
    ...
    return {
      ...
      room_type: normalizeRoomType(r.room_type) || r.room_type || '',
...
// ============================================================
// STEP 2: CONFIRM — CORRECT FIELD NAMES (R216f)
// ============================================================
export async function confirmExcelImport(tripId, rows) {
  ...
  const { data: trip } = await supabase.from('trips').select('id, kode_trip, name, price_breakdown').eq('id', tripId).maybeSingle();
  if (!trip) return { error: 'Trip gak ditemukan' };
  const _bd = (trip.price_breakdown && typeof trip.price_breakdown === 'object') ? trip.price_breakdown : {};
  const _brandCode = (() => { try { return currentBrandCode(); } catch { return ''; } })();
  ...
  // Insert customers + trip_passengers + payments
  for (const row of rows) {
    ...
    try {
      ...
      // R216f FIX: CORRECT field names confirmed dari diagnostic
      if (!customerId) {
        const customerData = {
          name: row.full_name,
          first_name: row.first_name,
          surname: row.surname,                    // ✓ confirmed
          phone: row.phone || null,
          whatsapp: row.phone || null,
          gender: row.gender || null,              // ✓ confirmed (L/P)
          passport_no: row.passport_no || null,    // ✓ confirmed
          city: row.place_of_birth || null,        // ✓ confirmed (NOT place_of_birth)
          birthday: row.birthdate || null,         // ✓ confirmed (NOT birthdate)
        ...
        if (error && /column.*does not exist|could not find.*column/i.test(error.message)) {
          // Defensive: identify problematic columns from error msg
          const problematicCols = [];
          ['surname', 'birthday', 'city', 'gender', 'passport_no', 'passport_issued_at',
           'passport_issued_date', 'passport_expiry', 'whatsapp', 'first_name'].forEach((col) => {
      ...
      const paxData = {
        ...
        room_type: normalizeRoomType(row.room_type) || row.room_type || null,
        // Importer dulu tak set age_type sama sekali -> infant dari Excel tersimpan 'adult'.
        age_type: normalizeAgeType(row.age_type, normalizeRoomType(row.room_type) || row.room_type),
        room_code: row.room_code || null,
        // price_paid = HARGA PAKET (lihat self-heal di lib/shop/fulfillment.js), BUKAN uang yg sudah masuk.
        // Dulu diisi row.total_payment -> invoice menganggap tagihan = uang yg sudah dibayar -> "LUNAS" palsu.
        // Uang yg sudah dibayar tetap tercatat sbg participant_payments di bawah.
        price_paid: mainExpectedPerPassenger(
          { room_type: normalizeRoomType(row.room_type) || row.room_type, age_type: normalizeAgeType(row.age_type, normalizeRoomType(row.room_type) || row.room_type) },
          _bd, _brandCode
        ) || null,
