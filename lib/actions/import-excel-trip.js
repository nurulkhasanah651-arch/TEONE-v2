...
// R216c: Import Excel ke Master Trip — FIX + Family auto-grouping
// FIX 1: room_code column missing → defensive insert (try with, fallback without)
// FIX 2: Auto-detect "(N PAX)" marker → bikin family_group otomatis
//        - "(1 PAX)" / no marker without head context → solo traveler
//        - "(N PAX)" N>1 → family head, next N-1 rows = members
...
// R216c: Parse "(N PAX)" / "(N PAX + M CNB)" marker
// Examples:
//   "DARA (1 PAX)" → { paxCount: 1, cleanName: "DARA" }
//   "NATALIA (2PAX)" → { paxCount: 2, cleanName: "NATALIA" }
//   "CLARA (2 PAX + 1 CNB)" → { paxCount: 3, cleanName: "CLARA" }
//   "ROMLAH HERLINA" → { paxCount: 0, cleanName: "ROMLAH HERLINA" }
...
export async function previewExcelImport(tripId, formData) {
  ...
  // Get trip + existing peserta + customers
  ...
  const { data: allCustomers } = await supabase
    .from('customers').select('id, name, first_name, surname, phone, whatsapp, passport_no, passport_number');
  ...
  // R216c: FAMILY GROUPING — assign family_marker per row
  // currentFamily tracks active head; remainingInFamily counts member slots remaining
  ...
  const previewRows = validRows.map((r, idx) => {
    ...
    // Family logic:
    let familyRole = ''; // 'head' | 'member' | 'solo'
    ...
    if (paxCount > 1) {
      ...
      if (remainingInFamily > 0 && currentFamilyHead) {
        ...
        // No active family — standalone (default solo)
        familyRole = 'solo';
      }
    }

    // Match strategy: passport → phone → nama (combo)
    ...
    return {
      ...
      surname: surname,
      full_name: fullName,
      original_name: rawFirstName, // preserve original (with marker) for display
      phone: r.phone || '',
...
// ============================================================
// STEP 2: CONFIRM — actual insert dgn family grouping
// ============================================================
export async function confirmExcelImport(tripId, rows) {
  ...
  // R216c: STEP A — Create family_groups untuk semua head (paxCount > 1)
  // Map: headName (lowercase) → family_group_id
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const { data: fg, error: fgErr } = await supabase
        .from('family_groups').insert(fgData).select().single();
      if (fgErr) {
        // Fallback minimal
        try {
          ...
          if (fg2) {
            familyGroupMap[(row.family_head_name || '').toLowerCase()] = fg2.id;
            insertedFamilies++;
          }
        } catch (e2) {
          errors.push(`Family ${row.full_name}: ${e2?.message || fgErr.message}`);
        }
        continue;
      }
  ...
  // R216c: STEP B — Insert customers + trip_passengers + payments
  for (const row of rows) {
    ...
    try {
      ...
      // R216c FIX: defensive insert — fallback kalau column gak ada
      let newPax;
      ...
      if (paxErr && /column.*does not exist|could not find.*column/i.test(paxErr.message)) {
        // Identify problematic columns from error message
        ...
        if (problematicCols.length > 0) {
          errors.push(`${row.full_name}: ⚠ inserted tanpa kolom: ${problematicCols.join(', ')} (column gak ada di DB)`);
        }
      } else if (paxErr) {
