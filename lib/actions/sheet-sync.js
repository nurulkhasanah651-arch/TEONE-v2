...
// Round 190c: sheet-sync.js
// Tambahin syncTripToSheetFromWebhook (skip auth check) buat webhook Supabase
// Plus keep semua R191 (auto-create via folder) + R189c (link existing)
//
...
function msgFriendly(msg) {
  ...
  if (/TEONE_DRIVE_FOLDER_ID/.test(msg)) return '⚠ Setup folder Drive dulu.';
  ...
  if (/caller does not have permission|permission.*denied|forbidden/i.test(msg)) {
    return '⚠ Permission denied. Share folder/sheet ke service account sebagai Editor.';
  }
  if (/quota|rate.*exceeded/i.test(msg)) return '⚠ Quota habis.';
...
// ============ R190c: CORE sync function (no auth check) ============
// Internal — bisa dipanggil dari webhook atau dari fungsi public yang udah check auth
async function _doSyncTripToSheet(tripId, syncedBy = 'system') {
  ...
  try {
    ...
    const passportRows = [
      ['Pax ID', 'Nama', 'Passport No', 'Nationality', 'Country', 'Issue Date', 'Expiry Date', 'Place of Birth', 'Date of Birth', 'Photo URL'],
      ...paxList.map((p) => {
        const c = custMap[p.customer_id] || {};
    ...
    return { ok: true, counts: { peserta: paxList.length, payment: payments.length, hpp: (items || []).length } };
  } catch (e) {
    const msg = msgFriendly(e?.message || String(e));
    try { await supabase.from('trips').update({ sheet_sync_error: msg }).eq('id', tripId); } catch {}
    return { error: msg };
  }
}

// ============ R190c: SYNC for webhook (NO auth check) ============
export async function syncTripToSheetFromWebhook(tripId) {
  return _doSyncTripToSheet(tripId, 'webhook-autosync');
}

// ============ SYNC TRIP DATA (public — checks auth) ============
export async function syncTripToSheet(tripId) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  return _doSyncTripToSheet(tripId, user.email || 'user');
}
...
export async function createBackupSheet(tripId) {
  ...
  try {
    ...
    if (!trip) return { error: 'Trip gak ditemukan' };
    if (trip.sheet_id) return { error: 'Trip udah punya Sheet.' };
    ...
    const syncResult = await _doSyncTripToSheet(tripId, user.email || 'user');
    if (syncResult.error) {
  ...
  try {
    ...
    try {
      ...
      if (/permission|403/i.test(msg)) {
        ...
        return { error: `⚠ Share sheet ke email: ${saEmail} sebagai Editor.` };
      }
