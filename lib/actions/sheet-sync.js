...
export async function linkExistingSheet(tripId, urlOrId) {
  ...
  try {
    ...
    const requiredTabs = ['Master Info', 'Client Data', 'Manifest', 'Payment Checklist'];
  ...
  try {
    ...
    const clientDataRows = [
      ...
      ...paxList.map((p, idx) => {
        ...
        const status = isTransferred ? 'Transferred' : isRefunded ? 'Refunded' : 'On Going';
    ...
    const manifestRows = [
      ...
      ...paxList.filter((p) => p.transfer_status !== 'transferred').map((p, idx) => {
        const c = custMap[p.customer_id] || {};
        const firstName = c.first_name || (c.name ? c.name.split(' ')[0] : '');
        const surname = c.surname || c.last_name || (c.name ? c.name.split(' ').slice(1).join(' ') : '');
    ...
    const paymentChecklistRows = [
      ...
      // Total row
      ['', 'TOTAL', '', '',
        paxList.reduce((s, p) => s + (Number(p.price_paid) || 0), 0),
        ...milestoneKeys.map((m) => {
          return payments
