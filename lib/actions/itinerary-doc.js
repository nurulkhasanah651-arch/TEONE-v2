...
// Prefill hari dari Tour Confirmation (kalau ada) atau itinerary web.
function defaultDoc(trip, tcItin) {
...
export async function getItineraryDoc(tripId) {
  ...
  const def = defaultDoc(trip, tcItin);
  const doc = saved ? {
...
export async function listItineraryTrips() {
  const g = await guard(); if (g.error) return { error: g.error };
  const { db } = g;
  const [{ data: trips }, { data: pax }] = await Promise.all([
