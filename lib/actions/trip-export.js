...
export async function getTripExportData(tripId) {
  ...
  const clientData = [
    ['No.', 'Kode Booking', 'First Name', 'Surname', 'Title', 'Sex', 'Phone (wa)', 'Room Type', 'Room Code',
     ...
     'Passport/KTP', ...(hasKtp ? ['NIK'] : []), 'Place Of Birth', 'Birthdate', 'Age', 'Issue Date', 'ExpDate', 'Issuing Office', 'Validity Paspor', 'Harga (Rp)'],
    ...activePax.map((p, i) => {
      ...
      return [
        ...
        fmt(c.passport_no || c.passport_number || c.ktp || ''), ...(hasKtp ? [fmt(c.nik || '')] : []), fmt(c.place_of_birth || c.city || ''),
  ...
  const manifest = [
    ...
    ['No.', 'First Name', 'Surname', 'Title', 'Gender', 'Passport/KTP', ...(hasKtp ? ['NIK'] : []), 'Place Of Birth', 'Birthdate', 'Age', 'Issue Date', 'ExpDate', 'Issuing Office', 'No. HP', 'Keterangan'],
    ...activePax.map((p, i) => {
      const c = custMap[p.customer_id] || {};
      const first = c.first_name || (c.name ? c.name.split(' ')[0] : '');
      const surname = c.surname || c.last_name || (c.name ? c.name.split(' ').slice(1).join(' ') : '');
