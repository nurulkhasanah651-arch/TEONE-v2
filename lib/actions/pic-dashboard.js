...
// Nama karyawan (untuk cocokkan trips.pic teks) + tampilan + flag SPV (lihat semua trip).
async function scopeOf(db, user) {
  ...
  let seeAll = false;
  try {
    let emp = (await db.from('employees').select('full_name, nickname, pic_see_all').eq('user_id', user.id).maybeSingle()).data;
    if (!emp && email) emp = (await db.from('employees').select('full_name, nickname, pic_see_all').ilike('email', email).maybeSingle()).data;
    const canon = (emp?.nickname && emp.nickname.trim()) || (emp?.full_name && emp.full_name.trim()) || '';
    if (canon) { display = canon; name = canon.toLowerCase(); }
    seeAll = emp?.pic_see_all === true;
  } catch {}
  return { email, name, display, seeAll };
}
...
export async function getPicDashboard() {
  ...
  const { email, name, display, seeAll } = await scopeOf(db, user);
  // SPV PIC (pic_see_all) → lihat SEMUA trip. PIC biasa → hanya trip yang di-assign ke dia
  // (cocok pic_email otoritatif ATAU teks pic = nama karyawan).
  const tripFilter = seeAll ? null : (t) =>
  ...
  return { ok: true, name: display, seeAll, monitor: m, payment };
}
