...
// ============ RESOLVE TL FROM TRIP (bukan dari auth user) ============
async function resolveTLForTrip(supabase, trip) {
  // 1) Pakai trip.tl_id dulu
  if (trip.tl_id) {
...
// ============ R177v2: OPS REQUEST TL PAYMENT ============
/**
 * Tim Operasional/Manager/Owner/CS yg ajukan request gaji TL untuk trip.
 * TL TIDAK BISA call ini langsung.
 *
 * @param {string} tripId
...
export async function requestTLPayment(tripId, paymentType, options = {}) {
  ...
  // R177v2: ROLE GUARD — Ops/Manager/Finance (+ Owner) only
  ...
  try {
    ...
    // 2) Resolve TL dari trip (BUKAN dari auth user)
    const tl = await resolveTLForTrip(supabase, trip);
    if (!tl) {
      return {
        error: `Trip ini belum punya TL ter-link. tl_name="${trip.tl_name || '-'}". Buka /hr/employees → tambah/edit TL → save, lalu coba lagi.`,
      };
    }

    if (!tl.per_trip_fee || tl.per_trip_fee <= 0) {
      return {
        error: `Fee per trip untuk TL "${tl.full_name}" belum di-set. Buka /hr/employees/${tl.id} → isi Fee per Trip → save.`,
      };
    }

    // 3) Cek existing request untuk trip+payment_type ini
    ...
    // 4) Untuk Final 30% — warning kalau DP belum dibayar
    let warning = null;
    ...
    // Auto-link trips.tl_id kalau sebelumnya kosong
    if (!trip.tl_id) {
