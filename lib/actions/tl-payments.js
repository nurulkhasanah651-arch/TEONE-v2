...
export async function markTLPaymentPaid(id, formData) {
  ...
  try {
    ...
    // R177v4: Update trip_finance_items.payment_status = 'lunas' → masuk REAL cashflow
    if (p.hpp_item_id) {
      try {
        const upd = await supabase
          ...
          .eq('id', p.hpp_item_id);
        if (upd.error && /paid_at|column/i.test(upd.error.message)) {
          // Defensive — kalau paid_at column gak ada, update payment_status saja
          await supabase
            .from('trip_finance_items')
      ...
      try {
        ...
        if (insertRes.data?.id) {
          ...
          const retry = await supabase.from('trip_finance_items').insert(stripped).select('id').single();
          if (retry.data?.id) {
            await supabase.from('tl_payments').update({ hpp_item_id: retry.data.id }).eq('id', id);
          }
        }
      } catch (e) {
  ...
  try {
    ...
    if (p.hpp_item_id) {
      ...
      const upd = await supabase
        ...
        .eq('id', p.hpp_item_id);
      if (upd.error) {
        // Defensive: ada possibility row di-delete manual — re-insert
      ...
      if (insRes.error && /source|column|schema/i.test(insRes.error.message)) {
        // strip source columns
        insRes = await supabase.from('trip_finance_items').insert(payload).select('id').single();
      }
      if (insRes.data?.id) {
