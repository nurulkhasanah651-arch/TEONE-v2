...
export async function requestTLPayment(tripId, paymentType, options = {}) {
  ...
  // ROLE GUARD — Ops/Manager/Finance/Accounting (+ Owner) only
  const role = (user.app_metadata?.role || user.user_metadata?.role || user.app_metadata?.role || '').toLowerCase();
  const ALLOWED = ['ops', 'manager', 'finance', 'owner', 'accounting'];
  if (!ALLOWED.includes(role)) {
    return { error: `Akses ditolak. Hanya Ops/Manager/Finance/Accounting yg bisa ajukan request gaji TL. (role kamu: ${role || 'unknown'})` };
  }
  ...
  try {
    ...
    if (existing) {
      const labels = {
        requested: 'sudah pernah diajukan, menunggu approval Accounting',
    ...
    return {
      ...
      warning,
      message: `✓ Request ${paymentType === 'dp_70' ? '70% DP' : '30% Final'} ${fmtIDR(amount)} untuk TL ${tl.full_name} terkirim ke Accounting.`,
    };
  } catch (e) {
...
// ============ ACCOUNTING APPROVE — 1 langkah: HPP (proyeksi) + Cash Out (real) ============
export async function accountingApproveTLPayment(id, formData) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const role = (user.app_metadata?.role || user.user_metadata?.role || '').toLowerCase();
  if (!['accounting', 'owner'].includes(role)) {
    return { error: `Hanya Accounting/Owner yang bisa approve gaji TL. (role: ${role || 'unknown'})` };
  }
  const supabase = getServiceClient() || authClient;
  ...
  try {
    const { data: p } = await supabase.from('tl_payments').select('*').eq('id', id).maybeSingle();
    if (!p) return { error: 'Request gak ditemukan' };
    if (p.status === 'paid') return { error: 'Sudah diproses (paid) sebelumnya' };
    if (p.status === 'rejected') return { error: 'Sebelumnya di-reject. Reset dulu kalau mau approve.' };

    const typeLabel = p.payment_type === 'dp_70' ? '70% DP' : '30% Final';
    // Insert HPP LANGSUNG LUNAS (transfer_date di-set) → masuk proyeksi + real cashflow group + cash out
    const fullPayload = {
      trip_id: p.trip_id, item_type: 'hpp', category: 'TL Fee',
      component: `TL Fee — ${p.tl_name} — ${typeLabel}`,
      vendor_name: p.tl_name || 'TL',
      basic_fare: Number(p.amount) || 0, qty: 1, total_amount: Number(p.amount) || 0,
      dp_paid: Number(p.amount) || 0, dp_date: today,
      payoff_amount: Number(p.amount) || 0, payoff_date: today, transfer_date: today,
      payment_status: 'lunas', source: 'tl_payment', source_id: id,
      account_id: accountId || null,
      notes: `Gaji TL (${typeLabel}) — approved Accounting by ${approvedBy}. Trip: ${p.trip_kode || p.trip_id}`,
    };
    let hppId = null;
    let { data: ins, error: insErr } = await supabase.from('trip_finance_items').insert(fullPayload).select('id').single();
    if (insErr) {
      // Strip kolom opsional bertahap kalau ada yg belum ada di skema
      const variants = [
        (() => { const x = { ...fullPayload }; delete x.account_id; return x; })(),
        { trip_id: p.trip_id, item_type: 'hpp', category: 'TL Fee', component: `TL Fee — ${p.tl_name} — ${typeLabel}`, vendor_name: p.tl_name || 'TL', basic_fare: Number(p.amount) || 0, qty: 1, total_amount: Number(p.amount) || 0, payment_status: 'lunas', transfer_date: today, payoff_date: today },
        { trip_id: p.trip_id, item_type: 'hpp', category: 'TL Fee', component: `TL Fee — ${p.tl_name} — ${typeLabel}`, vendor_name: p.tl_name || 'TL', total_amount: Number(p.amount) || 0, payment_status: 'lunas' },
      ];
      for (const v of variants) {
        const r = await supabase.from('trip_finance_items').insert(v).select('id').single();
        if (!r.error && r.data) { ins = r.data; insErr = null; break; }
      }
    }
    if (ins) hppId = ins.id;
    if (insErr && !hppId) return { error: 'Gagal catat ke HPP/Cash Out: ' + insErr.message };

    await supabase.from('tl_payments').update({
      status: 'paid',
      approved_at: new Date().toISOString(), approved_by: approvedBy, approval_notes: approvalNotes,
      paid_at: new Date().toISOString(), paid_amount: Number(p.amount) || 0, paid_by: approvedBy,
      hpp_item_id: hppId,
    }).eq('id', id);
    ...
    try {
      ...
      revalidatePath('/accounting'); revalidatePath('/accounting/groups'); revalidatePath('/finance');
    } catch {}
    return { ok: true, hppId, message: `✓ Disetujui & dicatat: ${fmtIDR(p.amount)} masuk Cash Out + HPP trip ${p.trip_kode || ''} (${typeLabel}).` };
  } catch (e) {
    return { error: 'Error: ' + (e?.message || 'unknown') };
  }
}
...
export async function resetTLPaymentToRequested(id) {
  const _auth = createClient();
  const { data: { user } } = await _auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const _g = await assertStaff(user); if (_g.error) return { error: _g.error };
