...
async function applyExtractToCustomer(db, customerId, d) {
  ...
  const { data: c } = await db.from('customers')
    .select('name, first_name, surname, passport_no, passport_number, birthday, dob, city, place_of_birth, gender, sex, nationality')
    .eq('id', customerId).maybeSingle();
  const upd = {};
  // KOLOM KEMBAR: tabel customers punya dua set kolom utk data yg sama.
  // Halaman edit & importer memakai passport_no / birthday / city / gender,
  // sedangkan auto-scan dulu hanya menulis passport_number / dob / place_of_birth / sex
  // -> datanya tersimpan tapi TIDAK tampil di UI (city cuma terisi 39 dari 1221).
  // Sekarang tulis KEDUANYA supaya konsisten apa pun yg dibaca kode lain.
  if (d.passport_number) {
    const v = String(d.passport_number).toUpperCase();
    upd.passport_number = v;
    upd.passport_no = v;
  }
  ...
  if (d.dob) { upd.dob = d.dob; upd.birthday = d.dob; }
  if (d.sex) {
    upd.sex = d.sex;
    const g = d.sex === 'M' ? 'L' : d.sex === 'F' ? 'P' : null;   // UI simpan L/P, bukan M/F
    if (g) upd.gender = g;
  }
  if (d.place_of_birth) {
    upd.place_of_birth = d.place_of_birth;
    if (c && !c.city) upd.city = d.place_of_birth;   // ikut perilaku halaman edit: isi hanya bila kosong
  }
  ...
  if (Object.keys(upd).length) {
    ...
    if (error) console.error('[applyExtractToCustomer] gagal simpan hasil scan', { customerId, kolom: Object.keys(upd), error: error.message });
  }
}
...
function passportNameMismatch(sysName, scannedFull) {
...
// DOWNLOAD paspor yg sudah diupload — signed URL dgn Content-Disposition: attachment,
// jadi browser langsung mengunduh file (bukan buka di tab). Nama file = paspor-<nama>.<ext>.
export async function getPassportDownloadUrl(passengerId) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const g = await assertStaff(user, '/trips'); if (g.error) return { error: g.error }; }
  ...
  if (!db) return { error: 'Service tidak tersedia' };
  const { data: pax } = await db.from('trip_passengers')
    .select('passport_upload_path, customer_id').eq('id', passengerId).maybeSingle();
  if (!pax?.passport_upload_path) return { error: 'Belum ada paspor diupload' };

  let nama = `peserta-${passengerId}`;
  if (pax.customer_id) {
    const { data: c } = await db.from('customers').select('name, first_name, surname').eq('id', pax.customer_id).maybeSingle();
    const n = (c?.name || `${c?.first_name || ''} ${c?.surname || ''}`).trim();
    if (n) nama = n;
  }
  const ext = (pax.passport_upload_path.split('.').pop() || 'jpg').split('?')[0].slice(0, 5);
  const safe = String(nama).normalize('NFKD').replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '_') || `peserta-${passengerId}`;
  const filename = `paspor-${safe}.${ext}`;

  const { data: signed, error } = await db.storage.from(BUCKET)
    .createSignedUrl(pax.passport_upload_path, 600, { download: filename });
  if (error || !signed?.signedUrl) return { error: 'Gagal membuat link download: ' + (error?.message || 'unknown') };
