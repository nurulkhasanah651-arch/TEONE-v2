...
export async function sendPassportUploadWA(passengerId) {
  ...
  const _tplPassport = process.env[`WABA_TPL_PASSPORT_${code === 'khasanah' ? 'KHASANAH' : 'TEONE'}`] || `passport_upload_${code}`;
  const _tripLabel = `${trip?.kode_trip ? trip.kode_trip + ' \u2014 ' : ''}${trip?.name || ''}`.trim();
  const _introLine = _picName ? `Saya ${_picName}, PIC trip kamu di ${brandName}.` : `Kami tim ${brandName}.`;
  const _passportParams = [cust?.name || 'Kak', _introLine, _tripLabel || '-'];
  let _waba = await trySendWabaTemplateForTrip(db, head.trip_id, phone, _tplPassport, _passportParams, { kind: 'waba_passport', buttonUrlSuffix: token });
...
async function runScan(db, pax) {
  ...
  await applyExtractToCustomer(db, pax.customer_id, r.data || {});
  await db.from('trip_passengers').update({ passport_autofilled: true }).eq('id', pax.id);
...
export async function scanUploadedPassport(passengerId) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const g = await assertStaff(user, '/trips'); if (g.error) return { error: g.error }; }
