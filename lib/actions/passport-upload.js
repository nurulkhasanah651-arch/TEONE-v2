...
async function membersForTokenPax(db, tokenPax) {
  if (tokenPax.family_group_id) {
    const { data: mem } = await db.from('trip_passengers')
      .select('id, customer_id, passport_upload_path, passport_uploaded_at, is_family_head')
...
export async function sendPassportUploadWA(passengerId) {
  ...
  // WABA resmi dulu kalau PIC trip ini bernomor WABA.
  const _waba = await trySendWabaForTrip(db, head.trip_id, phone, lines.join('\n'), { context: 'cs', kind: 'waba_passport' });
  if (!_waba?.ok) {
