...
export async function buildCeoMetrics() {
  ...
  const [invArr, paxArr, tripsRes, leadsArr, refundsArr, payArr, finArr] = await Promise.all([
    ...
    fetchAll(() => supabase.from('participant_payments').select('amount')),
    fetchAll(() => supabase.from('trip_finance_items').select('total_amount, item_type, payment_status, dp_paid')),
  ]);
  ...
  // Uang peserta yg SUDAH dibayar (kas masuk dari peserta) & HPP yg SUDAH dibayar (kas keluar).
  // Real uang perusahaan = uang peserta dibayar - HPP dibayar.
  let paidByParticipants = 0;
  for (const p of (payArr || [])) paidByParticipants += num(p.amount);
  let totalHpp = 0, hppPaid = 0;
  for (const f of (finArr || [])) {
    if (f.item_type !== 'hpp') continue;
    totalHpp += num(f.total_amount);
    hppPaid += String(f.payment_status || '').toLowerCase() === 'lunas' ? num(f.total_amount) : num(f.dp_paid);
  }
  const realCompanyMoney = paidByParticipants - hppPaid;
  ...
  return {
    ...
    bookedValue, paidByParticipants, totalHpp, hppPaid, realCompanyMoney, activePaxCount: activePax.length,
