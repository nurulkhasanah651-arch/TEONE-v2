...
export async function buildCeoMetrics() {
  ...
  const [invArr, paxArr, tripsRes, leadsArr, refundsArr, payArr, finArr] = await Promise.all([
    ...
    fetchAll(() => supabase.from('trip_finance_items').select('total_amount, item_type')),
  ]);
  ...
  // Uang peserta yg SUDAH dibayar (kas masuk dari peserta) & total HPP (biaya trip).
  let paidByParticipants = 0;
  for (const p of (payArr || [])) paidByParticipants += num(p.amount);
  let totalHpp = 0;
  for (const f of (finArr || [])) if (f.item_type === 'hpp') totalHpp += num(f.total_amount);
  ...
  return {
    ...
    bookedValue, paidByParticipants, totalHpp, grossProfit: bookedValue - totalHpp, activePaxCount: activePax.length,
