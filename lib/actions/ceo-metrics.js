...
export async function buildCeoMetrics() {
  ...
  return {
    ...
    omzetThisMonth, omzetPrevMonth, omzetYtd, omzetAllTime, omzetDelta, outstanding,
    bookedValue, activePaxCount: activePax.length, closeThisMonth, closePrevMonth, closeDelta,
...
export function metricsToContext(m) {
  ...
  return `DATA PERUSAHAAN — ${m.brandLabel} (bulan berjalan ${monthLabel(m.curMonth)}):
- Omzet masuk (cash paid) bulan ini: ${rp(m.omzetThisMonth)}${m.omzetDelta != null ? ` (${m.omzetDelta >= 0 ? '+' : ''}${m.omzetDelta}% vs bulan lalu ${rp(m.omzetPrevMonth)})` : ''}
...
- Closing bulan ini: ${m.closeThisMonth} pax${m.closeDelta != null ? ` (${m.closeDelta >= 0 ? '+' : ''}${m.closeDelta}% vs bulan lalu ${m.closePrevMonth} pax)` : ''}
- Leads bulan ini: ${m.leadsThisMonth}, conversion rate ${m.convRate}%
