...
// SAVE template default "Kenapa harus umroh/join bareng kami" (storefront)
// force=true → template ini dipakai di SEMUA trip (menimpa alasan yang diisi per-trip).
export async function saveReasonsDefault(text, force) {
  const auth = createClient();
  const { data: { user } } = await auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const db = svc();
  ...
  const { error } = await db.from('storefront_settings')
    .update({ reasons_default: clean, reasons_force: force === true, updated_at: new Date().toISOString() }).eq('id', 1);
