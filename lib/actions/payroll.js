...
// Round 178b: Payroll — + auto-send slip WA saat Mark All Paid
...
import { sendInternalPayslipToWA } from './payslip-extras';
import { assertStaff } from '@/lib/auth/require-staff';

// Guard payroll: hanya owner/accounting (path /hr). Dipakai di action tanpa getUser sendiri.
async function requireHR() {
  const auth = createClient();
  const { data: { user } } = await auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const _g = await assertStaff(user, '/hr'); if (_g.error) return { error: _g.error }; }
  const g = await assertStaff(user, '/hr'); if (g.error) return { error: g.error };
  return { user };
}
...
export async function syncPayrollEntryToAccounting(entryId) {
  { const _g = await requireHR(); if (_g.error) return { error: _g.error }; }
...
export async function markAllEntriesPaid(periodId, options = {}) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const _g = await assertStaff(user, '/hr'); if (_g.error) return { error: _g.error }; }
  ...
  const supabase = getServiceClient() || authClient;
  const skipWA = options?.skipWA === true;
  ...
  // Ambil semua entries untuk loop sync + WA
  const { data: entries } = await supabase
    .from('payroll_entries').select('id, net_pay').eq('period_id', periodId);

  // R178: Auto-sync semua entries ke cash out
  let syncedCount = 0;
  for (const e of entries || []) {
    try {
      const r = await syncPayrollEntryToAccounting(e.id);
      if (!r?.error) syncedCount++;
    } catch (err) { console.error('[markAllEntriesPaid sync]', err?.message); }
  }

  // R178b: Auto-send slip WA ke semua karyawan
  let waResult = { sent: 0, failed: 0, no_phone: 0, zero_pay: 0, errors: [] };
  if (!skipWA) {
    for (const e of entries || []) {
      // Skip kalau net_pay = 0 (gak perlu kirim slip kosong)
      if (!e.net_pay || Number(e.net_pay) <= 0) {
        waResult.zero_pay++;
        continue;
      }
      try {
        const r = await sendInternalPayslipToWA(e.id);
        if (r?.error) {
          if (/nomor HP|phone|WA/i.test(r.error)) {
            waResult.no_phone++;
          } else {
            waResult.failed++;
            waResult.errors.push(`#${e.id}: ${r.error}`);
          }
        } else {
          waResult.sent++;
        }
        ...
        waResult.failed++;
        waResult.errors.push(`#${e.id}: ${err?.message || 'unknown'}`);
      }
    }
  }
  ...
  return {
    ...
    cash_out_synced: syncedCount,
    wa_sent: waResult.sent,
    wa_failed: waResult.failed,
    wa_no_phone: waResult.no_phone,
    wa_zero_pay: waResult.zero_pay,
    wa_errors: waResult.errors,
    message: `✓ ${entries?.length || 0} entries marked PAID · ${syncedCount} masuk cash out · ${waResult.sent} slip dikirim via WA${waResult.no_phone > 0 ? ` (${waResult.no_phone} skip: no phone)` : ''}${waResult.failed > 0 ? ` · ${waResult.failed} gagal` : ''}`,
  };
}
...
// R178b: Send all slips via WA (manual trigger, gak update status)
export async function sendAllPayrollSlipsViaWA(periodId) {
  { const _g = await requireHR(); if (_g.error) return { error: _g.error }; }
  const supabase = getServiceClient() || createClient();
  try {
    const { data: entries } = await supabase
    ...
    for (const e of entries || []) {
      ...
      try {
        const r = await sendInternalPayslipToWA(e.id);
        if (r?.error) {
          if (/nomor HP|phone|WA/i.test(r.error)) waResult.no_phone++;
          else {
            waResult.failed++;
            waResult.errors.push(`#${e.id}: ${r.error}`);
          }
    ...
    return {
      ...
      total_entries: entries?.length || 0,
      ...waResult,
      message: `📱 ${waResult.sent} slip terkirim · ${waResult.no_phone} skip (no phone) · ${waResult.zero_pay} skip (zero pay) · ${waResult.failed} gagal`,
    };
  } catch (e) {
    return { error: 'Error: ' + (e?.message || 'unknown') };
  }
