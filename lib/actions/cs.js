...
// price_paid = HARGA PAKET, dihitung sistem (field manualnya sudah dibuang dari form).
// Balikan 0 kalau harga trip belum diisi -> pemanggil pakai nilai lama.
async function hitungHargaPaketCS(db, tripId, room_type, age_type) {
  try {
  ...
  for (let idx = 0; idx < participants.length; idx++) {
    ...
    const { data: passenger, error: pErr } = await supabase
      .from('trip_passengers').insert({
        ...
        price_paid: (await hitungHargaPaketCS(supabase, f.trip_id, p.room_type, normalizeAgeType(p.age_type, p.room_type))) || parseInt(p.price_paid) || 0,
        discount_amount: parseInt(p.discount) || 0,
        age_type: normalizeAgeType(p.age_type, p.room_type),
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    const { data: passenger, error: pErr } = await supabase
      .from('trip_passengers').insert({
        ...
        room_type: p.room_type || null,
        price_paid: (await hitungHargaPaketCS(supabase, tripId, p.room_type, normalizeAgeType(p.age_type, p.room_type))) || parseInt(p.price_paid) || 0,
        discount_amount: parseInt(p.discount) || 0,
        age_type: normalizeAgeType(p.age_type, p.room_type),
        mitra_id: (p.source === 'mitra') ? (parseInt(formData.get('mitra_id')) || null) : null,
        lead_source: 'cs',
...
export async function completeClosingDraft(draftId, formData) {
  ...
  const { data: passenger, error: pErr } = await db.from('trip_passengers').insert({
    ...
    price_paid: (await hitungHargaPaketCS(db, tripId, formData.get('room_type'), normalizeAgeType(formData.get('age_type'), formData.get('room_type')))) || parseInt(formData.get('price_paid')) || 0,
    ...
    include_evisa: formData.get('include_evisa') === 'on',
  }).select('id').single();
