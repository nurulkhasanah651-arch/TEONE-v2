...
// Round 135 HOTFIX: CS actions — SUPER defensive insert + better error logging
...
// Validate date string → null kalau invalid
function safeDate(s) {
  if (!s) return null;
...
// Build customer payload — ONLY include passport fields kalau ada SQL R134
function buildCustomerPayload(p, fullName, first, last) {
  ...
  // Passport fields (R134) — validate date format dulu
...
// SUPER DEFENSIVE: retry sampai berhasil dengan strip-down progressive
async function insertCustomerDefensive(supabase, payload) {
  ...
  // Attempt 2: strip passport fields
  const stripped2 = {
...
async function insertPassengerDefensive(supabase, payload) {
  ...
  // Attempt 2: strip cs_update_id (kalau SQL R135 belum jalan)
...
export async function createCSUpdate(formData) {
  ...
  // ROUND 135 HOTFIX: kasih warning kalau ada peserta yg gak ke-save
  ...
  if (participants.length > 0 && insertedCount === 0 && skippedNoName > 0) {
    return {
      error: `⚠ ${skippedNoName} peserta gak ke-save karena nama depan/belakang kosong. Pastikan isi minimal Nama Depan ATAU Nama Belakang.`,
    };
  }
...
export async function deleteCSUpdate(id) {
  ...
  const { data: linkedPax } = await supabase
    .from('trip_passengers').select('id, customer_id')
    .eq('cs_update_id', id);
