...
// Pastikan pemanggil adalah STAF yang berhak (bisa akses /invoices). Akun peserta/
// customer (role dari metadata) di-reject karena tidak terdaftar di master karyawan.
async function requireFinanceUser(supabase) {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const role = await resolveAuthoritativeRole(user, getRoleFromUser(user));
  if (!role || role === 'pending' || !canAccessPath(role, '/invoices')) {
    return { error: 'Akses ditolak: hanya staf finance yang boleh memverifikasi.' };
  }
  return { user, role };
}
...
export async function submitManualTransfer(bookingId, formData) {
  ...
  const { data: b } = await db.from('bookings')
    .select('id, status, manual_status, amount, notes').eq('id', bookingId).maybeSingle();
  if (!b) return { error: 'Booking tidak ditemukan' };
  if (b.status === 'paid') return { error: 'Booking ini sudah lunas' };

  // Transfer manual: hilangkan biaya admin (admin 13rb hanya utk pembayaran online).
  let newAmount = Number(b.amount) || 0;
  let newNotes = b.notes;
  try {
    const meta = JSON.parse(b.notes || '{}');
    const fee = Number(meta.admin_fee || 0);
    if (fee > 0) {
      newAmount = Math.max(newAmount - fee, 0);
      meta.admin_fee = 0;
  ...
  // Hanya terima URL file dari bucket storage kita (cegah injeksi URL eksternal/phishing).
  if (!/\/storage\/v1\/object\/.*payment-proofs\//.test(String(proof_url))) {
    return { error: 'Bukti transfer tidak valid' };
  }
...
export async function approveManualTransfer(bookingId) {
  ...
  const guard = await requireFinanceUser(supabase);
