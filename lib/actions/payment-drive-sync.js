...
export async function syncTripPaymentsToDrive(tripId) {
  ...
  const { data: payments } = await supabase
    ...
    .in('status', ['verified', 'pending']); // sync semua yg ada bukti, mau pending atau verified
  ...
  for (const payment of payments) {
    if (!payment.proof_url) { totalSkipped++; continue; }
    if (payment.drive_file_id) { totalSkipped++; continue; } // sudah ke-sync

    const inv = invMap[payment.invoice_id];
    if (!inv) { totalSkipped++; continue; }
