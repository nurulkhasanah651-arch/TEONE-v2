...
function countPax(paxRows) {
  ...
  for (const p of (paxRows || [])) {
    ...
    // Visa: peserta 'visa_ready' (sudah punya visa sendiri) TIDAK dihitung — konsisten dgn
    // invoice/billing yg tidak menagih visa ke peserta ready. (Kecuali memang sudah BAYAR visa.)
    if ((active && ((p.include_visa === true && p.visa_ready !== true) || p._paid_visa === true)) || (!active && p._paid_visa === true)) visa++;
