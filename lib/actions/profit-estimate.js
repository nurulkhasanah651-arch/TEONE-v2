...
function countPax(paxRows) {
  ...
  for (const p of (paxRows || [])) {
    ...
    // Ikut visa/asuransi kalau checkbox ON (peserta baru) ATAU sudah bayar (peserta lama).
    if (p.include_visa === true || p._paid_visa === true) visa++;
    if (p.include_asuransi === true || p._paid_asuransi === true) asr++;
  }
...
async function fetchAutoAdjust(db, tripIds) {
  const visaByTrip = {}; const hangusByTrip = {};
  ...
  return { visaByTrip, hangusByTrip };
}
...
export async function listProfitGroups() {
  ...
  const { visaByTrip, hangusByTrip } = await fetchAutoAdjust(db, active.map((t) => t.id));
  ...
  const rows = active.map((t) => {
    ...
    const tot = estimateTotals(bd, s || null, countPax(paxByTrip[t.id]), { visa: visaByTrip[t.id] || 0, hangus: hangusByTrip[t.id] || 0 });
...
export async function getProfitEstimate(tripId) {
  ...
  const { visaByTrip, hangusByTrip } = await fetchAutoAdjust(db, [tripId]);
  ...
  return {
    ...
    autoIncome: autoHangus > 0 ? [{ key: 'auto_hangus', label: 'Dana Hangus (refund/cancel)', amount: autoHangus }] : [],
