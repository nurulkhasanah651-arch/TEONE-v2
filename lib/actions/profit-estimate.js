...
import { isKhasanahBrand } from '@/lib/utils/price-breakdown';
...
const INCOME_KEYS = [
  ...
  // KHASANAH — item WAJIB gabungan (biaya pokok harga). Hanya tampil bila diisi di master trip.
  { key: 'asuransi_tips_local_guide', label: 'Asuransi & Tips Local Guide', bd: ['asuransi_tips_local_guide'], optional: true },
  { key: 'handling_perlengkapan', label: 'Handling & Perlengkapan', bd: ['handling_perlengkapan'], optional: true },
  { key: 'visa_asuransi', label: 'Visa & Asuransi', bd: ['visa_asuransi'], optional: true },
...
function countPax(paxRows, isKh = false) {
  ...
  for (const p of (paxRows || [])) {
    ...
    if (active) {
      ...
      if (p.visa_ready === true) vrActive++;
    }
    // Visa/asuransi opt-in (TEONE): peserta AKTIF (checkbox ON atau sudah bayar) ATAU peserta
    // refund/pindah yang SUDAH BAYAR. Peserta 'visa_ready' tidak dihitung visa (punya visa sendiri).
    if ((active && ((p.include_visa === true && p.visa_ready !== true) || p._paid_visa === true)) || (!active && p._paid_visa === true)) visaOpt++;
    if ((active && (p.include_asuransi === true || p._paid_asuransi === true)) || (!active && p._paid_asuransi === true)) asrOpt++;
  }
  ...
  // KHASANAH: visa & asuransi = BIAYA POKOK harga (wajib untuk semua jamaah), bukan opt-in.
  //  - visa   : semua peserta aktif kecuali infant & yang sudah 'ready visa'.
  //  - asuransi: semua peserta aktif kecuali infant.
  //  - item gabungan wajib (asuransi_tips_local_guide / handling_perlengkapan / visa_asuransi):
  //    semua peserta aktif kecuali infant & child-no-bed. (Hanya berharga bila diisi di master.)
  const visa = isKh ? Math.max(exInfant - vrActive, 0) : visaOpt;
  const asuransi = isKh ? exInfant : asrOpt;
  const kombo = isKh ? billable : 0;
  return {
    ...
    visa, asuransi,
    asuransi_tips_local_guide: kombo, handling_perlengkapan: kombo, visa_asuransi: kombo,
    headcount: head,
  };
}
...
function estimateTotals(bd, saved, paxBy, auto) {
  ...
  for (const ik of INCOME_KEYS) {
    ...
    // Baris income yg dihapus manual: tidak dihitung ke income & pax efektif = 0 (qty expense auto ikut 0).
    if (s.removed === true) { resolved[ik.key] = 0; continue; }
    const fare = masterPrice(bd, ik.bd);
    const pax = s.pax_override === true ? numOrZero(s.pax) : (paxBy[ik.key] || 0);
    resolved[ik.key] = pax;
    totalIncome += fare * pax;
  }
  for (const s of savedCustom) { if (s.removed === true) continue; totalIncome += numOrZero(s.basic_fare) * numOrZero(s.pax); }
...
export async function listProfitGroups() {
  ...
  try {
    for (let from = 0; ; from += 1000) {
      const { data } = await db.from('trip_passengers')
        .select('id, trip_id, room_type, include_visa, include_asuransi, visa_ready, transfer_status, refund_status').order('id', { ascending: true }).range(from, from + 999);
  ...
  const rows = active.map((t) => {
    ...
    const _paxBy = countPax(paxByTrip[t.id], isKhasanahBrand(brandCode));
...
export async function setOwnerReview(tripId, note, approval) {
  ...
  if (!REVIEW_ROLES.includes(role)) return { error: 'Hanya owner yang bisa memberi catatan/approval.' };
...
export async function getProfitEstimate(tripId) {
  ...
  const { data: pax } = await db.from('trip_passengers')
    .select('id, room_type, include_visa, include_asuransi, visa_ready, transfer_status, refund_status').eq('trip_id', tripId).range(0, 4999);
  await annotatePaidVA(db, pax || []);
  let _brandCode = 'teone'; try { _brandCode = getBrandCode(); } catch {}
  const paxMasterBy = countPax(pax, isKhasanahBrand(_brandCode));
  ...
  for (const s of savedCustom) {
    if (s.removed === true) continue; // custom yg dihapus tidak dikembalikan
    income.push({
...
export async function saveProfitEstimate(tripId, payload) {
  ...
  const income_rows = (p.income || []).map((r) => ({
    ...
    removed: r.removed === true, // baris income yg dihapus manual (tidak dihitung ke total income)
  }));
