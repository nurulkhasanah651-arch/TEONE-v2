...
function estimateTotals(bd, saved, paxBy) {
  ...
  for (const e of (saved?.expense_rows || [])) {
    if (e?.type === 'hotel') {
      for (const h of (e.rooms || [])) totalExpense += (numOrZero(h.unit_cost) / occFromKeyLabel(h.room, h.label)) * effQty(h) * (numOrZero(h.nights) || 1);
    } else {
      totalExpense += numOrZero(e.unit_cost) * effQty(e);
    }
  }
...
export async function listProfitGroups() {
  ...
  // Ambil peserta HANYA untuk trip yang punya estimate (hemat query).
  const estTripIds = active.filter((t) => savedByTrip[t.id]).map((t) => t.id);
  const paxByTrip = {};
  for (let i = 0; i < estTripIds.length; i += 100) {
    const { data } = await db.from('trip_passengers')
      .select('trip_id, room_type, include_visa, include_asuransi').in('trip_id', estTripIds.slice(i, i + 100));
    for (const p of (data || [])) { (paxByTrip[p.trip_id] = paxByTrip[p.trip_id] || []).push(p); }
  }

  const rows = active.map((t) => {
    const s = savedByTrip[t.id];
    let profit = null, income = null, expense = null;
    if (s) {
      const tot = estimateTotals((t.price_breakdown && typeof t.price_breakdown === 'object') ? t.price_breakdown : {}, s, countPax(paxByTrip[t.id]));
    ...
    return {
      ...
      hasEstimate: !!s, savedAt: s?.updated_at || null, profit, income, expense,
    };
  });
  return { ok: true, rows };
}
