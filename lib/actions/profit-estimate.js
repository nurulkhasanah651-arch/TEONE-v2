  ...
  try {
    for (let from = 0; ; from += 1000) {
      const { data } = await db.from('trip_passengers')
        .select('trip_id, room_type, include_visa, include_asuransi, transfer_status, refund_status').order('id', { ascending: true }).range(from, from + 999);
...
export async function getProfitEstimate(tripId) {
  ...
  const { data: pax } = await db.from('trip_passengers')
    .select('room_type, include_visa, include_asuransi, transfer_status, refund_status').eq('trip_id', tripId).range(0, 4999);
