...
export async function createReimbursement({
  ...
  const { data: { user } } = await _auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  // Staf internal boleh; TL boleh mengajukan reimbursement utk trip yang dia pimpin.
  const _staff = await assertStaff(user);
  if (_staff.error) {
    const _role = await resolveAuthoritativeRole(user, getRoleFromUser(user));
    if (_role !== 'tour_leader' || !tripId) return { error: _staff.error };
    let _owns = false;
    try {
      const _svc0 = getServiceClient();
      const { data: _trip } = await _svc0.from('trips').select('id, tl_id, tl_email, tl_phone').eq('id', tripId).maybeSingle();
      if (_trip) {
        const _identity = await resolveTlIdentity(user).catch(() => null);
        _owns = !!_identity && tlOwnsTrip(_identity, _trip, currentBrandCode());
      }
    } catch {}
    if (!_owns) return { error: 'Kamu hanya bisa mengajukan reimbursement untuk trip yang kamu pimpin.' };
  }
...
export async function getTripDocDownloadUrl(docId) {
  ...
  try {
    ...
    // Nama unduhan: judul + ekstensi asli dari path/url
    ...
    if (path) {
      for (const b of candidates) {
        ...
        if (!error && data?.signedUrl) return { ok: true, url: data.signedUrl, filename: fname };
      }
    }
    // Fallback terakhir: kembalikan file_url apa adanya
    if (doc.file_url) return { ok: true, url: doc.file_url, filename: fname };
