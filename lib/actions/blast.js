...
// Penerima: semua peserta aktif di trip milik PIC, dikelompokkan per nomor (keluarga)
export async function getPicBlastRecipients(picEmailArg) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const g = await assertStaff(user, '/blast'); if (g.error) return { error: g.error };
  const db = svc() || supabase;
  ...
  if (!tripIds.length) return { ok: true, picEmail: email, picName, tripCount: 0, tripKodes: [], families: [], totalPax: 0, noPhone: 0 };

  const { data: pax } = await db.from('trip_passengers')
    .select('id, customer_id, status, transfer_status, refund_status, is_family_head, trip_id').in('trip_id', tripIds);
  const active = (pax || []).filter(paxActive).filter((p) => p.customer_id);
  const ids = [...new Set(active.map((p) => p.customer_id))];
  const custMap = {};
  ...
  families.sort((a, b) => (b.count - a.count) || String(a.headName).localeCompare(String(b.headName)));
  return { ok: true, picEmail: email, picName, tripCount: tripIds.length, tripKodes: (trips || []).map((t) => t.kode_trip || t.name), families, totalPax: active.length, noPhone };
}

// Kirim blast perkenalan dari nomor PIC. {{nama}}->nama depan kepala keluarga, {{pic}}->nama PIC
export async function sendPicBlast(picEmailArg, message) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const g = await assertStaff(user, '/blast'); if (g.error) return { error: g.error };
  const msg = String(message || '').trim(); if (!msg) return { error: 'Pesan kosong.' };

  const r = await getPicBlastRecipients(picEmailArg);
  if (r.error) return r;
  ...
  for (const fam of r.families) {
    ...
    const res = await sendFonnte(fam.phone, personalized, { context: 'cs', brand, token: picTok });
    if (res?.ok) sent++;
