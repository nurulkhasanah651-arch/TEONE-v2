...
export async function sendBlast(tripId, message, selectedIds, mediaUrl, mediaName) {
  ...
  for (const phone of Object.keys(byPhone)) {
    ...
    const res = await sendFonnte(phone, personalized, { context: 'cs', brand, token: picTok, delay: BLAST_DELAY, typing: true, url: mediaUrl || null, filename: mediaName || null });
    if (res?.ok) sent++;
    else {
      failed++;
...
export async function sendPicBlast(picEmailArg, message, tripIdsArg, mediaUrl, mediaName) {
  ...
  for (const fam of r.families) {
    ...
    const res = await sendFonnte(fam.phone, personalized, { context: 'cs', brand, token: picTok, delay: BLAST_DELAY, typing: true, url: mediaUrl || null, filename: mediaName || null });
    if (res?.ok) sent++;
