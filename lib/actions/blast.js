...
export async function sendBlast(tripId, message, selectedIds, mediaUrl, mediaName) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const g = await assertStaff(user, '/blast');
  if (g.error) return { error: g.error };
  ...
  // Kirim dari nomor PIC trip; kalau PIC belum punya nomor Fonnte -> fallback CS.
  let picTok = null;
  try {
    const { data: trip } = await supabase.from('trips').select('pic, pic_email').eq('id', tripId).maybeSingle();
    picTok = await getPicFonnteTokenForTrip({ pic: trip?.pic, pic_email: trip?.pic_email });
  } catch {}
  let sent = 0, failed = 0;
  for (const phone of Object.keys(byPhone)) {
    ...
    let personalized = msg.replace(/\{\{\s*nama\s*\}\}/gi, first);
    if (mediaUrl) personalized += `\n\n\ud83d\udcce ${mediaName || 'Dokumen'}:\n${mediaUrl}`;
    const res = await sendFonnte(phone, personalized, { context: 'cs', brand, token: picTok, delay: BLAST_DELAY, typing: true });
    if (res?.ok) sent++;
    else {
      failed++;
...
export async function sendPicBlast(picEmailArg, message, tripIdsArg, mediaUrl, mediaName) {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const g = await assertStaff(user, '/blast'); if (g.error) return { error: g.error };
  ...
  for (const fam of r.families) {
    ...
    const _withDoc = mediaUrl ? `${personalized}\n\n\ud83d\udcce ${mediaName || 'Dokumen'}:\n${mediaUrl}` : personalized;
    const res = await sendFonnte(fam.phone, _withDoc, { context: 'cs', brand, token: picTok, delay: BLAST_DELAY, typing: true });
    if (res?.ok) sent++;
...
// Upload dokumen untuk blast (PDF/gambar) -> bucket publik 'blast-docs' -> URL utk Fonnte.
export async function uploadBlastDoc(formData) {
  const auth = createClient();
  const { data: { user } } = await auth.auth.getUser();
  const g = await assertStaff(user, '/blast'); if (g.error) return { error: g.error };
  const file = formData.get('file');
  if (!file || typeof file === 'string') return { error: 'File tidak ada' };
  if (file.size > 26214400) return { error: 'File maksimal 25MB' };
  const db = svc();
  if (!db) return { error: 'Storage tidak tersedia' };
  try {
    const buf = Buffer.from(await file.arrayBuffer());
    const safe = String(file.name || 'dokumen').replace(/[^a-zA-Z0-9._-]/g, '_').slice(-60);
    const path = `${Date.now()}-${safe}`;
    const { error: upErr } = await db.storage.from('blast-docs').upload(path, buf, { contentType: file.type || 'application/octet-stream', upsert: false });
    if (upErr) return { error: 'Upload gagal: ' + upErr.message };
    const { data: pub } = db.storage.from('blast-docs').getPublicUrl(path);
    return { ok: true, url: pub?.publicUrl || null, name: file.name || safe };
  } catch (e) { return { error: 'Error: ' + (e?.message || 'unknown') }; }
}

// Signed upload URL -> browser upload LANGSUNG ke Supabase (cepat, bypass server/Vercel).
export async function createBlastUploadUrl(filename) {
  const auth = createClient();
  const { data: { user } } = await auth.auth.getUser();
  const g = await assertStaff(user, '/blast'); if (g.error) return { error: g.error };
  const db = svc(); if (!db) return { error: 'Storage tidak tersedia' };
  const safe = String(filename || 'dokumen').replace(/[^a-zA-Z0-9._-]/g, '_').slice(-60);
  const path = `${Date.now()}-${safe}`;
  const { data, error } = await db.storage.from('blast-docs').createSignedUploadUrl(path);
  if (error) return { error: 'Gagal siapkan upload: ' + error.message };
  let base = '';
  try { base = customerSiteUrlFor(getBrandCode()); } catch {}
  const publicUrl = base ? `${base}/doc/${encodeURIComponent(path)}` : (db.storage.from('blast-docs').getPublicUrl(path)?.data?.publicUrl || null);
  return { ok: true, signedUrl: data.signedUrl, token: data.token, path, publicUrl, name: filename };
}
