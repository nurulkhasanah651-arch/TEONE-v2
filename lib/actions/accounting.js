...
import { syncPnrToHPP } from '@/lib/actions/pnr';
...
export async function createAccountingEntry(formData) {
  ...
  const linked_pnr_id = parseInt(formData.get('linked_pnr_id')) || null;
  const pnr_mode = formData.get('pnr_mode') || 'deposit'; // 'deposit' | 'lunas'
  ...
  // Sinkron ke PNR (flight_inventory): cash out -> tambah deposit, atau lunas penuh
  if (type === 'out' && linked_pnr_id) {
    const { data: pnr } = await supabase
      .from('flight_inventory').select('*').eq('id', linked_pnr_id).maybeSingle();
    if (pnr) {
      const isFit = pnr.ticket_type === 'fit';
      const totalCost = isFit
        ? (Number(pnr.total_amount) || 0)
        : (Number(pnr.pax) || 0) * (Number(pnr.price_per_pax) || 0);
      const curDeposit = Number(pnr.deposit_total) || 0;
      const newDeposit = pnr_mode === 'lunas'
        ? (totalCost > 0 ? totalCost : curDeposit + amount)   // lunas: penuhi total
        : curDeposit + amount;                                 // deposit: tambah
      await supabase.from('flight_inventory').update({ deposit_total: newDeposit }).eq('id', linked_pnr_id);
      const ptrip = pnr.trip_id || trip_id;
      // Sinkron ke HPP hanya bila PNR sudah terkait trip (kalau belum, cukup catat deposit di PNR)
      if (ptrip) {
        try { await syncPnrToHPP(supabase, linked_pnr_id, ptrip); } catch (e) { /* best-effort */ }
        revalidatePath(`/finance/cashflow/${ptrip}`);
        revalidatePath(`/accounting/groups/${ptrip}`);
      }
      revalidatePath('/finance/pnr');
    }
  }

  // Revalidate SEMUA tampilan yang menghitung cashflow/akun supaya nominal
  // langsung terupdate (real cashflow per grup, posisi kas, finance), apa pun
  // jenis entri-nya (bukan hanya yang ter-link ke HPP).
  if (trip_id) {
  ...
  revalidatePath('/accounting/cash-position');
...
// ============ DELETE TRANSAKSI (multi-sumber) ============
// source: 'manual'|'auto' → accounting_entries ; 'payment' → participant_payments ; 'hpp' → trip_finance_items
export async function deleteAccountingTransaction(source, id) {
  const authClient = createClient();
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const _g = await assertStaff(user, '/invoices'); if (_g.error) return { error: _g.error }; }

  // batasi ke finance/manager/owner/accounting
  try {
    const { data: u } = await authClient.from('users').select('role').eq('id', user.id).maybeSingle();
    const role = u?.role || '';
    if (!['finance', 'manager', 'owner', 'accounting'].includes(role)) {
      return { error: 'Hanya Finance/Manager/Owner yang bisa menghapus transaksi.' };
    }
  } catch { /* kalau gagal cek role, tetap lanjut via RLS */ }

  const supabase = getServiceClient() || authClient;
  if (!id) return { error: 'ID kosong' };

  let table = 'accounting_entries';
  if (source === 'payment') table = 'participant_payments';
  else if (source === 'hpp') table = 'trip_finance_items';

  const { error } = await supabase.from(table).delete().eq('id', id);
  if (error) return { error: error.message };

  // kalau hapus pembayaran peserta → bersihkan cash-in terkait (best-effort)
  if (source === 'payment') { try { await autoDeleteCashInFromPayment(id); } catch {} }
