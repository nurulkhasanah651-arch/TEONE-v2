...
export async function saveFinalRoomlist(tripId, rooms) {
  ...
  // SINKRON BALIK ke master trip: room_assignment tiap peserta mengikuti Final Roomlist.
  // room_type HANYA di-update utk peserta paket KAMAR BIASA. Peserta paket KHUSUS
  // (Land Tour / Infant / Child no Bed) TIDAK diubah room_type-nya, supaya paket & harga
  // di invoice tidak berubah walau ditaruh di kamar fisik apa pun.
  const _memberIds = [];
  for (const r of rooms) for (const m of (r.members || [])) if (m.passenger_id) _memberIds.push(m.passenger_id);
  let _curByPax = {};
  if (_memberIds.length) {
    const { data: _cur } = await supabase
      .from('trip_passengers').select('id, room_type, age_type').in('id', _memberIds).eq('trip_id', tripId);
    _curByPax = Object.fromEntries((_cur || []).map((p) => [p.id, p]));
  }
  const _isPaketLocked = (p) => {
    const at = String(p?.age_type || '').toLowerCase();
    if (at === 'infant' || at === 'child_no_bed') return true;
    const k = roomTypeToKey(p?.room_type);
    return k === 'infant' || k === 'child_no_bed' || (typeof k === 'string' && k.startsWith('land_tour'));
  };
  for (const r of rooms) {
    const ids = (r.members || []).map((m) => m.passenger_id).filter(Boolean);
    if (ids.length === 0) continue;
    const assignment = `Room ${r.room_no}${r.label ? ` — ${r.label}` : ''}`;
    // room_assignment (posisi kamar fisik) utk SEMUA anggota kamar ini
    await supabase.from('trip_passengers').update({ room_assignment: assignment }).in('id', ids).eq('trip_id', tripId);
    // room_type (penentu paket & harga) HANYA utk peserta paket biasa
    const updIds = ids.filter((id) => !_isPaketLocked(_curByPax[id]));
    if (updIds.length) {
      await supabase.from('trip_passengers').update({ room_type: r.room_type || null }).in('id', updIds).eq('trip_id', tripId);
    }
  }
