...
// Round 130: TL Checklist + Final Report + Vendor Reviews actions
...
export async function toggleChecklistItem(tripId, field, value, userEmail = '') {
...
export async function saveChecklistNotes(tripId, notes) {
  ...
  try {
    ...
    if (existing) {
      ...
      await supabase.from('tl_checklist').insert({
        trip_id: tripId,
        pre_departure_notes: notes,
      });
    }
...
export async function saveFinalReport({
  ...
  try {
    ...
    const reportData = {
      ...
      suggestions,
      total_petty_cash_spent: totalPettyCashSpent ? Number(totalPettyCashSpent) : null,
      updated_at: new Date().toISOString(),
    };
