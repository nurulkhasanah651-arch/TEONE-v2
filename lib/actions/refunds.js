...
export async function createRefund({
  ...
  const { data: { user: _staff } } = await _auth.auth.getUser();
  const _guard = await assertStaff(_staff, '/invoices');
  if (_guard.error) return { error: _guard.error };
...
export async function approveRefund(refundId, userEmail = '') {
  ...
  try {
    ...
    const refundUpdate = {
      ...
      approved_by: _staff?.email || userEmail || 'system',
      processed_at: approvedAt,
      processed_by: _staff?.email || userEmail || 'system',
    };
...
export async function rejectRefund(refundId, rejectReason = '', userEmail = '') {
  ...
  try {
    ...
    const { error: updErr } = await supabase.from('refunds').update({
      ...
      rejected_by: _staff?.email || userEmail || 'system',
