...
export async function createInvoice(params) {
  ...
  if (allIn) {
    ...
    const { data: _trip } = await supabase.from('trips').select('price_breakdown').eq('id', trip_id).maybeSingle();
    ...
    for (const pid of ids) {
      ...
      const visaOwed = (px.include_visa && !px.visa_ready && !(pys || []).some((x) => String(x.type).toLowerCase() === 'visa')) ? (Number(bd.visa) || 0) : 0;
      const asrOwed = (px.include_asuransi && !(pys || []).some((x) => String(x.type).toLowerCase() === 'asuransi')) ? (Number(bd.asuransi) || 0) : 0;
