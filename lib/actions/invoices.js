...
// Round 100c: sendInvoiceWA branch by status
// - status === 'paid' → kirim pesan RECEIPT ("Pembayaran Sudah Diterima")
// - lainnya → kirim pesan INVOICE/TAGIHAN
// - Termasuk auto-include sisa pembayaran & next milestone
...
// Round 99/100: AUTO-SYNC invoice paid → participant_payments
// Round 100: support family invoice (covers_passenger_ids)
async function syncInvoiceToMatrix(supabase, inv, paidAmount) {
  ...
  const amountPerPax = pesertaIds.length > 1 ? Math.round(totalAmount / pesertaIds.length) : totalAmount;
...
export async function createInvoice(params) {
  ...
  if (error && /family_group_id|covers_passenger_ids|is_family_invoice/.test(error.message)) {
...
// ============================================================
// Round 100c: HELPER — bikin pesan WA berdasarkan status invoice
// ============================================================
async function buildWAMessage(supabase, inv) {
  ...
  // === RECEIPT MODE (sudah dibayar)
  if (inv.status === 'paid') {
    // Hitung sisa pembayaran trip untuk peserta ini
    let nextInfo = '';
...
// ============================================================
// SEND INVOICE/RECEIPT via WA Fonnte — Round 100c: branch by status
// ============================================================
export async function sendInvoiceWA(invoiceId) {
  ...
  // Update tracking (jangan rubah status paid → paid)
  await supabase
    .from('invoices')
    .update({
...
export async function approveInvoicePayment(paymentId) {
  ...
  // AUTO-SEND receipt WA via helper buildWAMessage (status paid)
...
export async function markInvoicePaidManual(invoiceId) {
  ...
  // AUTO-SEND receipt WA pakai helper buildWAMessage
  if (inv.customer_phone) {
    const updatedInv = { ...inv, status: 'paid', paid_at: new Date().toISOString() };
    const message = await buildWAMessage(supabase, updatedInv);
    await sendFonnte(normalizePhone(inv.customer_phone), message);
...
// ============================================================
// createInvoiceAsPaid — generate invoice yang langsung status PAID
// ============================================================
