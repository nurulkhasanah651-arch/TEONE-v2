...
import { paxRoomKey } from '@/lib/utils/price-breakdown';
...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  ...
  // POKOK = kamar + SEMUA biaya wajib (tiket domestik, bagasi domestik, tips, city tax, base).
  // INFANT: harga dasar saja, tanpa biaya wajib.
  const isInfant = (paxRes.data?.age_type === 'infant') || String(roomType).toLowerCase().includes('infant');
  const mainExpected = isInfant ? roomPrice : (roomPrice + tips + cityTax + flight + baggage + baseFee);
  ...
  return { expectedTotal, totalPaid, pokokPaid, addonPaid, sisa, discount, mainExpected, optExpected, nextMilestone, roomPrice, tips: isInfant ? 0 : tips, cityTax: isInfant ? 0 : cityTax, flight: isInfant ? 0 : flight, baggage: isInfant ? 0 : baggage, baseFee: isInfant ? 0 : baseFee, roomType };
}
...
async function buildWAMessage(supabase, inv) {
  ...
  if (inv.status === 'paid') {
    ...
    if (inv.trip_id && inv.passenger_id) {
      const { expectedTotal, totalPaid, pokokPaid, addonPaid, sisa, nextMilestone, roomPrice, tips, cityTax, flight, baggage, baseFee, discount, roomType } = await getExpectedAndPaidForPassenger(supabase, inv.trip_id, inv.passenger_id);
      const paketShown = Math.max((Number(expectedTotal)||0) + (Number(discount)||0) - (Number(tips)||0) - (Number(cityTax)||0) - (Number(flight)||0) - (Number(baggage)||0) - (Number(baseFee)||0), 0);
...
export async function createManualInvoice(formData) {
  ...
  const { data: manRows } = await supabase.from('invoices').select('invoice_no').is('trip_id', null).like('invoice_no', 'TEONE-INV-%');
  let maxMan = 0;
  for (const r of (manRows || [])) {
    const m = String(r.invoice_no || '').match(/(\d+)\s*$/);
    if (m) { const n = parseInt(m[1], 10); if (Number.isFinite(n) && n > maxMan) maxMan = n; }
  }
  let invoice_no = `TEONE-INV-${String(maxMan + 1).padStart(4, '0')}`;
  const token = genToken();
  ...
  while (error && /invoice_no|duplicate key|23505/i.test(error.message || '') && mTries < 5) {
    mTries++;
    payload.invoice_no = `TEONE-INV-${String(maxMan + 1 + mTries).padStart(4, '0')}`;
