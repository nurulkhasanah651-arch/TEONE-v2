...
${companyName}`;
  ...
  if (inv.trip_id && inv.passenger_id) {
    const { expectedTotal, totalPaid, pokokPaid, addonPaid, sisa, roomPrice, tips, cityTax, roomType } = await getExpectedAndPaidForPassenger(supabase, inv.trip_id, inv.passenger_id);
...
${tips > 0 ? `🍽 Tips: ${fmtRupiah(tips)}\n` : ''}${cityTax > 0 ? `🏛 City Tax: ${fmtRupiah(cityTax)}\n` : ''}
💰 Total Paket (Pokok): *${fmtRupiah(expectedTotal)}*
${pokokPaid > 0 ? `✅ Dibayar (pokok): *${fmtRupiah(pokokPaid)}*\n⚠ Sisa Pokok: *${fmtRupiah(sisa)}*` : ''}${addonPaid > 0 ? `\n➕ Pembayaran lain (visa/ongkir/optional): *${fmtRupiah(addonPaid)}*` : ''}`;
    }
  }
