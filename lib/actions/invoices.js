...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  const [tripRes, paxRes, paysRes] = await Promise.all([
    ...
    supabase.from('trip_passengers').select('room_type, age_type, discount_amount, price_paid, price_detail, visa_type, include_visa, include_asuransi, visa_ready, family_group_id, joined_at').eq('id', passenger_id).maybeSingle(),
  ...
  try {
    if (roomPrice <= 0) console.log('[INVDBG]', JSON.stringify({ trip_id: String(trip_id), passenger_id: String(passenger_id), roomType, roomKey, roomPrice, bdDouble: breakdown?.double, bdQuad: breakdown?.quad, bdKeys: Object.keys(breakdown || {}).slice(0, 30) }));
  } catch {}
  ...
  return { expectedTotal, totalPaid, pokokPaid, addonPaid, addonItems, sisa, discount, mainExpected, optExpected, nextMilestone, roomPrice, tips: effTips, cityTax: effCity, flight: effFlight, baggage: effBaggage, baseFee: effBase, perlengkapan: effPerlengkapan, roomType, visaExpected, asuransiExpected,
