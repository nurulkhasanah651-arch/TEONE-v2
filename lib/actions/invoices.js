...
export async function createManualInvoice(formData) {
  ...
  // Nomor invoice manual (trip_id null)
  const { count } = await supabase.from('invoices').select('id', { count: 'exact', head: true }).is('trip_id', null);
  const invoice_no = `TEONE-INV-${String((count || 0) + 1).padStart(4, '0')}`;
  const token = genToken();
  ...
  const { data, error } = await supabase.from('invoices').insert(payload).select('id, invoice_no, public_token').single();
  if (error) return { error: error.message };
