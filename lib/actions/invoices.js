...
async function sendFonnte(phone, message) {
  const { token } = getFonnteToken('finance', getBrandCodeSafe());
...
async function buildWAMessage(supabase, inv) {
  const baseUrl = siteUrlFor(getBrandCodeSafe());
  ...
  if (inv.status === 'paid') {
    ...
    if (inv.trip_id && inv.passenger_id) {
      const { expectedTotal, totalPaid, pokokPaid, addonPaid, sisa, nextMilestone, roomPrice, tips, cityTax, roomType } = await getExpectedAndPaidForPassenger(supabase, inv.trip_id, inv.passenger_id);
...
${companyName}`;
  ...
  if (inv.trip_id && inv.passenger_id) {
    ...
    const memberIds = (inv.is_family_invoice && Array.isArray(inv.covers_passenger_ids) && inv.covers_passenger_ids.length)
      ...
      : [inv.passenger_id];
    let roomSum = 0, tipsSum = 0, citySum = 0, expSum = 0, pokokPaidSum = 0, sisaSum = 0, addonSum = 0, firstRoom = '';
...
export async function approveInvoicePayment(paymentId) {
  ...
  if (phoneForWA) {
    ...
    await sendFonnte(normalizePhone(phoneForWA), message);
  }
