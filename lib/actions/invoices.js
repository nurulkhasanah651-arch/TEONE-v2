...
import { paxRoomKey, mainExpectedPerPassenger, isPokokMilestone } from '@/lib/utils/price-breakdown';
...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  const [tripRes, paxRes, paysRes] = await Promise.all([
    ...
    supabase.from('trip_passengers').select('room_type, age_type, discount_amount, price_paid').eq('id', passenger_id).maybeSingle(),
  ...
  // LAND TOUR: tanpa tiket/bagasi domestik & base (tetap tips + city tax)
  const isLandTour = String(roomKey || '').startsWith('land_tour') || String(roomType).toLowerCase().includes('land');
  const effTips = (isInfant || isChildNoBed) ? 0 : tips;
  const effCity = (isInfant || isChildNoBed) ? 0 : cityTax;
  const effFlight = (isInfant || isLandTour) ? 0 : flight;
  const effBaggage = (isInfant || isLandTour) ? 0 : baggage;
  const effBase = (isInfant || isLandTour) ? 0 : baseFee;
  const mainExpected = roomPrice + effTips + effCity + effFlight + effBaggage + effBase;

  const visaExpected = paidTypes.has('Visa') ? Number(breakdown.visa || 0) : 0;
  const asuransiExpected = paidTypes.has('Asuransi') ? Number(breakdown.asuransi || 0) : 0;
  let optExpected = visaExpected + asuransiExpected;

  // POKOK trip = paket utama (room + tips + city tax). Visa/asuransi/ongkir/optional = ADDON terpisah.
  ...
  return { expectedTotal, totalPaid, pokokPaid, addonPaid, sisa, discount, mainExpected, optExpected, nextMilestone, roomPrice, tips: effTips, cityTax: effCity, flight: effFlight, baggage: effBaggage, baseFee: effBase, roomType, visaExpected, asuransiExpected };
}
...
export async function createInvoice(params) {
  let {
    ...
    passenger_amounts = null, allIn = false,
  } = params;
  ...
  if (allIn) {
    ...
    for (const pid of ids) {
      const { data: px } = await supabase.from('trip_passengers').select('room_type, age_type, price_paid, discount_amount, include_visa, include_asuransi, visa_ready').eq('id', pid).maybeSingle();
      if (!px) continue;
      ...
      const visaOwed = (px.include_visa && !px.visa_ready && !(pys || []).some((x) => String(x.type).toLowerCase() === 'visa')) ? _vp : 0;
      const asrOwed = (px.include_asuransi && !(pys || []).some((x) => String(x.type).toLowerCase() === 'asuransi')) ? _ap : 0;
      total += Math.max(pokokExp - pokokPaid, 0) + visaOwed + asrOwed;
    }
    amount = total;
    milestone = 'Pelunasan + Visa + Asuransi';
...
async function buildWAMessage(supabase, inv) {
  ...
  let financeName = (_bcode === 'khasanah' ? process.env.WA_FINANCE_NAME_KHASANAH : process.env.WA_FINANCE_NAME) || 'Putri';
  if (_bcode === 'khasanah') { const _pn = await getPicNameForTrip(supabase, inv.trip_id); if (_pn) financeName = _pn; }
  const _isDP = /dp/i.test(String(inv.milestone || ''));
  const greet = (nm) => {
    if (_bcode === 'khasanah') {
      return `Assalamu'alaikum ${nm || 'Bapak/Ibu'} \ud83d\ude4f` + (_isDP ? `\nPerkenalkan, saya ${financeName} dari ${companyName}.` : '');
    }
    return `Halo ${nm || 'Bapak/Ibu'}, perkenalkan saya ${financeName} dari tim Finance ${companyName}.`;
  };
  ...
  if (inv.status === 'paid') {
    ...
    if (inv.trip_id && inv.passenger_id) {
      ...
      const memberIds = (inv.is_family_invoice && Array.isArray(inv.covers_passenger_ids) && inv.covers_passenger_ids.length)
        ...
        : [inv.passenger_id];
      let expectedTotal = 0, pokokPaid = 0, addonPaid = 0, tips = 0, cityTax = 0, flight = 0, baggage = 0, baseFee = 0, discount = 0, firstRoom = '';
      ...
      const paketShown = Math.max(expectedTotal + discount - tips - cityTax - flight - baggage - baseFee, 0);
...
💼 Paket Tour${roomType ? ` (${roomType})` : ''}: ${fmtRupiah(paketShown)}
...
${nextMilestone.type}: ${fmtRupiah(nextMilestone.amount)}`;
    ...
    return `${greet(inv.customer_name)}
...
${companyName}`;
  ...
  if (inv.trip_id && inv.passenger_id) {
    ...
    const memberIds = (inv.is_family_invoice && Array.isArray(inv.covers_passenger_ids) && inv.covers_passenger_ids.length)
      ? inv.covers_passenger_ids
      : [inv.passenger_id];
    let roomSum = 0, tipsSum = 0, citySum = 0, expSum = 0, pokokPaidSum = 0, sisaSum = 0, addonSum = 0, discSum = 0, flightSum = 0, baggageSum = 0, baseSum = 0, firstRoom = '';
    for (const pid of memberIds) {
      const s = await getExpectedAndPaidForPassenger(supabase, inv.trip_id, pid);
    ...
    if (expSum > 0) {
      ...
      const paketSum = Math.max(expSum + discSum - tipsSum - citySum - flightSum - baggageSum - baseSum, 0);
...
💼 Paket${lbl}: ${fmtRupiah(paketSum)}
${tipsSum > 0 ? `🍽 Tips: ${fmtRupiah(tipsSum)}\n` : ''}${citySum > 0 ? `🏛 City Tax: ${fmtRupiah(citySum)}\n` : ''}
