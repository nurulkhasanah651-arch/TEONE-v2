...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  const [tripRes, paxRes, paysRes] = await Promise.all([
    ...
    supabase.from('trip_passengers').select('room_type, age_type, discount_amount, price_paid, visa_type').eq('id', passenger_id).maybeSingle(),
  ...
  const visaExpected = paidTypes.has('Visa') ? visaPriceFor(breakdown, paxRes.data?.visa_type) : 0;
