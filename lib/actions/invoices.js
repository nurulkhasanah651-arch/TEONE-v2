...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  ...
  // PPN yang SUDAH DIBAYAR (proporsional dari pokok terbayar) — utk cash-in/accounting.
  const ppnPaid = (ppn > 0 && expectedTotal > 0) ? Math.round(ppn * Math.min(Math.max(pokokPaid / expectedTotal, 0), 1)) : 0;
