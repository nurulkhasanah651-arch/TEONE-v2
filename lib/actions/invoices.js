...
import { STD_PAYMENT_TYPES, isAddonPayment } from '@/lib/utils/payment-types';
...
async function resolveCustomerPhone(supabase, inv) {
  // R236: MASTER customer adalah sumber kebenaran nomor HP.
  // Snapshot invoice.customer_phone hanya cadangan bila master tak ketemu/kosong.
  // Sebelumnya snapshot diprioritaskan -> saat nomor diubah di Master Trip,
  // finance & WA tetap pakai nomor lama (mis. Diah Ayu 231). Karena sendInvoiceWA
  // menulis ulang snapshot dari hasil fungsi ini, snapshot ikut sembuh tiap kirim.

  // 1. Fresh lookup dari customers table via customer_id
  if (inv.customer_id) {
    const { data: cust } = await supabase
      ...
      .maybeSingle();
    const fresh = cust ? (cust.phone || cust.whatsapp || null) : null;
    if (fresh && String(fresh).trim()) return fresh;
  }

  // 2. Fallback: lookup via passenger → customer (master juga)
  if (inv.passenger_id) {
    const { data: pax } = await supabase
      ...
      .maybeSingle();
    if (pax?.customers) {
      const fresh2 = pax.customers.phone || pax.customers.whatsapp || null;
      if (fresh2 && String(fresh2).trim()) return fresh2;
    }
  }

  // 3. Last resort: snapshot lama di invoice (hanya kalau master benar-benar kosong)
...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  const [tripRes, paxRes, paysRes] = await Promise.all([
    supabase.from('trips').select('price_breakdown, payment_template, name, departure').eq('id', trip_id).maybeSingle(),
    supabase.from('trip_passengers').select('room_type, age_type, discount_amount, price_paid, price_detail, visa_type, include_visa, include_asuransi, include_evisa, visa_ready, family_group_id, joined_at').eq('id', passenger_id).maybeSingle(),
    supabase.from('participant_payments').select('type, amount, is_addon').eq('passenger_id', passenger_id),
  ]);
  ...
  // Anggota keluarga yang tipe kamarnya belum diisi ikut tipe kamar KEPALA keluarga.
  // Tanpa ini harga kamarnya terhitung Rp 0 dan invoice keluarga jadi kurang.
  let roomType = paxRes.data?.room_type || '';
  if (!roomType && paxRes.data?.family_group_id) {
    try {
      const { data: fg } = await supabase.from('family_groups').select('head_passenger_id').eq('id', paxRes.data.family_group_id).maybeSingle();
      if (fg?.head_passenger_id && fg.head_passenger_id !== passenger_id) {
        const { data: head } = await supabase.from('trip_passengers').select('room_type').eq('id', fg.head_passenger_id).maybeSingle();
        roomType = head?.room_type || '';
      }
    } catch {}
  }
  ...
  // KHASANAH: visa & asuransi juga biaya WAJIB → masuk pokok, bukan addon opsional.
  // Infant bebas; peserta 'visa ready' bebas biaya visa (asuransi tetap kena).
  const _isKh = getBrandCodeSafe() === 'khasanah';
  // _harga_final: price_paid = tagihan all-in per peserta (landtour+tiket+visa+addon sudah termasuk).
  // Semua pembayaran (DP/Visa/Tiket dll) dihitung sbg BAYAR tagihan all-in, tanpa tagihan terpisah.
  const _hf = !!(breakdown && breakdown._harga_final === true);
  const _visaPrice = visaPriceFor(breakdown, paxRes.data?.visa_type);
  const _asrPrice = Number(breakdown.asuransi || 0);
  const _evisaPrice = Number(breakdown.visa_evisa || 0);
  const effVisaPokok = (_isKh && !isInfant && !paxRes.data?.visa_ready) ? _visaPrice : 0;
  const effAsuransiPokok = (_isKh && !isInfant) ? _asrPrice : 0;

  // KHASANAH: 3 item wajib gabungan baru (Asuransi & Tips Local Guide, Handling &
  // Perlengkapan, Visa & Asuransi). Wajib POKOK kalau diisi. Infant & Child-no-bed BEBAS.
  const _khCombo = (k) => (_isKh && !isInfant && !isChildNoBed) ? (Number(breakdown[k]) || 0) : 0;
  const effAsrTipsLocal = _khCombo('asuransi_tips_local_guide');
  const effHandlingPerlengkapan = _khCombo('handling_perlengkapan');
  const effVisaAsuransi = _khCombo('visa_asuransi');

  const mainExpected = roomPrice + effTips + effCity + effFlight + effBaggage + effBase + effPerlengkapan
    + effVisaPokok + effAsuransiPokok + effAsrTipsLocal + effHandlingPerlengkapan + effVisaAsuransi;
  ...
  // E-Visa = include TERPISAH (independen dari visa biasa). Canada: bisa Visa biasa + E-Visa sekaligus.
  const _incEvisa = !!paxRes.data?.include_evisa;
  const visaExpected = (_isKh || _hf) ? 0 : ((_incVisa || paidTypes.has('Visa')) ? _visaPrice : 0);
  const asuransiExpected = (_isKh || _hf) ? 0 : ((_incAsr || paidTypes.has('Asuransi')) ? _asrPrice : 0);
  const evisaExpected = (_isKh || _hf) ? 0 : ((_incEvisa || paidTypes.has('E-Visa')) ? _evisaPrice : 0);
  let optExpected = visaExpected + asuransiExpected + evisaExpected;
  ...
  if (_isKh) milestonesPokok.push('Visa', 'Asuransi'); // wajib → pembayarannya mengurangi sisa pokok
  // Tipe bebas ada 2 macam: cicilan berlabel bebas ("Payment ke 2 & 3") -> POKOK,
  // dan biaya tambahan ("Jadwal Ulang") -> ADDON (ditagihkan terpisah, lihat addonItems).
  const _isCicilanBebas = (p) => !STD_PAYMENT_TYPES.includes(p.type) && isAddonPayment(p) === false;
  // _harga_final: SEMUA pembayaran (DP/Visa/Tiket/dll) = bayar tagihan all-in → masuk pokokPaid.
  const pokokPaid = _hf ? totalPaid : pays
    .filter((p) => milestonesPokok.includes(p.type) || _isCicilanBebas(p))
    .reduce((s, p) => s + Number(p.amount || 0), 0);
  const addonPaid = _hf ? 0 : Math.max(totalPaid - pokokPaid, 0);
  // Biaya tambahan: dikelompokkan per jenis supaya bisa MUNCUL sbg baris tagihan di invoice.
  // Tanpa ini, bayarannya mengurangi sisa tapi biayanya tak pernah ditagih (bocor).
  // _harga_final: tidak ada biaya tambahan terpisah (semua sudah di dalam harga all-in).
  const addonItems = _hf ? [] : (() => {
    const m = {};
    for (const p of pays) {
      if (milestonesPokok.includes(p.type) || _isCicilanBebas(p)) continue;
      if (STD_PAYMENT_TYPES.includes(p.type)) continue;   // Visa/Asuransi opt-in → sudah jadi optItems
      if (!isAddonPayment(p)) continue;
      m[p.type] = (m[p.type] || 0) + Number(p.amount || 0);
    }
  ...
  // KHASANAH: harga SELALU dari Master Trip (kamar + biaya wajib) → price_paid DIABAIKAN.
  // Nego/potongan lewat kolom Diskon (discount_amount), bukan lewat price_paid. Ini menghapus
  // "Penyesuaian harga khusus" akibat price_paid yg beda dari master. TEONE tetap perilaku lama.
  // GUARD: kalau harga KAMAR di master belum diisi (roomPrice 0) sedangkan price_paid lebih besar
  // dari biaya wajib, pakai price_paid supaya harga kamar TIDAK HILANG dari invoice (cegah
  // undercharge). Idealnya harga kamar diisi di Master Trip agar tampil rapi tanpa penyesuaian.
  const _khRoomMissing = _isKh && roomPrice <= 0 && Number(paxRes.data?.price_paid || 0) > mainExpected;
  const pokokGross = _isKh
    ? (_khRoomMissing ? Number(paxRes.data.price_paid) : mainExpected)
    : (Number(paxRes.data?.price_paid || 0) > 0 ? Number(paxRes.data.price_paid) : mainExpected);

  // PPN 1,1% — trip BERANGKAT mulai Okt 2026. Ditambahkan ke tagihan (Total & Sisa).
  ...
  // PPN (per 2 Agu 2026): Khasanah DIBATALKAN → 0. TEONE hanya PENDAFTAR BARU (joined_at >= 2 Agu 2026).
  // Peserta lama otomatis tanpa PPN.
  let ppn = 0, ppnBase = 0, ppnLabel = '';
  try {
    if (!_isKh && roomPrice > 0) {
      const _p = tourPpnReg(roomPrice, 'teone', paxRes.data?.joined_at);
      if (_p > 0) { ppn = _p; ppnBase = roomPrice; ppnLabel = ''; } // TEONE: label kosong → "PPN 1,1% (Paket Tour)"
    }
  } catch {}

  const expectedTotal = Math.max(pokokGross - discount, 0) + ppn;  // pokok − diskon + PPN tour
  const sisa = Math.max(expectedTotal - pokokPaid, 0);             // sisa termasuk PPN
  // PPN dianggap dibayar PALING AKHIR: pembayaran menutup pokok dulu, kelebihannya baru PPN.
  // Peserta yang baru lunas pokok (belum nambah bayar PPN) → ppnPaid = 0.
  const _pokokNet = Math.max(pokokGross - discount, 0);
  const ppnPaid = ppn > 0 ? Math.min(Math.max(pokokPaid - _pokokNet, 0), ppn) : 0;
  ...
  return { expectedTotal, totalPaid, pokokPaid, addonPaid, addonItems, sisa, discount, mainExpected, optExpected, nextMilestone, roomPrice, tips: effTips, cityTax: effCity, flight: effFlight, baggage: effBaggage, baseFee: effBase, perlengkapan: effPerlengkapan, roomType, visaExpected, asuransiExpected, evisaExpected,
    visaPokok: effVisaPokok, asuransiPokok: effAsuransiPokok,
    asuransiTipsLocalGuide: effAsrTipsLocal, handlingPerlengkapan: effHandlingPerlengkapan, visaAsuransi: effVisaAsuransi,
    hargaDetail: (paxRes.data?.price_detail && typeof paxRes.data.price_detail === 'object') ? paxRes.data.price_detail : null,
    ppn, ppnBase, ppnLabel, ppnPaid };
}
...
export async function createInvoice(params) {
  ...
  if (allIn) {
    ...
    const _evp = Number(_tpl.visa_evisa ?? _tpl.Visa_Evisa) || Number(bd.visa_evisa) || 0;
    // Khasanah: visa & asuransi WAJIB → sudah termasuk pokok, jangan ditambah lagi.
    const _khBrand = getBrandCodeSafe() === 'khasanah' ? 'khasanah' : '';
    let total = 0;
    for (const pid of ids) {
      const { data: px } = await supabase.from('trip_passengers').select('room_type, age_type, price_paid, discount_amount, include_visa, include_asuransi, include_evisa, visa_ready, visa_type').eq('id', pid).maybeSingle();
      if (!px) continue;
      const { data: pys } = await supabase.from('participant_payments').select('type, amount').eq('passenger_id', pid);
      const pokokExp = Math.max(mainExpectedPerPassenger(px, bd, _khBrand) - (Number(px.discount_amount) || 0), 0);
      const pokokPaid = (pys || []).filter((x) => isPokokMilestone(x.type)
        || (_khBrand && ['visa', 'asuransi'].includes(String(x.type).toLowerCase()))).reduce((a, x) => a + (Number(x.amount) || 0), 0);
      const visaOwed = _khBrand ? 0 : ((px.include_visa && !px.visa_ready && !(pys || []).some((x) => String(x.type).toLowerCase() === 'visa')) ? visaPriceTpl(_tpl, bd, px.visa_type) : 0);
      const asrOwed = _khBrand ? 0 : ((!skipAsuransi && px.include_asuransi && !(pys || []).some((x) => String(x.type).toLowerCase() === 'asuransi')) ? _ap : 0);
      const evisaOwed = _khBrand ? 0 : ((px.include_evisa && !(pys || []).some((x) => String(x.type).toLowerCase() === 'e-visa')) ? _evp : 0);
      total += Math.max(pokokExp - pokokPaid, 0) + visaOwed + asrOwed + evisaOwed;
    }
    amount = total;
    milestone = _khBrand ? 'Pelunasan' : (skipAsuransi ? 'Pelunasan + Visa' : 'Pelunasan + Visa + Asuransi');
...
async function buildWAMessage(supabase, inv) {
  ...
  if (inv.status === 'paid') {
    ...
    if (inv.trip_id && inv.passenger_id) {
      ...
      let visaPokokSum = 0, asuransiPokokSum = 0;
      let asrTipsLocalSum = 0, handlingPerlSum = 0, visaAsrSum = 0;
      ...
      for (const pid of memberIds) {
        ...
        visaPokokSum += Number(s.visaPokok) || 0;
        asuransiPokokSum += Number(s.asuransiPokok) || 0;
        asrTipsLocalSum += Number(s.asuransiTipsLocalGuide) || 0;
        handlingPerlSum += Number(s.handlingPerlengkapan) || 0;
        visaAsrSum += Number(s.visaAsuransi) || 0;
      ...
      const paketShown = Math.max(expectedTotal + discount - tips - cityTax - flight - baggage - baseFee - perlengkapan - visaPokokSum - asuransiPokokSum - asrTipsLocalSum - handlingPerlSum - visaAsrSum, 0);
...
${companyName}`);
  ...
  if (inv.trip_id && inv.passenger_id) {
    ...
    let visaPokokSum2 = 0, asuransiPokokSum2 = 0;
    let asrTipsLocalSum2 = 0, handlingPerlSum2 = 0, visaAsrSum2 = 0;
    for (const pid of memberIds) {
      const s = await getExpectedAndPaidForPassenger(supabase, inv.trip_id, pid);
      ...
      visaPokokSum2 += Number(s.visaPokok) || 0;
      asuransiPokokSum2 += Number(s.asuransiPokok) || 0;
      asrTipsLocalSum2 += Number(s.asuransiTipsLocalGuide) || 0;
      handlingPerlSum2 += Number(s.handlingPerlengkapan) || 0;
      visaAsrSum2 += Number(s.visaAsuransi) || 0;
      if (!firstRoom) firstRoom = s.roomType || '';
    }
    if (expSum > 0) {
      const lbl = memberIds.length > 1 ? ` (${memberIds.length} pax)` : (firstRoom ? ` (${firstRoom})` : '');
      const paketSum = Math.max(expSum + discSum - tipsSum - citySum - flightSum - baggageSum - baseSum - perlengkapanSum - visaPokokSum2 - asuransiPokokSum2 - asrTipsLocalSum2 - handlingPerlSum2 - visaAsrSum2, 0);
...
export async function sendInvoiceWA(invoiceId, opts = {}) {
  ...
  // Invoice sudah LUNAS/PAID -> ini "tanda terima" (pembayaran diterima), bukan penagihan.
  // Kirim template KONFIRMASI (via sendPaymentReceivedWA), bukan template invoice.
  if (inv.status === 'paid' && inv.passenger_id && !opts.forceInvoiceTemplate) {
    try {
      const _rcpt = await sendPaymentReceivedWA(inv.passenger_id);
      if (_rcpt && !_rcpt.error) return _rcpt;
    } catch {}
  }
  ...
  // Param template invoice_khasanah: {{1}}nama {{2}}pic {{3}}trip {{4}}noInvoice {{5}}tagihan {{6}}link
  let _picNameInv = '';
  try { _picNameInv = await getPicNameForTrip(supabase, inv.trip_id); } catch {}
  const _tripLabelInv = `${inv.trip_kode || ''} ${inv.trip_name || ''}`.trim();
  const _invLink = `${customerSiteUrlFor(getBrandCodeSafe())}/invoice/${inv.public_token}`;
  const invoiceParams = [(inv.customer_name ? 'Kak ' + inv.customer_name : 'Kak'), _picNameInv || '-', _tripLabelInv || '-', inv.invoice_no || '-', inv.milestone || '-', _invLink || '-'];
  ...
  if (!opts.forceSend && await isPicWaManualForTrip(supabase, inv.trip_id)) {
    // WABA: template invoice dulu (bisa ke peserta belum chat), lalu teks (24h), lalu manual.
    const _tplInv = process.env.WABA_TPL_INVOICE_KHASANAH || 'invoice_khasanah';
    let _waba = await trySendWabaTemplateForTrip(supabase, inv.trip_id, phone, _tplInv, invoiceParams, { kind: 'waba_invoice' });
    if (!_waba?.ok) { const t = await trySendWabaForTrip(supabase, inv.trip_id, phone, message); if (t?.ok) _waba = t; }
    if (_waba?.ok) {
      await supabase.from('invoices')
        .update({ status: inv.status === 'paid' ? 'paid' : 'sent', sent_at: new Date().toISOString(), sent_via: 'waba' })
        .eq('id', invoiceId);
      revalidateAll(inv?.trip_id);
      return { ok: true, via: 'waba', wa_phone: phone };
    }
    if (opts.queueManual) {
      try {
  ...
  // WABA (Api.co.id) utk PIC Khasanah yg punya nomor WABA — coba SEBELUM Fonnte.
  // No-op utk TEONE / PIC tanpa nomor WABA (apicoidConfig/getPicApicoidPhone -> null),
  // jadi alur lama (Fonnte) tak berubah utk mereka.
  {
    const _tplInv2 = process.env.WABA_TPL_INVOICE_KHASANAH || 'invoice_khasanah';
    let _waba2 = await trySendWabaTemplateForTrip(supabase, inv.trip_id, phone, _tplInv2, invoiceParams, { kind: 'waba_invoice' });
    if (!_waba2?.ok) { const t2 = await trySendWabaForTrip(supabase, inv.trip_id, phone, message); if (t2?.ok) _waba2 = t2; }
    if (_waba2?.ok) {
      await supabase.from('invoices')
        .update({ status: inv.status === 'paid' ? 'paid' : 'sent', sent_at: new Date().toISOString(), sent_via: 'waba' })
        .eq('id', invoiceId);
  ...
  // Template LENGKAP (detail + link invoice) bila pembayaran terkait peserta trip.
  {
    ...
    if (_pid) {
      ...
      if (await isPicWaManualForTrip(supabase, inv.trip_id)) {
        const _waba = await tryWabaApproval(_pid);
        if (_waba?.ok) { revalidateAll(inv?.trip_id); return { ok: true, via: 'waba' }; }
        let tpl = null;
