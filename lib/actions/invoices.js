...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  ...
  const expectedTotal = mainExpected;                       // tagihan POKOK
  const sisa = Math.max(expectedTotal - pokokPaid, 0);      // sisa POKOK (addon tidak mengurangi)
  ...
  return { expectedTotal, totalPaid, pokokPaid, addonPaid, sisa, mainExpected, optExpected, nextMilestone, roomPrice, tips, cityTax, roomType };
}
