...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  const [tripRes, paxRes, paysRes] = await Promise.all([
    ...
    supabase.from('trip_passengers').select('room_type, age_type, discount_amount, price_paid, visa_type, include_visa, include_asuransi, visa_ready, family_group_id, joined_at').eq('id', passenger_id).maybeSingle(),
