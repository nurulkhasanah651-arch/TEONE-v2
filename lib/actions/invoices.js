...
// Round 102e: Fix sisa calc — hitung dari price_breakdown + participant_payments
// (sebelumnya hitung dari invoices table → kalau cuma 1 invoice salah jadi LUNAS)
...
// ============================================================
// ROUND 102e: HELPER — Hitung expected & paid & sisa per peserta
// ============================================================
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  ...
  // Room base price — coba beberapa key variations
  const roomKey = String(roomType).toLowerCase().replace(/[^a-z_]/g, '');
  const roomPrice = Number(
    breakdown[roomKey] ||
    breakdown[roomType] ||
    breakdown[roomType?.toLowerCase()] ||
    0
  );
  ...
  // Opt-in (visa, asuransi) — masuk expected kalau peserta sudah ✓ di matrix
  let optExpected = 0;
  ...
  // Next milestone unpaid (from template)
  const milestones = ['DP', 'P1', 'P2', 'P3', 'Pelunasan'];
  ...
  return { expectedTotal, totalPaid, sisa, mainExpected, optExpected, nextMilestone };
}
...
// ============================================================
// ROUND 102e: buildWAMessage — Pakai expected dari breakdown (FIX!)
// ============================================================
async function buildWAMessage(supabase, inv) {
  ...
  // Breakdown per pax kalau family + custom amounts
  let breakdownText = '';
  ...
  if (inv.status === 'paid') {
    // ROUND 102e FIX: Hitung sisa dari breakdown + participant_payments (not invoices)
    let nextInfo = '';
    if (inv.trip_id && inv.passenger_id) {
      const { expectedTotal, totalPaid, sisa, nextMilestone } = await getExpectedAndPaidForPassenger(supabase, inv.trip_id, inv.passenger_id);
