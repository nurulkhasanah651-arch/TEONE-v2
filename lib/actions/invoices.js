...
async function buildWAMessage(supabase, inv) {
  ...
  if (inv.status === 'paid') {
    ...
    if (inv.trip_id && inv.passenger_id) {
      ...
      const paketShown = Math.max(expectedTotal + discount - tips - cityTax - flight - baggage - baseFee - perlengkapan - visaPokokSum - asuransiPokokSum, 0);
...
${companyName}`);
  ...
  if (inv.trip_id && inv.passenger_id) {
    ...
    if (expSum > 0) {
      ...
      const paketSum = Math.max(expSum + discSum - tipsSum - citySum - flightSum - baggageSum - baseSum - perlengkapanSum - visaPokokSum2 - asuransiPokokSum2, 0);
