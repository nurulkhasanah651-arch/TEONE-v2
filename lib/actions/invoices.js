...
export async function getExpectedAndPaidForPassenger(supabase, trip_id, passenger_id) {
  const [tripRes, paxRes, paysRes] = await Promise.all([
    ...
    supabase.from('trip_passengers').select('room_type, age_type, discount_amount, price_paid, visa_type, include_visa, include_asuransi, visa_ready, family_group_id').eq('id', passenger_id).maybeSingle(),
  ...
  try {
    if (_isKh) {
      if (!isInfant) {
        ...
        const _addon = detectTourAddon(tripRes.data?.name, _tpl);
        if (_addon && _addon.amount > 0) {
          const _p = tourPpn(_addon.amount, tripRes.data?.departure);
          if (_p > 0) { ppn = _p; ppnBase = _addon.amount; ppnLabel = _addon.label; }
        }
      }
    } else if (roomPrice > 0) {
      const _p = tourPpn(roomPrice, tripRes.data?.departure);
