...
// Round 116: Transfer peserta — payment di-MARK transferred (bukan di-copy double)
// Trip lama: payment ke-tag is_transferred=true → di-exclude dari cashflow
// Trip baru: payment baru insert dengan reference ke payment lama
...
export async function transferPassenger({
  ...
  try {
    ...
    for (const oldP of passengersToTransfer) {
      ...
      // ROUND 116: Copy payments + MARK old as transferred (jangan double count)
      try {
        const { data: payments } = await supabase
          .from('participant_payments').select('*')
          .eq('passenger_id', oldP.id)
          .eq('is_transferred', false);

        if (payments && payments.length > 0) {
          // Insert new payments di trip baru
          ...
          // Mark OLD payments sebagai transferred (gak di-count lagi di trip lama)
          if (insertedPayments && insertedPayments.length === payments.length) {
...
export async function undoTransfer(passengerId) {
  ...
  try {
    ...
    if (newPaxId) {
      // Restore is_transferred=false di old payments
