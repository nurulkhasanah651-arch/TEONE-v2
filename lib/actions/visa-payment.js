...
export async function getVisaApplyList() {
  ...
  // PENTING: paginasi via .range() — PostgREST batasi 1000 baris/response. Tanpa ini,
  // peserta di atas baris ke-1000 hilang (mis. 77 trip = 1700+ pax) & tak muncul di Payment Visa.
  let pax = [];
  for (let i = 0; i < tripIds.length; i += 100) {
    const chunk = tripIds.slice(i, i + 100);
    for (let from = 0; ; from += 1000) {
      const { data } = await db.from('trip_passengers')
        .select('id, trip_id, customer_id, include_visa, include_asuransi, visa_ready, visa_result, visa_docs, visa_biometric_date, visa_type, transfer_status, refund_status')
        .in('trip_id', chunk)
  ...
  const payByPax = {}; const asrByPax = {};
  for (let i = 0; i < allIds.length; i += 500) {
    const { data: pays } = await db.from('participant_payments').select('passenger_id, amount, is_transferred, type').in('type', ['Visa', 'Asuransi']).in('passenger_id', allIds.slice(i, i + 500));
    for (const r of (pays || [])) {
      if (r.is_transferred === true || !(Number(r.amount) > 0)) continue;
      if (r.type === 'Visa') payByPax[r.passenger_id] = true;
      else if (r.type === 'Asuransi') asrByPax[r.passenger_id] = true;
    }
  }
  // Muncul di Payment Visa = butuh proses visa ATAU asuransi:
  //   include_visa / bayar visa  ATAU  include_asuransi / bayar asuransi, dan bukan visa_ready.
  // Peserta refund/cancel/pindah trip TETAP muncul KALAU sudah BAYAR visa/asuransi
  // (biayanya sudah keluar & non-refundable). Yang cuma checkbox tanpa bayar -> tidak.
  pax = pax.filter((p) => {
    const needsVA = p.include_visa === true || payByPax[p.id] === true || p.include_asuransi === true || asrByPax[p.id] === true;
    if (!needsVA || p.visa_ready === true) return false;
    const inactive = p.transfer_status === 'transferred' || p.refund_status === 'refunded' || p.refund_status === 'partial_refund';
    const paidVA = payByPax[p.id] === true || asrByPax[p.id] === true;
    if (inactive && !paidVA) return false;
    return true;
  });
  if (!pax.length) return { ok: true, rows: [] };
  ...
  const rows = pax.map((p) => {
    ...
    return {
      ...
      pesertaStatus: p.transfer_status === 'transferred' ? 'pindah'
        : (p.refund_status === 'refunded' || p.refund_status === 'partial_refund') ? 'refund' : '',
    };
  });
...
async function syncVisaPaymentToHPP(db, pid, row, customerId) {
  ...
  const payload = {
    ...
    payment_status: transferred ? 'lunas' : 'belum bayar',
