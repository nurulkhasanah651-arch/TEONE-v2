...
export async function uploadVisaDocByToken(token, docName, formData, targetPassengerId = null) {
  ...
  try {
    ...
    // Auto-analisa AI saat dokumen sudah lengkap (best-effort; gagal tidak memblokir upload)
    try {
      const { data: _t } = await supabase.from('trips').select('visa_doc_template').eq('id', pax.trip_id).maybeSingle();
      const reqCount = Array.isArray(_t?.visa_doc_template) ? _t.visa_doc_template.length : 0;
      if (reqCount > 0 && nextDocs.length >= reqCount) {
        const { autoAnalyzeAfterUpload } = await import('@/lib/actions/visa-ai');
        await autoAnalyzeAfterUpload(pax.id);
      }
    } catch (e) { /* non-fatal */ }
