...
${companyName}`;
    ...
    waResult = await sendFonnte(normalizePhone(headInfo.customer.phone), message, await getPicFonnteTokenById(supabase, pendingReqs[0]?.trip_id));
    if (waResult.ok) {
...
Terima kasih,
${companyName}`;
      await sendFonnte(normalizePhone(r.customer_phone), message, await getPicFonnteTokenById(supabase, r.trip_id));
      await supabase
        .from('dp_payment_requests')
        .update({ wa_sent: true, wa_sent_at: nowISO })
        .eq('id', r.id);
