...
// Visa server actions — trip-level template + per-participant docs
...
export async function updateVisaGroupInfo(tripId, formData) {
  ...
  const visa_biometric_date = formData.get('visa_biometric_date') || null;
  const visa_status = formData.get('visa_status') || 'pending';
  const visa_notes = (formData.get('visa_notes') || '').trim() || null;

  const { error } = await supabase.from('trips').update({
    visa_country, visa_biometric_date, visa_status, visa_notes,
  }).eq('id', tripId);
...
export async function updateDocTemplate(tripId, docs) {
  ...
  // docs = array of doc names ['Passport', 'Photo 4x6', ...]
