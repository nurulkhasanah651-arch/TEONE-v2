...
// Round 134: Passport AI extract via Claude Vision API
// Path: lib/actions/passport.js
// Extracts: surname, given_names, passport_number, nationality, dob, expiry, sex, place_of_issue
...
export async function extractPassportData(imageUrl) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { error: `Claude API error (${response.status}): ${errText.substring(0, 200)}` };
    }
    ...
    // Normalize: if surname kosong tapi given_names ada (typical Indonesian passport)
