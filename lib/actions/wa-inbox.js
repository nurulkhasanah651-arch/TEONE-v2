...
async function syncClosingDraft(db, convId, user) {
  try {
    const { data: conv } = await db.from('wa_conversations').select('id, pipeline_stage, trip_id, closing_pax, customer_name, customer_phone').eq('id', convId).maybeSingle();
    if (!conv || !conv.trip_id || !CLOSING_STAGES.includes(conv.pipeline_stage || '')) return;
    const patch = {
      trip_id: conv.trip_id, customer_name: conv.customer_name || null, customer_phone: conv.customer_phone || null,
      pax_count: conv.closing_pax || 1, stage: conv.pipeline_stage,
    };
    const { data: existing } = await db.from('cs_closing_drafts').select('id').eq('conversation_id', conv.id).maybeSingle();
    if (existing) { await db.from('cs_closing_drafts').update(patch).eq('id', existing.id); }
    else { await db.from('cs_closing_drafts').insert({ brand: currentBrandCode(), conversation_id: conv.id, ...patch, status: 'pending', created_by: user?.email || null }); }
  } catch {}
}
export async function setPipelineStage(conversationId, stage) {
  ...
  try {
    ...
    await syncClosingDraft(db, conversationId, user);
  } catch (e) { return { error: e?.message }; }
  revalidatePath('/inbox');
  return { ok: true };
}
...
export async function setConversationTrip(conversationId, tripId) {
  ...
  if (!user) return { error: 'Not authenticated' };
  const db = svc() || auth;
  try { await db.from('wa_conversations').update({ trip_id: tripId || null }).eq('id', conversationId); await syncClosingDraft(db, conversationId, user); } catch (e) { return { error: e?.message }; }
  revalidatePath('/inbox');
  return { ok: true };
}

export async function setClosingPax(conversationId, pax) {
  const auth = createClient();
  const { data: { user } } = await auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const db = svc() || auth;
  const n = Math.max(0, parseInt(pax) || 0) || null;
  try { await db.from('wa_conversations').update({ closing_pax: n }).eq('id', conversationId); await syncClosingDraft(db, conversationId, user); } catch (e) { return { error: e?.message }; }
  revalidatePath('/inbox');
  return { ok: true };
}
