...
async function ensureHeadToken(db, passengerId, formType) {
  ...
  if (pax.family_group_id) {
    // Grup HARUS milik trip yg sama (lihat catatan di passport-upload).
    ...
    if (fg?.head_passenger_id && fg.trip_id === pax.trip_id) {
      ...
      console.error('[visa-form ensureHeadToken] family_group lintas trip diabaikan', { pax_id: pax.id, pax_trip: pax.trip_id, fgid: pax.family_group_id, fg_trip: fg.trip_id });
    }
  }
...
export async function sendVisaFormWA(passengerId, formType) {
  ...
  const lines = [
    `${salam()} *${cust?.name || 'Kak'}* 🙏`,
    _picName ? `Saya *${_picName}*, PIC trip kamu di *${brandName}*.` : `Kami tim *${brandName}*.`,
    '',
  ...
  const _tplVisaForm = process.env[`WABA_TPL_VISAFORM_${code === 'khasanah' ? 'KHASANAH' : 'TEONE'}`] || `visa_form_${code}`;
  const _tripLabelVF = `${trip?.kode_trip ? trip.kode_trip + ' \u2014 ' : ''}${trip?.name || ''}`.trim();
  const _introLineVF = _picName ? `Saya ${_picName}, PIC trip kamu di ${brandName}.` : `Kami tim ${brandName}.`;
  const _pesertaCsv = (memNames && memNames.length) ? memNames.join(', ') : (cust?.name || '-');
  // Template Meta blast form (sesuai yg didaftarkan): 6 variabel body + link sbg teks (bukan tombol)
  // {{1}}nama · {{2}}PIC · {{3}}brand · {{4}}jenis form/negara · {{5}}peserta · {{6}}link (auto ikut jenis form terpilih)
  const _visaFormParams = [cust?.name || 'Kak', _picName || 'Tim', brandName, visaFormLabel(formType) || 'Visa', _pesertaCsv, link];
  let _waba = await trySendWabaTemplateForTrip(db, head.trip_id, phone, _tplVisaForm, _visaFormParams, { kind: 'waba_visa_form' });
  if (!_waba?.ok) _waba = await trySendWabaForTrip(db, head.trip_id, phone, lines.join('\n'), { context: 'visa', kind: 'waba_visa_form' });
  if (!_waba?.ok) {
    const _picTok = await getPicFonnteTokenById(db, head.trip_id); // dari nomor PIC bila di-set
    const res = await sendFonnte(phone, lines.join('\n'), { context: 'visa', brand: code, token: _picTok });
