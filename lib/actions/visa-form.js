...
export async function sendVisaFormWABulk(tripId, formType) {
  ...
  if (!db) return { error: 'Service tidak tersedia' };
  const { data: pax } = await db.from('trip_passengers')
    .select('id, family_group_id, is_family_head').eq('trip_id', tripId);
  ...
  for (const p of pax) {
    if (p.family_group_id) {
      ...
      if (!submittedSet.has(p.id)) famMissing[p.family_group_id] = true;
    } else if (!submittedSet.has(p.id)) solos.push(p.id);
  }
