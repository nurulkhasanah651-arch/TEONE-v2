...
export async function sendVisaFormWA(passengerId, formType) {
  ...
  const _visaFormParams = [cust?.name || 'Kak', _introLineVF, visaFormLabel(formType) || 'Visa', _tripLabelVF || '-'];
  let _waba = await trySendWabaTemplateForTrip(db, head.trip_id, phone, _tplVisaForm, _visaFormParams, { kind: 'waba_visa', buttonUrlSuffix: token });
  if (!_waba?.ok) _waba = await trySendWabaForTrip(db, head.trip_id, phone, lines.join('\n'), { context: 'visa', kind: 'waba_visa' });
  if (!_waba?.ok) {
