...
import { STD_PAYMENT_TYPES, guessIsAddon } from '@/lib/utils/payment-types';
...
export async function addPayment(passengerId, tripId, formData) {
  ...
  // Tipe bebas: CS memilih apakah ini biaya tambahan (menambah tagihan) atau cicilan (mengurangi).
  const _kind = String(formData.get('addon_kind') || '').trim();
  const is_addon = STD_PAYMENT_TYPES.includes(type) ? null
    : (_kind === 'addon' ? true : _kind === 'cicilan' ? false : guessIsAddon(type));
  ...
  const { data: inserted, error } = await supabase.from('participant_payments').insert({
    passenger_id: passengerId, type, label, amount, paid_at, due_at, notes, created_by, is_addon,
  }).select().maybeSingle();
...
export async function updatePaymentTemplate(tripId, formData) {
  ...
  // .select() supaya UPDATE yang ditolak RLS (0 baris, tanpa error) tidak terlihat sukses.
  const { data: updated, error } = await supabase.from('trips').update(upd).eq('id', tripId).select('id');
  if (error) return { error: error.message };
  if (!updated || updated.length === 0) {
    return { error: 'Template tidak tersimpan — kamu tidak punya izin mengubah trip ini (PIC hanya bisa mengubah trip miliknya sendiri).' };
  }
...
export async function settlePelunasanAll(passengerId, tripId, opts = {}) {
  const skipAsuransi = opts && opts.skipAsuransi === true;
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  { const _g = await assertStaff(user, '/invoices'); if (_g.error) return { error: _g.error }; }
  ...
  const { data: theP } = await supabase.from('trip_passengers').select('id, family_group_id, is_family_head').eq('id', passengerId).maybeSingle();
  if (!theP) return { error: 'Peserta tidak ditemukan' };

  // Family-aware: bila peserta bagian keluarga, lunasi SEMUA anggota keluarga sekaligus.
  let ids = [passengerId];
  if (theP.family_group_id) {
    const { data: fam } = await supabase.from('trip_passengers').select('id, is_family_head').eq('trip_id', tripId).eq('family_group_id', theP.family_group_id);
    if (fam && fam.length) ids = fam.map((x) => x.id);
  }
  ...
  let grandPokok = 0, grandVisa = 0, grandAsr = 0;
  for (const pid of ids) {
    const { data: pax } = await supabase.from('trip_passengers').select('id, room_type, age_type, price_paid, discount_amount, include_visa, include_asuransi, visa_ready, visa_type').eq('id', pid).maybeSingle();
    if (!pax) continue;
    const { data: pays } = await supabase.from('participant_payments').select('type, amount').eq('passenger_id', pid);
    const pokokExp = Math.max(mainExpectedPerPassenger(pax, bd) - (Number(pax.discount_amount) || 0), 0);
    ...
    const visa = (pax.include_visa && !pax.visa_ready && !hasVisa) ? visaPriceTpl(tpl, bd, pax.visa_type) : 0;
    const asuransi = (!skipAsuransi && pax.include_asuransi && !hasAsr) ? aPrice : 0;
    const ins = async (type, amt) => {
      if (amt <= 0) return;
      const { data: ex } = await supabase.from('participant_payments').select('id, amount').eq('passenger_id', pid).eq('type', type).maybeSingle();
      let rid;
      if (ex) { const na = (Number(ex.amount) || 0) + amt; await supabase.from('participant_payments').update({ amount: na, paid_at: today, created_by }).eq('id', ex.id); rid = ex.id; }
      else { const { data: nu } = await supabase.from('participant_payments').insert({ passenger_id: pid, type, amount: amt, paid_at: today, created_by }).select().maybeSingle(); rid = nu?.id; }
      if (rid) { try { await autoCreateCashInFromPayment({ paymentId: rid, tripId, passengerId: pid, type, amount: amt }); } catch {} }
    };
    await ins('Pelunasan', pokok);
    await ins('Visa', visa);
    await ins('Asuransi', asuransi);
    grandPokok += pokok; grandVisa += visa; grandAsr += asuransi;
  }

  if (grandPokok <= 0 && grandVisa <= 0 && grandAsr <= 0) return { error: 'Tidak ada sisa untuk dilunasi.' };

  // WA tanda terima lengkap ke kepala family (atau peserta).
  // PIC dgn WA manual -> sendPaymentReceivedWA balikan template (tidak dikirim).
  let _wa = null;
  try {
    const headPid = theP.is_family_head ? passengerId : (ids[0] || passengerId);
    const { sendPaymentReceivedWA } = await import('@/lib/actions/wa-payment-notif');
    _wa = await sendPaymentReceivedWA(headPid);
  } catch {}
  ...
  revalidatePath('/accounting');
  return {
    ok: true, settled: { pokok: grandPokok, visa: grandVisa, asuransi: grandAsr }, members: ids.length,
    ...(_wa && _wa.wa_manual ? { wa_manual: true, wa_message: _wa.wa_message, wa_phone: _wa.wa_phone, customer_name: _wa.customer_name } : {}),
  };
}
