...
export async function createEmployee(formData) {
  ...
  const payload = {
    ...
    waba_tpl_invoice: clean(formData.get('waba_tpl_invoice')),
    waba_tpl_konfirmasi: clean(formData.get('waba_tpl_konfirmasi')),
    waba_tpl_paspor: clean(formData.get('waba_tpl_paspor')),
    waba_tpl_ongkir: clean(formData.get('waba_tpl_ongkir')),
    waba_tpl_pengiriman: clean(formData.get('waba_tpl_pengiriman')),
    waba_tpl_perubahan_jadwal: clean(formData.get('waba_tpl_perubahan_jadwal')),
    waba_tpl_finalisasi_tiket: clean(formData.get('waba_tpl_finalisasi_tiket')),
    waba_tpl_resi: clean(formData.get('waba_tpl_resi')),
    waba_tpl_visa_nonbio: clean(formData.get('waba_tpl_visa_nonbio')),
    waba_tpl_dokumen: clean(formData.get('waba_tpl_dokumen')),
    waba_tpl_visa_jadwalbio: clean(formData.get('waba_tpl_visa_jadwalbio')),
    waba_tpl_visa_diterima: clean(formData.get('waba_tpl_visa_diterima')),
    waba_tpl_visa_kurang: clean(formData.get('waba_tpl_visa_kurang')),
    waba_tpl_visa_reminderbio: clean(formData.get('waba_tpl_visa_reminderbio')),
    waba_tpl_visa_approved: clean(formData.get('waba_tpl_visa_approved')),
    waba_tpl_visa_rejected: clean(formData.get('waba_tpl_visa_rejected')),
    waba_tpl_visa_family: clean(formData.get('waba_tpl_visa_family')),
    waba_tpl_visa_form: clean(formData.get('waba_tpl_visa_form')),
...
async function syncWaNumber(supabase, emp) {
  try {
    ...
    // Lepas nomor lain milik karyawan ini (ganti nomor -> yang lama dinonaktifkan).
    try { await supabase.from('wa_numbers').update({ active: false }).eq('pic_employee_id', emp.id).neq('phone_number_id', pid); } catch {}
...
// RESET PASSWORD KARYAWAN — OWNER ONLY.
// Owner set password login sementara utk karyawan (mis. karyawan lupa & email tidak aktif).
// Pakai Supabase Admin API (service role) updateUserById. Tidak menyimpan password sbg teks.
export async function resetEmployeePassword(employeeId, newPassword) {
  const auth = createClient();
  const { data: { user } } = await auth.auth.getUser();
  if (!user) return { error: 'Not authenticated' };
  const g = await assertStaff(user, '/hr');
  if (g.error) return { error: g.error };
  if (g.role !== 'owner') return { error: 'Hanya owner yang boleh reset password karyawan.' };

  const pw = String(newPassword || '').trim();
  if (pw.length < 8) return { error: 'Password minimal 8 karakter.' };

  const db = getServiceClient();
  if (!db) return { error: 'Service tidak tersedia.' };

  const { data: emp } = await db.from('employees').select('id, full_name, email').eq('id', employeeId).maybeSingle();
  if (!emp) return { error: 'Karyawan tidak ditemukan.' };
  const email = (emp.email || '').trim().toLowerCase();
  if (!email) return { error: 'Karyawan ini belum punya email login. Isi email dulu di data karyawan.' };

  // Cari akun auth berdasarkan email (paginasi admin listUsers)
  let target = null;
  try {
    for (let page = 1; page <= 20 && !target; page++) {
      const { data, error } = await db.auth.admin.listUsers({ page, perPage: 200 });
      if (error) return { error: 'Gagal akses daftar akun: ' + error.message };
      const users = data?.users || [];
      target = users.find((u) => (u.email || '').toLowerCase() === email) || null;
      if (users.length < 200) break;
    }
  } catch (e) {
    return { error: 'Gagal cari akun: ' + (e?.message || 'unknown') };
  }
  if (!target) return { error: `Akun login untuk ${email} belum ada di sistem. Buat akun dulu / cek emailnya.` };

  const { error: updErr } = await db.auth.admin.updateUserById(target.id, { password: pw });
  if (updErr) return { error: 'Gagal reset password: ' + updErr.message };
