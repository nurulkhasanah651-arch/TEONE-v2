...
async function syncTourLeaderTable(supabase, employee) {
  if (employee.employment_type !== 'tour_leader') return;

  try {
    // Cek kalau ada legacy_tl_id, update yang itu
    if (employee.legacy_tl_id) {
      await supabase
        ...
        .eq('id', employee.legacy_tl_id);
    } else {
      // Insert baru ke tour_leaders + simpan id-nya
