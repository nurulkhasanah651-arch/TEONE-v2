...
import { brandServiceRoleKey, brandSupabaseUrl, runWithBrand } from '@/lib/supabase/service-env';
 ...
 */
export async function addTLExpense(...args) {
  const __brand = args[0] && args[0].brand;
  const __g = await assertTLTripAccess(args[0] && args[0].tripId, __brand);
  if (__g && __g.error) return { error: __g.error };
  return runWithBrand(__brand, () => _addTLExpense(...args));
}
async function _addTLExpense({
  ...
  try {
    ...
    // CATAT SETIAP expense ke log trip_tl_expenses (termasuk yg ketutup petty cash) + bukti
    try {
      const statusLog = routing === 'petty_full' ? 'petty'
        : routing === 'petty_partial' ? 'petty+reimburse' : 'reimburse';
      await supabase.from('trip_tl_expenses').insert({
        trip_id: tripId,
        category: category || null,
        description,
        amount_idr: expenseAmount,
        receipt_url: receiptUrl || null,
        tanggal: spentAt || null,
        status: statusLog,
        notes: notes || null,
        submitted_by: userName || userEmail || null,
