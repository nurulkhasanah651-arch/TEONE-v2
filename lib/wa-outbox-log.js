...
const _DEPT_LABEL = { cs: 'CS', finance: 'Finance', visa: 'Visa', delivery: 'Ops', pic: 'PIC', ops: 'Ops' };
export async function logWA({ context, phone, message, kind, tripId, status, state, reason, fonnteId, senderToken } = {}) {
  try {
    const db = svc(); if (!db) return;
    let brand = 'teone'; try { brand = currentBrandCode(); } catch {}
    // Keterangan pengirim: kalau token = token PIC (di employees) -> nama + nomor PIC; kalau tidak -> label departemen.
    let sender = _DEPT_LABEL[context] || context || null;
    if (senderToken) {
      try {
        const { data: emp } = await db.from('employees')
          .select('full_name, nickname, phone, whatsapp').eq('fonnte_token', String(senderToken)).maybeSingle();
        if (emp) {
          const nm = (emp.nickname || emp.full_name || 'PIC');
          const no = emp.phone || emp.whatsapp || '';
          sender = no ? `${nm} (${no})` : nm;
        }
      } catch {}
    }
