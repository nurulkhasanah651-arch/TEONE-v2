...
export async function isPicWaManualForTrip(db, tripId) {
  try {
    ...
    const chk = (row) => !!(row && row.wa_manual === true);
    if (email) {
      const { data: e } = await db.from('employees').select('wa_manual').ilike('email', email).maybeSingle();
      if (e) return chk(e);
    }
    if (name) {
      const { data: byFull } = await db.from('employees').select('wa_manual').ilike('full_name', name).maybeSingle();
      if (byFull) return chk(byFull);
      const { data: byNick } = await db.from('employees').select('wa_manual').ilike('nickname', name).maybeSingle();
