...
import { BRAND_IDS, resolveBrandCode, supabaseEnvFor, isStorefrontHost } from '@/lib/brand-shared';
...
export async function updateSession(request) {
  ...
  // .com = etalase publik; .app (khasanahtravel.app) = internal/login. Pakai exact-match
  // isStorefrontHost (bukan includes longgar yg ikut menangkap .app).
  const isStorefront =
    isStorefrontHost(hostLower) ||
    request.nextUrl.searchParams.get('shop') === '1';
  if (path === '/' && isStorefront) {
    const url = request.nextUrl.clone();
    url.pathname = '/home';
    return NextResponse.rewrite(url);
  }

  // SEO — Redirect URL lama (situs WordPress/WooCommerce sebelumnya) ke halaman baru,
  // supaya ranking Google yang sudah terkumpul tidak hangus & pengunjung tidak nyasar
  // ke halaman login. Hanya di domain etalase publik.
  if (isStorefront) {
    let legacyDest = null;
    let legacyPermanent = true;
    if (path === '/blog' || path.startsWith('/blog/')) {
      legacyDest = '/'; legacyPermanent = false;            // blog belum ada lagi → sementara ke home
    } else if (
      path === '/shop' || path.startsWith('/shop/') ||
      path === '/open-trip' || path.startsWith('/open-trip/') ||
      path === '/product' || path.startsWith('/product/') ||
      path.startsWith('/product-category/') ||
      path.startsWith('/product-tag/') ||
      path.startsWith('/tag/')
    ) {
      legacyDest = '/trip';                                  // semua listing/produk lama → daftar trip
    }
    if (legacyDest) {
      const url = request.nextUrl.clone();
      url.pathname = legacyDest;
      url.search = '';
      return NextResponse.redirect(url, legacyPermanent ? 301 : 302);
    }
  }
  ...
  const isPublicTokenRoute =
    ...
    path.startsWith('/tc/') ||                              // /tc/[token] Tour Confirmation publik (dikirim ke peserta)
    path === '/home' || path === '/masuk' || path === '/akun' || path.startsWith('/akun/') ||
    path === '/trip' || path.startsWith('/trip/') ||      // /trip storefront publik (BUKAN /trips internal)
    path === '/reset-password' ||                          // set password baru via link email (recovery)
    path === '/request-trip' || path === '/request-private-trip' ||  // form private trip publik
