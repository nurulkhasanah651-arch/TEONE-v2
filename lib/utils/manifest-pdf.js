...
export async function downloadManifestPDF({ trip = {}, rows = [], showKtp = false }) {
  // NIK disisipkan setelah 'No. Paspor' (index 7) hanya utk Khasanah. TEONE tak berubah.
  const COLS = showKtp ? [...BASE_COLS.slice(0, 8), 'NIK', ...BASE_COLS.slice(8)] : BASE_COLS;
  const W = showKtp ? [...BASE_W.slice(0, 8), 24, ...BASE_W.slice(8)] : BASE_W;
