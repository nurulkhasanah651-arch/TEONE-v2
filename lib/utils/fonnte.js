...
export function getFonnteToken(context = 'finance') {
  ...
  if (ctx === 'tl' || ctx === 'tour_leader' || ctx === 'tourleader') {
    const tlToken = process.env.FONNTE_TOKEN_TL;
    if (tlToken) return { token: tlToken, source: 'FONNTE_TOKEN_TL' };
  }

  if (ctx === 'finance' || ctx === 'invoice' || ctx === 'payment' || ctx === 'peserta') {
    const financeToken = process.env.FONNTE_TOKEN_FINANCE;
    if (financeToken) return { token: financeToken, source: 'FONNTE_TOKEN_FINANCE' };
  }
...
export async function sendFonnte(phone, message, options = {}) {
  ...
  const { token, source } = getFonnteToken(context);
