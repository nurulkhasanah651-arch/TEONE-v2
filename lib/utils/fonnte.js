...
export async function sendFonnte(phone, message, options = {}) {
  ...
  if (r.ok) return { ok: true, sentVia: source };
  ...
  if (usedOverride) {
    ...
    if (fb.token && fb.token !== token) {
      ...
      if (r2.ok) return { ok: true, sentVia: fb.source + ' (fallback dari token PIC yg gagal)' };
      finalErr = { ...r2, sentVia: fb.source + ' (fallback)' };
    }
  }
  // Catat kegagalan (nomor mungkin logout) → wa_outbox untuk banner + kirim ulang. Best-effort.
  try {
