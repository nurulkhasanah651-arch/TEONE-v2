...
export async function sendFonnte(phone, message, options = {}) {
  ...
  if (usedOverride) {
    ...
    if (fb.token && fb.token !== token) {
      ...
      if (r2.ok) { await _logWA({ status: 'sent', state: 'sent', fonnteId: r2.id, senderToken: fb.token }); return { ok: true, sentVia: fb.source + ' (fallback dari token PIC yg gagal)', id: r2.id }; }
