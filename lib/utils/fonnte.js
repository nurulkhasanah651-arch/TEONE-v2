...
export function getFonnteToken(context = 'finance', brand = '') {
  ...
  const suffix = String(brand || '').toLowerCase() === 'khasanah' ? '_KHASANAH' : '';
  // ambil env var: utamakan versi brand (_KHASANAH), fallback ke base
  const pick = (base) => process.env[base + suffix] || process.env[base] || null;
  const picked = (base) => (process.env[base + suffix] ? base + suffix : base);
  ...
  const fb = process.env['FONNTE_TOKEN' + suffix] || process.env.FONNTE_TOKEN;
  if (fb) return { token: fb, source: 'FONNTE_TOKEN (fallback)' };
...
export async function sendFonnte(phone, message, options = {}) {
  ...
  if (options.token && String(options.token).trim()) {
    ...
    ({ token, source } = getFonnteToken(context, options.brand || ''));
  }
  ...
  if (r.ok) { await _logWA({ status: 'sent', state: 'sent', fonnteId: r.id }); return { ok: true, sentVia: source, id: r.id }; }
  ...
  if (usedOverride) {
    const fb = getFonnteToken(context, options.brand || '');
    if (fb.token && fb.token !== token) {
      const r2 = await _post(fb.token);
      if (r2.ok) { await _logWA({ status: 'sent', state: 'sent', fonnteId: r2.id }); return { ok: true, sentVia: fb.source + ' (fallback dari token PIC yg gagal)', id: r2.id }; }
      finalErr = { ...r2, sentVia: fb.source + ' (fallback)' };
    }
  }
  // Gagal: catat ke wa_log (History) + wa_outbox (banner + kirim ulang). Best-effort.
  await _logWA({ status: 'failed', state: 'failed', reason: finalErr.error });
  try {
    const m = await import('@/lib/wa-outbox-log');
    await m.logFailedWA({ context, phone, message, reason: finalErr.error, brand: options.brand });
  } catch { /* abaikan */ }
  return finalErr;
}
