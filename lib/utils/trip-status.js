...
export function effectiveSellingStatus(trip) {
  ...
  if (s === 'closed selling') return 'closed selling';   // di-close manual
  if (sold > 0) return 'open selling';                    // sudah ada peserta = pasti open
  if (s === 'open selling') return 'open selling';        // sudah di-publish / open manual
  return 'prepare to sell';                                // termasuk status tersimpan 'prepare to sell'
}
