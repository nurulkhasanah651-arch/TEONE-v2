...
function formatDocList(raw) {
  ...
  if (Array.isArray(raw)) lines = raw.slice();
    ...
    const text = (raw && String(raw).trim()) ? String(raw) : DEFAULT_VISA_DOCS;
    lines = text.split('\n');
  }
  let out = lines
    .map((l) => String(l).replace(/^[\s\-•☐📸📝]+/, '').trim())
    .filter(Boolean);
  if (out.length === 0) out = DEFAULT_VISA_DOCS.split('\n');   // fallback daftar standar
...
✅ Jadwal *Biometrik Visa {{country_name}}* Kaka sudah DIJADWALKAN:

🗓 Tanggal: *{{tanggal_biometrik}}*
⏰ Jam: *{{jam_biometrik}}*
