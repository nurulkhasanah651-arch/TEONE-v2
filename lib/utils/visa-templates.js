...
📤 *Mohon kirim juga file dokumennya ke sini* agar kami bisa screening kelengkapan lebih cepat:
{{upload_portal_url}}
_Catatan: dokumen fisik TETAP WAJIB dikirim ke alamat kantor di atas ya, Kak._
...
📮 *Pengembalian Paspor*
Paspor asli Kaka mau *dikirim ke alamat* atau *dibawa Tour Leader saat trip*?

• Kalau mau *DIKIRIM*, mohon isi alamat pengiriman di link berikut:
🔗 {{delivery_link}}
_(setelah diisi, alamat otomatis tercatat di sistem kami)_

• Kalau mau *DIBAWA TOUR LEADER* saat keberangkatan, cukup *balas pesan ini* ya 🙏
...
{{refund_section}}
📮 *Pengembalian Paspor*
Paspor asli Kaka akan kami kembalikan. Mohon isi alamat pengiriman di link berikut:
🔗 {{delivery_link}}

📞 Mau konsultasi lebih lanjut? Chat aja langsung ya.
...
export function renderTemplate(templateKey, vars = {}, trip = null) {
  ...
  const sections = {
    ...
    pdf_syarat_section: (() => {
      const lines = [];
      if (Array.isArray(vars.syarat_docs)) for (const d of vars.syarat_docs) { if (d?.url) lines.push(`📎 ${d.label}:\n${d.url}`); }
      if (vars.pdf_syarat_visa_url) lines.push(`🔗 Detail lengkap: ${vars.pdf_syarat_visa_url}`);
      return lines.length ? `\n*Syarat lengkap & contoh dokumen:*\n${lines.join('\n')}` : '';
    })(),
    fillable_docs_section: (() => {
      const docs = Array.isArray(vars.fillable_docs) ? vars.fillable_docs.filter((d) => d && d.url) : [];
      if (!docs.length) return '';
      const links = docs.map((d) => `📄 ${d.label}:\n${d.url}`).join('\n');
      return `\n📝 *Dokumen yang harus DIISI & DITANDATANGANI:*\n${links}\n_Mohon di-download, diisi lengkap, ditandatangani, lalu dikirim balik ke kami bersama dokumen lainnya ya, Kak 🙏_`;
    })(),
  ...
  const replacements = {
    ...
    delivery_link: vars.delivery_link || '(link akan dikirim terpisah)',
    rejection_solution: (vars.rejection_solution && String(vars.rejection_solution).trim()) ? String(vars.rejection_solution).trim() : '(akan kami informasikan via chat)',
    refund_section: (Number(vars.refund_amount) > 0)
      ? `💰 Estimasi dana yang dapat dikembalikan (refund): *Rp ${Number(vars.refund_amount).toLocaleString('id-ID')}*\n`
      : '',
    ...sections,
  };
