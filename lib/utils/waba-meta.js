...
export async function sendMetaTemplate(cfg, phoneNumberId, toPhone, templateName, lang, params) {
  const to = normalizeWaPhone(toPhone);
  if (!to) return { ok: false, error: 'Nomor tujuan kosong' };
  if (!templateName) return { ok: false, error: 'template name kosong' };
  const components = (params && params.length)
    ? [{ type: 'body', parameters: params.map((p) => ({ type: 'text', text: String(p == null || p === '' ? '-' : p) })) }]
    : [];
