...
export function buildManifestAOA({ trip = {}, rows = [] }) {
  ...
  const COLS = ['No.', 'First Name', 'Last Name', 'Gender', 'Tempat Lahir', 'Tgl Lahir', 'Umur', ...(showKtp ? ['NIK', 'Alamat KTP'] : []), 'No. Paspor', 'Tgl Issue', 'Issuing Office', 'Tgl Expired', 'No. HP', 'Keterangan', 'Catatan / Request'];
  ...
  for (const r of (rows || [])) {
    aoa.push([
      ...
      ...(showKtp ? [r.nik || '', r.ktp_alamat || ''] : []),
      r.passport_no, r.issue_date, r.issuing_office, r.expiry_date, r.phone, r.keterangan || '', r.catatan || '',
    ]);
  }
  const cols = [{ wch: 5 }, { wch: 18 }, { wch: 18 }, { wch: 7 }, { wch: 16 }, { wch: 13 }, { wch: 6 }, ...(showKtp ? [{ wch: 18 }, { wch: 40 }] : []), { wch: 18 }, { wch: 13 }, { wch: 18 }, { wch: 13 }, { wch: 16 }, { wch: 18 }, { wch: 28 }];
