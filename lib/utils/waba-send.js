...
import { wabaConfig, sendWabaText, sendWabaTemplate, normalizeWaPhone } from '@/lib/utils/waba';
...
/**
 * Kirim TEMPLATE via WABA untuk PIC trip ini (dipakai saat kirim ke peserta yang
 * belum pernah chat — mis. konfirmasi DP/pembayaran).
 */
export async function trySendWabaTemplateForTrip(supabase, tripId, phone, templateName, params, opts = {}) {
  try {
    const cfg = wabaConfig();
    if (!cfg || !tripId || !supabase || !templateName) return null;
    const picKey = await getPicWabaKey(supabase, tripId, cfg.accountKey);
    if (!picKey) return null;
    const r = await sendWabaTemplate({ key: picKey.key, endpoint: cfg.endpoint, phoneId: picKey.phoneId }, phone, templateName, cfg.lang, params);
    if (r.ok) {
      try {
        await supabase.from('wa_log').insert({
          brand: currentBrandCode(), context: opts.context || 'finance', kind: opts.kind || 'waba_tpl',
          trip_id: tripId, target_phone: normalizeWaPhone(phone), message: '[template] ' + templateName, status: 'sent', sender: picKey.pic,
        });
      } catch {}
      return { ok: true };
    }
    console.error('[WABA template gagal]', r.error);
    return { ok: false, error: r.error };
  } catch (e) {
