...
async function getPicApicoidPhone(supabase, tripId) {
  try {
    ...
    const cols = 'waba_phone_id, full_name, waba_tpl_invoice, waba_tpl_konfirmasi';
    let emp = null;
    ...
    const _tplInv = (emp.waba_tpl_invoice || '').trim();
    const _tplKon = (emp.waba_tpl_konfirmasi || '').trim();
    // Suffix per-PIC diambil dari nama template invoice/konfirmasi (mis. invoice_khasanah_lia -> lia,
    // konfirmasi_payment_khasanah_lia -> lia). Dipakai utk nama template passpor/visa per-PIC.
    const _sm = (_tplInv || _tplKon).match(/_(khasanah|teone)_(.+)$/);
    const tplSuffix = _sm ? _sm[2].trim() : '';
    return { phoneId, pic: emp.full_name || name, tplInvoice: _tplInv, tplKonfirmasi: _tplKon, tplSuffix };
  } catch { return null; }
}
  ...
  try {
    ...
    if (acfg) {
      ...
      if (ap) {
        ...
        else if ((_kind === 'waba_passport' || _kind === 'waba_visa') && ap.tplSuffix) tname = `${templateName}_${ap.tplSuffix}`;
