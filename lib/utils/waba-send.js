...
async function getPicApicoidPhone(supabase, tripId) {
  try {
    ...
    return { phoneId, pic: emp.full_name || name, tplInvoice: (emp.waba_tpl_invoice || '').trim(), tplKonfirmasi: (emp.waba_tpl_konfirmasi || '').trim() };
  } catch { return null; }
}
  ...
  try {
    ...
    if (acfg) {
      ...
      if (ap) {
        ...
        const r = await sendApicoidTemplate(acfg, ap.phoneId, phone, tname, acfg.lang, params);
