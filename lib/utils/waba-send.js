...
async function getPicApicoidPhone(supabase, tripId) {
  try {
    ...
    const cols = 'waba_phone_id, full_name, waba_tpl_invoice, waba_tpl_konfirmasi, waba_tpl_paspor, waba_tpl_ongkir, waba_tpl_pengiriman, waba_tpl_perubahan_jadwal, waba_tpl_finalisasi_tiket, waba_tpl_resi, waba_tpl_dokumen, waba_tpl_visa_nonbio, waba_tpl_visa_jadwalbio, waba_tpl_visa_diterima, waba_tpl_visa_kurang, waba_tpl_visa_reminderbio, waba_tpl_visa_approved, waba_tpl_visa_rejected, waba_tpl_visa_family, waba_tpl_visa_form';
    let emp = null;
    ...
    return {
      ...
      tplPerubahanJadwal: _t(emp.waba_tpl_perubahan_jadwal), tplFinalisasiTiket: _t(emp.waba_tpl_finalisasi_tiket),
      tplResi: _t(emp.waba_tpl_resi),
      // Template VISA per-PIC (diisi di HR)
      tplVisaDokumen: _t(emp.waba_tpl_dokumen), tplVisaNonbio: _t(emp.waba_tpl_visa_nonbio),
      tplVisaJadwalBio: _t(emp.waba_tpl_visa_jadwalbio), tplVisaDiterima: _t(emp.waba_tpl_visa_diterima),
      tplVisaKurang: _t(emp.waba_tpl_visa_kurang), tplVisaReminderBio: _t(emp.waba_tpl_visa_reminderbio),
      tplVisaApproved: _t(emp.waba_tpl_visa_approved), tplVisaRejected: _t(emp.waba_tpl_visa_rejected),
      tplVisaFamily: _t(emp.waba_tpl_visa_family), tplVisaForm: _t(emp.waba_tpl_visa_form),
    };
  } catch { return null; }
}
  ...
  try {
    ...
    if (acfg) {
      ...
      if (ap) {
        ...
        else if (_kind === 'waba_perubahan_jadwal' && ap.tplPerubahanJadwal) tname = ap.tplPerubahanJadwal;
        else if (_kind === 'waba_finalisasi_tiket' && ap.tplFinalisasiTiket) tname = ap.tplFinalisasiTiket;
        else if (_kind === 'waba_resi' && ap.tplResi) tname = ap.tplResi;
        // VISA per-PIC
        else if (_kind === 'waba_visa_dokumen' && ap.tplVisaDokumen) tname = ap.tplVisaDokumen;
        else if (_kind === 'waba_visa_nonbio' && ap.tplVisaNonbio) tname = ap.tplVisaNonbio;
        else if (_kind === 'waba_visa_jadwalbio' && ap.tplVisaJadwalBio) tname = ap.tplVisaJadwalBio;
        else if (_kind === 'waba_visa_diterima' && ap.tplVisaDiterima) tname = ap.tplVisaDiterima;
        else if (_kind === 'waba_visa_kurang' && ap.tplVisaKurang) tname = ap.tplVisaKurang;
        else if (_kind === 'waba_visa_reminderbio' && ap.tplVisaReminderBio) tname = ap.tplVisaReminderBio;
        else if (_kind === 'waba_visa_approved' && ap.tplVisaApproved) tname = ap.tplVisaApproved;
        else if (_kind === 'waba_visa_rejected' && ap.tplVisaRejected) tname = ap.tplVisaRejected;
        else if (_kind === 'waba_visa_family' && ap.tplVisaFamily) tname = ap.tplVisaFamily;
        else if (_kind === 'waba_visa_form' && ap.tplVisaForm) tname = ap.tplVisaForm;
        // Kalau template per-PIC utk kind visa belum diisi di HR -> jangan pakai nama default
        // yg mungkin tak terdaftar (bikin gagal). Balikan null supaya caller fallback ke Fonnte.
        if (_kind.startsWith('waba_visa') && tname === templateName) return null;
        if (opts.countrySuffix && _kind.startsWith('waba_visa')) tname = `${tname}_${opts.countrySuffix}`;
        const _lang = opts.lang || acfg.lang;
        const r = await sendApicoidTemplate(acfg, ap.phoneId, phone, tname, _lang, params, opts.buttonUrlSuffix, opts.headerImageUrl);
