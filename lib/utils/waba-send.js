...
async function getPicApicoidPhone(supabase, tripId) {
  try {
    ...
    const cols = 'waba_phone_id, full_name';
    let emp = null;
    ...
    return { phoneId, pic: emp.full_name || name };
  } catch { return null; }
}
...
export async function logOutbound(supabase, { phoneNumberId, numberRowId, toPhone, body, templateName, wamid, sentBy }) {
  try {
    ...
    if (!conv) {
      ...
      await db.from('wa_conversations').update({ last_message_at: now, last_message_preview: preview }).eq('id', conv.id);
    }
    if (conv?.id) {
...
export async function trySendWabaTemplateForTrip(supabase, tripId, phone, templateName, params, opts = {}) {
  try {
    ...
    if (acfg) {
      const ap = await getPicApicoidPhone(supabase, tripId);
      if (ap) {
        const r = await sendApicoidTemplate(acfg, ap.phoneId, phone, templateName, acfg.lang, params);
        if (r.ok) { await logOutbound(supabase, { phoneNumberId: ap.phoneId, toPhone: phone, templateName, wamid: r.id }); return { ok: true, via: 'apicoid' }; }
        console.error('[Apicoid template gagal]', r.error);
        return { ok: false, error: r.error };
      }
    }
