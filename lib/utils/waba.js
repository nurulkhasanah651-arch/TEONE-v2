...
export function wabaConfig() {
  ...
  return {
    // Key akun dari Vercel env (satu akun Api.co.id = satu key). Nomor pengirim
    // ditentukan per-PIC lewat Phone Number ID. Key bisa juga di-override per-PIC.
    accountKey: String(process.env.WABA_API_KEY_KHASANAH || '').trim(),
...
/**
 * Kirim TEMPLATE message via WABA (wajib utk peserta yang belum chat / di luar 24 jam).
 * params = array nilai variabel body {{1}}..{{n}} sesuai urutan template.
 */
export async function sendWabaTemplate({ key, endpoint, phoneId }, phone, templateName, lang, params) {
  if (!key) return { ok: false, error: 'WABA key kosong' };
  ...
  try {
    const body = {
      ...
      template: {
        ...
        components: [{
          type: 'body',
          parameters: (params || []).map((p) => ({ type: 'text', text: String(p == null || p === '' ? '-' : p) })),
        }],
      },
    };
    ...
    let json = {};
    try { json = JSON.parse(txt); } catch {}
    if (!res.ok || json?.success === false) return { ok: false, error: `WABA tpl ${res.status}: ${txt.slice(0, 300)}` };
    return { ok: true, id: json?.data?.message_id || null };
  } catch (e) {
    return { ok: false, error: 'WABA tpl fetch: ' + (e?.message || e) };
  }
}
