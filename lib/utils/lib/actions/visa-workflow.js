// R215m: Visa workflow server actions
...
// ============================================================
// 1. UPDATE TRIP VISA CONFIG (PDF urls, default cost, biometric location, dll)
// ============================================================
export async function updateTripVisaConfig(tripId, config) {
  ...
  const allowed = [
    ...
    'visa_biometric_location', 'visa_biometric_time', 'visa_biometric_date',
    'visa_pickup_address',
...
// ============================================================
// 2. UPDATE PER-PESERTA VISA COST (override default)
// ============================================================
...
// ============================================================
// 3. CREATE HPP ITEM untuk visa cost + REQUEST DP ke Finance
// ============================================================
export async function requestVisaCostToFinance(passengerId, type, amount, vendor, notes) {
  ...
  // R215m — Fallback payment_status (sama kayak R215k)
  ...
  // Link HPP item ke peserta
  const refField = type === 'biometric' ? 'visa_biometric_hpp_item_id' : 'visa_cost_hpp_item_id';
  try {
    await supabase
      .from('trip_passengers')
      .update({ [refField]: inserted.id, [type === 'biometric' ? 'visa_biometric_cost' : 'visa_visa_cost']: amount })
      .eq('id', passengerId);
  } catch {}
  ...
  return { ok: true, hpp_item: inserted, message: `HPP item dibuat — silakan klik 💰 Request DP di Finance Cashflow untuk approval` };
}
...
// ============================================================
// 5. SEND VISA WA (per peserta atau bulk family-aware)
// ============================================================
export async function sendVisaWA({ tripId, passengerIds, templateKey, customVars = {}, familyAware = false }) {
  ...
  // R215m — Family-aware grouping (kirim cuma 1× per family ke kepala family)
  let targetPaxList = passengers || [];
  let familyMemberMap = {}; // family_group_id → list nama anggota
  if (familyAware) {
    const grouped = {};
    const noFamily = [];
  ...
  const baseVars = {
    ...
    tanggal_biometrik: trip.visa_biometric_date,
    jam_biometrik: trip.visa_biometric_time,
  ...
  for (const pax of targetPaxList) {
    ...
    // R215m — Attach foto visa kalau template approved/rejected
...
// ============================================================
// 6. UPLOAD VISA RESULT (foto visa approved/rejected)
// ============================================================
export async function uploadVisaResult(passengerId, fileUrl, result, extras = {}) {
  ...
  // R215m — Auto-send WA hasil (kalau auto_send_wa = true)
...
// ============================================================
// 7. GENERATE upload token (untuk visa tanpa biometrik / portal upload)
// ============================================================
