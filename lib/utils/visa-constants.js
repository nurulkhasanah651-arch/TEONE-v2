...
export const VISA_STAGES = {
  ...
  belum_mulai:        { label: 'Belum Diurus',        color: 'slate'  },
};

export function deriveVisaStage(p, template = []) {
  if (!p) return { key: 'belum_mulai', ...VISA_STAGES.belum_mulai };
  // Butuh visa jika checkbox include_visa ON ATAU sudah bayar visa (deteksi peserta lama
  // yang di-input lewat pembayaran walau checkbox belum di-set). Non-destruktif.
  const needs = (p.include_visa === true || p.visaPaid === true) && p.visa_ready !== true;
  ...
  else if (p.visaPaid === true) key = 'siap_biometrik';   // sudah bayar visa tapi belum dijadwalkan → perlu dijadwalkan
  else {
