...
export function generateRoomlist(passengers, customers = []) {
  ...
  const activeG = active.map((p) => {
    ...
    return { ...p, gender: p.gender || p.sex || c.gender || c.sex || '' };
  });
  ...
  for (const familyId of Object.keys(familyGroups)) {
    ...
    // room_type dari master file (kalau ditulis); 'family' = ikut jumlah anggota
    let writtenType = members.find((m) => m.room_type && m.room_type !== 'family')?.room_type || null;
    let roomType, capacity;
    if (writtenType && ROOM_CAPACITY[writtenType]) {
      roomType = writtenType;
      capacity = ROOM_CAPACITY[writtenType];
    } else {
      const auto = capacityByCount(members.length);
      roomType = auto.type;
      capacity = auto.cap;
    }
    ...
    for (let i = 0; i < members.length; i += capacity) {
      ...
      const roomLabel = members.length > capacity
        ...
        : `Family ${familyName}`;
      // Pecahan terakhir yang cuma 1 orang di kamar >1 → masih oke (family), tapi kasih catatan
      const lonely = slice.length === 1 && capacity > 1;
      rooms.push({
        room_no: roomCounter++,
        room_type: roomType,
        capacity: members.length > capacity ? capacity : Math.max(slice.length, 1),
        label: roomLabel,
        is_family: true,
        family_id: familyId,
        gender: 'family',
