...
// Roomlist FLAT rows dari generateRoomlist (auto live — SAMA dengan proyeksi income ops).
// Dipakai download roomlist Visa (CSV) & Portal TL (XLS) supaya konsisten.
export function roomlistFlatRows(passengers, customers = []) {
  const custMap = Object.fromEntries((customers || []).map((c) => [c.id, c]));
  const rooms = generateRoomlist(passengers, customers);
  const out = []; let no = 0;
  for (const r of rooms) {
    const roomLabel = `Kamar ${r.room_no} · ${(r.room_type || '').toUpperCase()}${r.is_family ? ' (Family)' : r.gender === 'M' ? ' (Cowok)' : r.gender === 'F' ? ' (Cewek)' : ''}`;
    for (const p of (r.pax || [])) {
      const c = custMap[p.customer_id] || {};
      const g = normalizeGender({ ...p, gender: c.gender || c.sex });
      out.push({
        no: ++no, room_no: r.room_no, room: roomLabel, room_type: r.room_type || '', is_family: !!r.is_family,
        name: c.name || `#${p.id}`, gender: g === 'M' ? 'L' : g === 'F' ? 'P' : '',
        phone: c.phone || c.whatsapp || '', passport_no: c.passport_no || c.passport_number || '',
        passport_expiry: c.passport_expiry || c.expiry_date || '', birthday: c.birthday || c.dob || c.date_of_birth || '',
        notes: r.needs_upgrade ? (r.upgrade_note || '') : '',
      });
    }
  }
  return out;
