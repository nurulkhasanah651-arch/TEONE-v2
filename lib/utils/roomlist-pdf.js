...
export async function downloadRoomlistPDF({ trip = {}, rooms = [] }) {
  ...
  const W = showKtp
    ? [6, 16, 16, 7, 15, 17, 6, 17, 18, 18, 17, 17, 17, 15, 18]
    : [7, 22, 22, 9, 20, 19, 7, 20, 19, 20, 19, 16, 27];
  // Index kolom Room Type & Catatan (dua kolom terakhir)
  const RT_IDX = COLS.length - 2;
  // Kolom tanggal (Tgl Lahir, Tgl Issue, Tgl Expired) — jangan wrap, tulis satu baris.
  const DATE_IDX = showKtp ? [5, 10, 12] : [5, 8, 10];
  ...
  // Tanggal: paksa satu baris (tidak turun ke bawah)
  DATE_IDX.forEach((i) => { columnStyles[i] = Object.assign({}, columnStyles[i], { overflow: 'visible', halign: 'center' }); });
