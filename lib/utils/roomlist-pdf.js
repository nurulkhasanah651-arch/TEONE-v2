...
function shortDate(s) {
  if (!s) return '';
  const d = new Date(s);
  // Tanggal yang sudah diformat (mis. "9 Jul 2026") tidak bisa di-parse ulang → tampilkan apa adanya.
...
const COLS = [
  ...
  'No Room', 'No Room', 'Remarks / Catatan',
  'Passport No / KTP', 'Place Of Birth', 'Birthdate', 'Age',
];
// lebar kolom (mm) - total ~270mm, muat di A4 landscape (usable ~281mm)
// Remarks dilebarkan supaya catatan peserta (halal meals, kursi roda, dll) terbaca.
const W = [8, 36, 28, 10, 18, 12, 12, 48, 30, 26, 22, 8];
...
export async function downloadRoomlistPDF({ trip = {}, rooms = [] }) {
  ...
  [0, 3, 5, 6, 11].forEach((i) => { columnStyles[i] = Object.assign({}, columnStyles[i], { halign: 'center' }); });
