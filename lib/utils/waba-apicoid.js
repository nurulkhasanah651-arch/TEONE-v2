...
export async function sendApicoidTemplate(cfg, phoneId, toPhone, name, lang, params, buttonUrlSuffix, headerImageUrl) {
  const to = normalizeWaPhone(toPhone);
  if (!to) return { ok: false, error: 'Nomor tujuan kosong' };
  if (!name) return { ok: false, error: 'template kosong' };
  const components = [];
  // Header image (opsional) — utk template dgn foto (mis. hasil visa / penolakan).
  if (headerImageUrl && String(headerImageUrl).trim()) {
    components.push({ type: 'header', parameters: [{ type: 'image', image: { link: String(headerImageUrl).trim() } }] });
  }
