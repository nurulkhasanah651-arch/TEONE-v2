...
export function canAccessPath(role, path) {
  ...
  // Estimate Profit Group — hanya Ops, Owner, Manager, Accounting (bukan PIC/CS).
  if (pathMatchesPrefix(path, '/operasional/profit-estimate')) {
    return ['owner', 'accounting', 'manager', 'ops'].includes(role);
  }

  // Ganti password sendiri — semua staf internal (login email+password) boleh akses.
  // Tour Leader & Mitra login via Google (tanpa password), jadi tidak perlu.
  if (pathMatchesPrefix(path, '/ganti-password')) {
    return role !== 'tour_leader' && role !== 'mitra';
  }
  ...
  // Morning Monitoring (Manager Dashboard): hanya owner / manager / accounting.
  if (pathMatchesPrefix(path, '/manager-dashboard')) return ['owner', 'manager', 'accounting'].includes(role);

  // Tab IT (laporan bug/error/request fitur): hanya owner & manager.
  if (pathMatchesPrefix(path, '/it')) return ['owner', 'manager'].includes(role);

  // Blast TL (kirim info ke semua TL dari nomor Putri): manajemen & ops.
  if (pathMatchesPrefix(path, '/tl-blast')) return ['owner', 'manager', 'ops', 'accounting'].includes(role);

  // Dashboard PIC (monitor + to-do, discope ke trip milik PIC): PIC + manajemen (oversight).
  if (pathMatchesPrefix(path, '/pic-dashboard')) return ['owner', 'manager', 'accounting', 'ops', 'pic'].includes(role);

  // To-Do List Harian (laporan ke owner): owner / manager / accounting / ops / pic.
  if (pathMatchesPrefix(path, '/daily-todo')) return ['owner', 'manager', 'accounting', 'ops', 'pic'].includes(role);
