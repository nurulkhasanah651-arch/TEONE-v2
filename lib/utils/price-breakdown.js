...
export function mainExpectedPerPassenger(p, breakdown = {}) {
  ...
  try {
    ...
    // Infant: harga dasar saja, tanpa biaya wajib (tips/city tax/bagasi/tiket domestik/base)
    const mainAddonTotal = key === 'infant' ? 0 : MAIN_ADDONS.reduce(
...
export function computeIncomeProjection(passengers = [], breakdown = {}, allPayments = []) {
  ...
  for (const p of safePassengers) {
    ...
    try {
      ...
      const addonForThis = key === 'infant' ? 0 : mainAddonTotal;
