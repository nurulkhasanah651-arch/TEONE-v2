...
export function mainAddonTotalForKey(bd, key) {
  ...
  // LAND TOUR: tanpa tiket & bagasi domestik & base. Hanya tips + city tax.
  if (isLandTourKey(key)) return (Number(bd.tips) || 0) + (Number(bd.city_tax) || 0);
...
export function expectedPerPassenger(passenger, breakdown = {}, payments = []) {
  ...
  try {
    ...
    if (passenger.include_visa && !_typesLower.has('visa')) total += Number(safeBreakdown.visa) || 0;
