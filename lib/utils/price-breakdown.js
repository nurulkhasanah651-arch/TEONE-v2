...
// Harga visa per peserta sesuai tipe: e-paspor (visa_epassport) vs biasa (visa).
// Fallback ke visa biasa bila e-paspor belum diisi. Khusus trip yg punya 2 tipe (mis. Jepang).
export function visaPriceFor(bd, visaType) {
  const b = bd || {};
  if (String(visaType || '') === 'epassport') {
    const ep = Number(b.visa_epassport) || 0;
    if (ep > 0) return ep;
  }
...
export function visaPriceTpl(tpl, bd, visaType) {
  ...
  if (String(visaType || '') === 'epassport') {
    const ep = Number(t.visa_epassport ?? t.Visa_Epassport) || Number(b.visa_epassport) || 0;
    if (ep > 0) return ep;
  }
