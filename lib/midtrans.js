...
export async function createSnapTransaction(code, { orderId, grossAmount, fee = 0, customer = {}, itemName = 'Trip', finishUrl, enabledPayments } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      const detail = (j?.error_messages && j.error_messages.join(', ')) || j?.status_message || `HTTP ${res.status}`;
      console.error('[midtrans snap] gagal', code, res.status, JSON.stringify(j).slice(0, 600));
      return { error: `Gagal membuat transaksi: ${detail}` };
    }
