...
export async function createSnapTransaction(code, { orderId, grossAmount, fee = 0, customer = {}, itemName = 'Trip', finishUrl, enabledPayments } = {}) {
  ...
  // QRIS: batas nasional Rp 10 juta/transaksi. Kalau hanya QRIS yang diaktifkan &
  // nominal melebihi batas, Midtrans tak menampilkan QR → tolak dengan pesan jelas.
  if (Array.isArray(enabledPayments) && enabledPayments.length === 1 && enabledPayments[0] === 'other_qris' && gross > 10000000) {
    return { error: 'QRIS maksimum Rp 10.000.000 per transaksi. Silakan pakai Virtual Account untuk nominal ini.' };
  }
  ...
  // QRIS: kunci acquirer ke GoPay (channel yang aktif) supaya "Other QRIS" tidak
  // pernah nyasar ke ShopeePay yang masih proses aktivasi -> cegah "Failed to Load QR".
  // Hanya diterapkan untuk transaksi QRIS; VA/lainnya tidak terpengaruh.
  if (Array.isArray(enabledPayments) && enabledPayments.includes('other_qris')) {
