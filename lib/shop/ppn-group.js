...
export async function computePpnByGroup(db, tripIds = null) {
  ...
  const { data: trips } = await db.from('trips')
    .select('id, name, departure, price_breakdown, status, visa_requirement').gte('departure', '2026-10-01');
  const relevant = [];
  ...
  for (let i = 0; i < relIds.length; i += 100) {
    ...
    for (;;) {
      const { data } = await db.from('trip_passengers')
        .select('id, trip_id, room_type, age_type, price_paid, discount_amount, transfer_status, refund_status, include_visa, visa_ready, visa_type')
  ...
  for (const t of relevant) {
    ...
    for (const p of list) {
      ...
      // PPN PESERTA (dari harga kamar/tour). base bisa 0 utk pax yg tipe kamarnya tak kebaca.
      if (ppnPer > 0) {
      ...
      // PPN PERUSAHAAN (potensi) — hanya TEONE, dari biaya non-kamar (kecuali city tax & asuransi).
      if (t.mode === 'teone') {
        const cb = companyBaseOf(p, t.bd, t.grpVisa);
        const cppn = tourPpn(cb, t.departure);
        if (cppn > 0) { coBase += cb; ppnCompany += cppn; }
      }
      cnt += 1;   // hitung SEMUA pax → jumlah pax = master trip
    }
