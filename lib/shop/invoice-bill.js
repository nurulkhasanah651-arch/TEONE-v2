...
export async function getInvoiceBilling(supabase, inv) {
  ...
  for (const pid of ids) {
    ...
    members.push({
      ...
      evisaExpected: Number(s.evisaExpected) || 0,
      ...
      asuransiTipsLocalGuide: Number(s.asuransiTipsLocalGuide) || 0, handlingPerlengkapan: Number(s.handlingPerlengkapan) || 0, visaAsuransi: Number(s.visaAsuransi) || 0,
      hargaDetail: (s.hargaDetail && typeof s.hargaDetail === 'object') ? s.hargaDetail : null,
      ppn: Number(s.ppn) || 0, ppnLabel: s.ppnLabel || '',
  ...
  // E-Visa (include terpisah) — mirror visa
  const evisaExpected = members.reduce((a, m) => a + (m.evisaExpected || 0), 0);
  const evisaCount = members.filter((m) => (m.evisaExpected || 0) > 0).length;
  ...
  const ppnTotal = members.reduce((a, m) => a + (m.ppn || 0), 0);
  const ppnLabel = (members.find((m) => m.ppnLabel)?.ppnLabel) || '';
  return { ids, members, addonItems, expectedTotal, pokokPaid, sisa, totalPaid, addonPaid, discount, billedTotal, visaExpected, asuransiExpected, evisaExpected, visaCount, asuransiCount, evisaCount, visaPokok, asuransiPokok, visaPokokCount, asuransiPokokCount, ppnTotal, ppnLabel, milestone: inv?.milestone || null, count: ids.length, isFamily: ids.length > 1 };
}
