...
function hayOf(t) { return `${t?.destination || ''} ${t?.name || ''} ${t?.public_title || ''}`.toLowerCase(); }

// Pilih region berdasarkan keyword yang PALING SPESIFIK (paling panjang) yang cocok,
// bukan region pertama di urutan. Mencegah keyword umum (mis. "turki"/"vip") merebut
// trip yang seharusnya masuk region lebih spesifik ("abu dhabi"/"west europe").
// Seri panjang → region yang lebih dulu di daftar menang (tie-break urutan).
function bestRegionKey(hay, list) {
  let bestKey = null, bestLen = 0;
  for (const r of (list || [])) {
    for (const k of (r.kw || [])) {
      const kk = String(k || '').toLowerCase().trim();
      if (kk && hay.includes(kk) && kk.length > bestLen) { bestKey = r.key; bestLen = kk.length; }
    }
  }
  return bestKey;
}
...
export function tripRegion(t) {
  return bestRegionKey(hayOf(t), REGIONS);
}
...
export function tripRegionIn(t, regions) {
  ...
  return bestRegionKey(hayOf(t), list);
}
