...
import { ROOM_KEYS, roomTypeToKey, MAIN_ADDONS, OPTIONAL_ADDONS, KHASANAH_MANDATORY_ADDONS, mainAddonTotalForKey, LAND_TOUR_KEYS } from '@/lib/utils/price-breakdown';
...
export function tripPriceItems(t) {
  ...
  const addons = MAIN_ADDONS.map((a) => ({ label: a.label, value: Number(bd[a.key]) || 0 })).filter((a) => a.value > 0);
  const byKey = {};
  ROOM_KEYS.forEach((r) => { const base = Number(bd[r.key]) || 0; const px = base > 0 ? base + mainAddonTotalForKey(bd, r.key) : 0; const discount = Number(bd?._diskon?.[r.key]) || 0; byKey[r.key] = { key: r.key, label: r.label, icon: r.icon, base, discount, baseBefore: base + discount, priceBefore: px > 0 ? px + discount : 0, addons: addonsForKey(bd, r.key), price: px }; });
