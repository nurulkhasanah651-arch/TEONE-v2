...
export async function getStorefrontSettingsPublic() {
  const db = svc();
  if (!db) return null;
  const { data } = await db.from('storefront_settings').select('hero_images, regions, private_images, terms_default, logo_url').eq('id', 1).maybeSingle();
  if (!data) return null;
  return {
...
const LIST_COLS = 'id, name, slug, destination, departure, return_date, price, public_price, dp_amount, quota, sold, seat_left, cover_image_url, status, highlights, price_breakdown, is_flash_sale, is_best_seller';
...
export async function getBestSellerTrips(limit = 6) {
  ...
  const { data } = await db.from('trips').select(LIST_COLS)
    ...
    .order('departure', { ascending: true, nullsFirst: false })
    .limit(limit);
  // Fallback: kalau belum ada yang ditandai best seller, tampilkan jadwal terdekat
  if (!data || data.length === 0) return getLatestTrips(limit);
  return data;
}
