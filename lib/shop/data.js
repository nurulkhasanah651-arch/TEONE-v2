...
export function tripRoomPrices(t) {
  ...
  return ROOM_KEYS
    .map((r) => ({ key: r.key, label: r.label, icon: r.icon, price: (Number(bd[r.key]) || 0) > 0 ? (Number(bd[r.key]) || 0) + add : 0 }))
    .filter((r) => r.price > 0);
}
...
export function tripPriceItems(t) {
  ...
  const add = mainAddonTotal(bd);
  const byKey = {};
  ROOM_KEYS.forEach((r) => { const base = Number(bd[r.key]) || 0; byKey[r.key] = { key: r.key, label: r.label, icon: r.icon, price: base > 0 ? base + add : 0 }; });
