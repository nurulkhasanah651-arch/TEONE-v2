...
export async function getStorefrontSettingsPublic() {
  const db = svc();
  if (!db) return null;
  const { data } = await db.from('storefront_settings').select('hero_images, regions, private_images, terms_default, logo_url, about_image, reasons_default').eq('id', 1).maybeSingle();
  if (!data) return null;
  return {
