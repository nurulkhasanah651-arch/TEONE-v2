...
export async function getBrandBank(codeArg) {
  // codeArg opsional: kalau diisi, baca bank brand SPESIFIK (lintas-brand, mis. template
  // WA trip Khasanah saat sedang di brand TEONE). Tanpa arg = perilaku lama (brand aktif).
  let code = codeArg || null;
  if (!code) { try { code = currentBrandCode(); } catch {} }
  code = code || 'teone';
  const db = codeArg ? serviceClientFor(codeArg) : svc();
  if (!db) return null;
  let data = null;
  {
