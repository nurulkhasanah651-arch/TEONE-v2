...
export function tripRoomPrices(t) {
  ...
  return ROOM_KEYS
    .map((r) => { const base = Number(bd[r.key]) || 0; return { key: r.key, label: r.label, icon: r.icon, base, addons, price: base > 0 ? base + add : 0 }; })
    .filter((r) => r.price > 0);
}
...
export function tripPriceItems(t) {
  ...
  const addons = MAIN_ADDONS.map((a) => ({ label: a.label, value: Number(bd[a.key]) || 0 })).filter((a) => a.value > 0);
  const byKey = {};
  ROOM_KEYS.forEach((r) => { const base = Number(bd[r.key]) || 0; byKey[r.key] = { key: r.key, label: r.label, icon: r.icon, base, addons, price: base > 0 ? base + add : 0 }; });
