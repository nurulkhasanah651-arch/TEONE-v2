...
export async function recordInvoiceMilestone(passengerId, type, amount) {
  const db = svc();
  if (!db) return { error: 'no_service' };
  ...
  await db.from('participant_payments').insert({
    passenger_id: passengerId, type, label: 'Midtrans (online invoice)',
...
export async function applyInvoiceOnlinePaid(invoiceId, paidAmount) {
  const db = svc();
  if (!db) return { error: 'no_service' };
  ...
  for (const pid of ids) {
    ...
    if (ex) await db.from('participant_payments').update({ amount: amt, paid_at: today(), notes: note }).eq('id', ex.id);
    else await db.from('participant_payments').insert({ passenger_id: pid, type: milestone, amount: amt, paid_at: today(), notes: note, created_by: 'website' });
  }
