...
export async function fulfillPaidBooking(orderCode, method) {
  ...
  if (customerId && !alreadyCount) {
    for (let idx = 0; idx < paxList.length; idx++) {
      ...
      const { data: p } = await db.from('trip_passengers').insert({
        ...
        include_visa: incVisaFlag, visa_ready: visaReadyFlag, include_asuransi: incAsuransiFlag,
      }).select('id').single();
      if (p?.id) {
  ...
  try {
    if (booking.lead_phone) {
      ...
      await sendFonnte(booking.lead_phone, lines.join('\n'), { context: 'cs', brand: currentBrandCode(), token: await getPicFonnteTokenById(db, booking.trip_id) }); // DP pertama dari web → nomor CS
    }
...
export async function recordMilestonePayment(orderCode, milestoneType, method) {
  ...
  try {
    if (booking.lead_phone) {
      ...
      await sendFonnte(booking.lead_phone, msg, { context: 'finance', brand: currentBrandCode(), token: await getPicFonnteTokenById(db, booking.trip_id) }); // pembayaran lanjutan → nomor Finance
    }
  } catch {}
  ...
  try {
    ...
    if (pax?.customer_id) {
      ...
      if (phone) {
        ...
        await sendFonnte(phone, msg, { context: 'finance', brand: currentBrandCode(), token: await getPicFonnteTokenById(db, pax.trip_id) });
      }
    }
  } catch {}
  return { ok: true };
    ...
    if (headPid) {
      ...
      if (pax?.customer_id) {
        ...
        if (phone) {
          ...
          await sendFonnte(phone, msg, { context: 'finance', brand: currentBrandCode(), token: await getPicFonnteTokenById(db, inv.trip_id) });
        }
      }
    }
  } catch {}
...
export async function invoiceAllInOutstanding(invId) {
  ...
  const { data: trip } = await db.from('trips').select('price_breakdown').eq('id', inv.trip_id).maybeSingle();
  const bd = trip?.price_breakdown || {};
  let total = 0; const members = [];
  for (const pid of ids) {
    const { data: pax } = await db.from('trip_passengers').select('id, room_type, age_type, price_paid, discount_amount, include_visa, include_asuransi, visa_ready').eq('id', pid).maybeSingle();
    if (!pax) continue;
    ...
    const visa = (pax.include_visa && !pax.visa_ready && !hasVisaPay) ? (Number(bd.visa) || 0) : 0;
    const asuransi = (pax.include_asuransi && !hasAsrPay) ? (Number(bd.asuransi) || 0) : 0;
...
export async function applyInvoiceAllInPaid(invId, paidAmount, method) {
  ...
  try {
    ...
    if (headPid) { const { sendPaymentReceivedWA } = await import('@/lib/actions/wa-payment-notif'); await runWithBrand(currentBrandCode(), async () => { await sendPaymentReceivedWA(headPid); }); }
  } catch {}
  return { ok: true };
}
