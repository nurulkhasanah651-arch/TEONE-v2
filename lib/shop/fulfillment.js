  ...
  try {
    if (paymentId && Number(booking.amount) > 0) {
      ...
      if (acct) {
        ...
        // (a) Pembayaran peserta (pokok, tanpa biaya admin)
        const { data: exist } = await db.from('accounting_entries').select('id').eq('linked_payment_id', paymentId).limit(1).maybeSingle();
        if (!exist && tripPortion > 0) {
          await db.from('accounting_entries').insert({
            type: 'in', amount: tripPortion, category: 'Payment Peserta',
            description: `${booking.payment_type === 'full' ? 'Pelunasan' : 'DP'} web - ${booking.lead_name || 'peserta'}${tripLabel}`,
            trip_id: booking.trip_id, account_id: acct.id, date: today(), created_by: 'website', linked_payment_id: paymentId,
          });
        }
        // (b) Biaya admin web (kategori sendiri biar jelas)
        if (adminFee > 0) {
...
export async function recordMilestonePayment(orderCode, milestoneType) {
  const db = svc();
  if (!db) return { error: 'no_service' };
  const { data: booking } = await db.from('bookings').select('*').eq('order_code', orderCode).maybeSingle();
  if (!booking) return { error: 'booking_not_found' };
  ...
  for (const pid of pids) {
    const { data: p } = await db.from('participant_payments').insert({
      passenger_id: pid, type: milestoneType, label: 'Midtrans (web)',
  ...
  try {
    ...
    if (acct) {
      ...
      const { data: ex } = await db.from('accounting_entries').select('id').eq('linked_payment_id', firstPaymentId).limit(1).maybeSingle();
      if (firstPaymentId && !ex && total > 0) {
        await db.from('accounting_entries').insert({ type: 'in', amount: total, category: 'Payment Peserta', description: `${milestoneType} web - ${booking.lead_name || 'peserta'}${tripLabel}`, trip_id: booking.trip_id, account_id: acct.id, date: today(), created_by: 'website', linked_payment_id: firstPaymentId });
      }
