...
async function ensureWebReceiptInvoice(db, { booking, trip, pids, headPid, headCid, familyGroupId, milestone, amount, paidAt, passengerAmounts }) {
  try {
    ...
    // Idempoten (webhook Midtrans bisa masuk >1x): sudah ada invoice sendiri, atau
    // invoice keluarga yg mencakup kepala -> pakai yg itu, jangan bikin baru.
    const { data: own } = await db.from('invoices').select('id, public_token').eq('passenger_id', headPid).limit(1);
    if (own && own.length) return own[0];
    const { data: fam } = await db.from('invoices').select('id, public_token')
      .eq('trip_id', booking.trip_id).contains('covers_passenger_ids', [headPid]).limit(1);
    if (fam && fam.length) return fam[0];
