...
export async function fulfillPaidBooking(orderCode, method) {
  ...
  try {
    if (booking.lead_phone) {
      ...
      await sendFonnte(booking.lead_phone, lines.join('\n'), { context: 'cs', brand: currentBrandCode(), token: currentBrandCode() === 'khasanah' ? await getPicFonnteTokenById(db, booking.trip_id) : null }); // DP pertama web: TEONE=nomor CS, Khasanah=PIC (existing)
    }
...
export async function recordMilestonePayment(orderCode, milestoneType, method) {
  ...
  try {
    if (booking.lead_phone) {
      ...
      const msg = [`*Pembayaran ${milestoneType} Diterima* ✅`, '', `Halo *${booking.lead_name || 'Kak'}*, pembayaran lanjutan untuk order *${booking.order_code}* sudah kami terima 🙏`, `💳 ${milestoneType}: *${rp(total)}*`, '', `Bukti pembayaran (tanda terima resmi) klik di sini 👇`, `🔗 ${receiptLink}`, '', 'Terima kasih 🙏'].join('\n');
      await sendFonnte(booking.lead_phone, msg, { context: 'finance', brand: currentBrandCode(), token: await getPicFonnteTokenById(db, booking.trip_id) }); // pembayaran lanjutan → nomor PIC/Finance
    }
  } catch {}
...
export async function invoiceAllInOutstanding(invId) {
  ...
  for (const pid of ids) {
    ...
    const visa = (pax.include_visa && !pax.visa_ready && !hasVisaPay) ? visaPriceTpl(tpl, bd, pax.visa_type) : 0;
    const asuransi = (pax.include_asuransi && !hasAsrPay) ? _aPrice : 0;
