...
export async function fulfillPaidBooking(orderCode, method) {
  ...
  try {
    if (booking.lead_phone) {
      ...
      await sendFonnte(booking.lead_phone, lines.join('\n'), { context: 'cs', brand: currentBrandCode() }); // DP pertama dari web → nomor CS
    }
...
export async function recordMilestonePayment(orderCode, milestoneType, method) {
  ...
  try {
    ...
    if (acct) {
      ...
      if (!exA && ADMIN_FEE_ONLINE > 0) {
        await db.from('accounting_entries').insert({ type: 'in', amount: ADMIN_FEE_ONLINE, category: 'Biaya Admin Web', description: `Biaya admin web - ${orderCode} ${milestoneType}${tripLabel}`, trip_id: booking.trip_id, account_id: acct.id, date: today(), created_by: 'website' });
      }
    }
  } catch {}
  ...
  try {
    if (booking.lead_phone) {
      ...
      await sendFonnte(booking.lead_phone, msg, { context: 'finance', brand: currentBrandCode() }); // pembayaran lanjutan → nomor Finance
    }
  } catch {}
  ...
  try {
    ...
    if (pax?.customer_id) {
      ...
      if (phone) {
        ...
        await sendFonnte(phone, msg, { context: 'finance', brand: currentBrandCode() });
      }
    }
  } catch {}
  return { ok: true };
...
export async function applyInvoiceOnlinePaid(invoiceId, paidAmount, method) {
  ...
  // Invoice MANUAL online (tanpa peserta trip) → catat Cash IN ke Accounting
  if (ids.length === 0) {
    try {
      await db.from('accounting_entries').insert({
        type: 'in',
        amount: Number(paidAmount) || Number(inv.amount) || 0,
