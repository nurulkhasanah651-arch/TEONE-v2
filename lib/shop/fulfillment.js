...
export async function applyInvoiceAllInPaid(invId, paidAmount, method) {
  ...
  try {
    ...
    if (headPid) { const { sendPaymentReceivedWA } = await import('@/lib/actions/wa-payment-notif'); await runWithBrand(currentBrandCode(), async () => { await sendPaymentReceivedWA(headPid, false, { forceSend: true }); }); }
  } catch {}
  return { ok: true };
}
