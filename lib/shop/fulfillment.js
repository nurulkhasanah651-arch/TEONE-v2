...
// ============================================================
// TANDA TERIMA (invoice paid) untuk pembayaran WEB.
//
// Kenapa perlu: konfirmasi WA dikirim oleh sendPaymentReceivedWA, yg mengambil
// nominal dari invoice yg di-link (_invRow.amount). Kalau invoice BELUM ada, ia
// jatuh ke lastPayment.amount = pembayaran SATU peserta -> keluarga 2 pax yg bayar
// DP 7jt dikonfirmasi "DP: Rp 3.500.000", dan tanpa link krn invoiceLink null.
// (Bukti: order TE-MRMYLTQF — booking paid 10.41 WIB, invoice TEONE-557-008 baru
//  dibuat CS 14.44 WIB, 3 jam SETELAH WA terkirim.)
//
// Ini menyamakan jalur web dgn jalur CS daily (ensureReceiptInvoice di
// lib/actions/dp.js): 1 invoice atas nama kepala, mencakup semua anggota order,
// amount = total yg benar-benar dibayar.
//
// Sengaja TIDAK memakai createInvoiceAsPaid(): fungsi itu butuh sesi staff
// (assertStaff) — di sini konteksnya webhook tanpa user — dan memanggil
// syncInvoiceToMatrix, padahal pembayaran SUDAH dicatat di participant_payments
// oleh fulfillment. Memanggilnya akan dobel-hitung.
// ============================================================
function genInvoiceToken() {
...
// Nomor invoice: ikut pola generateInvoiceNo() di lib/actions/invoices.js — ambil
// urutan TERTINGGI yg ada (bukan count), supaya tak bentrok bila ada yg dihapus.
async function nextInvoiceNo(db, tripId, bump = 0) {
  const { data: trip } = await db.from('trips').select('kode_trip').eq('id', tripId).maybeSingle();
  const kode = String(trip?.kode_trip || tripId).replace(/[^A-Z0-9]/gi, '').toUpperCase();
  const { data: rows } = await db.from('invoices').select('invoice_no').eq('trip_id', tripId);
  let maxSeq = 0;
  for (const r of (rows || [])) {
  ...
  const prefix = currentBrandCode() === 'khasanah' ? 'KT' : 'TEONE';
  return `${prefix}-${kode}-${seq}`;
}

async function ensureWebReceiptInvoice(db, { booking, trip, pids, headPid, headCid, familyGroupId, milestone, amount, paidAt, passengerAmounts }) {
  try {
    if (!headPid || !(Number(amount) > 0)) return null;

    // Idempoten (webhook Midtrans bisa masuk >1x): kalau tanda terima utk MILESTONE
    // ini sudah ada -> pakai yg itu. Sengaja per-milestone: kalau dijaga "punya
    // invoice apa pun", pembayaran P1 tak akan pernah dapat tanda terima sendiri
    // karena invoice DP sudah ada -> WA-nya menulis nominal DP.
    const _sameMilestone = (rows) => (rows || []).find((x) =>
      String(x.milestone || '').trim().toLowerCase() === String(milestone || '').trim().toLowerCase());
    const { data: own } = await db.from('invoices').select('id, public_token, milestone').eq('passenger_id', headPid);
    const _ownHit = _sameMilestone(own);
    if (_ownHit) return _ownHit;
    const { data: fam } = await db.from('invoices').select('id, public_token, milestone')
      .eq('trip_id', booking.trip_id).contains('covers_passenger_ids', JSON.stringify([headPid]));
    const _famHit = _sameMilestone(fam);
    if (_famHit) return _famHit;

    const isFamily = Array.isArray(pids) && pids.length > 1;
    let cust = null;
    if (headCid) {
      const { data } = await db.from('customers').select('name, phone, whatsapp, email').eq('id', headCid).maybeSingle();
      cust = data || null;
    }

    const payload = {
      invoice_no: await nextInvoiceNo(db, booking.trip_id),
      trip_id: booking.trip_id,
      passenger_id: headPid,
      customer_id: headCid || null,
      milestone,
      amount: Number(amount) || 0,
      status: 'paid',
      paid_at: paidAt || new Date().toISOString(),
      paid_by_check: 'website',
      description: `Tanda terima ${milestone} — order ${booking.order_code}`,
      public_token: genInvoiceToken(),
      created_by: 'website',
      customer_name: cust?.name || booking.lead_name || null,
      customer_phone: cust?.phone || cust?.whatsapp || booking.lead_phone || null,
      customer_email: cust?.email || booking.lead_email || null,
      trip_name: trip?.name || null,
      trip_kode: trip?.kode_trip || null,
      family_group_id: familyGroupId || null,
      covers_passenger_ids: isFamily ? pids : [],
      is_family_invoice: isFamily,
      passenger_amounts: passengerAmounts && typeof passengerAmounts === 'object' ? passengerAmounts : {},
    };

    console.log('[ensureWebReceiptInvoice] insert', payload.invoice_no, 'amount', payload.amount, 'head', headPid, 'family', isFamily);
    let { data, error } = await db.from('invoices').insert(payload).select('id, public_token').single();
    if (error) console.error('[ensureWebReceiptInvoice] insert error:', error.message, '| code', error.code, '| details', error.details, '| hint', error.hint);
    // Nomor invoice bentrok (unique invoice_no) -> ambil urutan berikutnya.
    let tries = 0;
    while (error && /invoice_no|duplicate key|23505/i.test(error.message || '') && tries < 5) {
      tries++;
      payload.invoice_no = await nextInvoiceNo(db, booking.trip_id, tries);
      const r = await db.from('invoices').insert(payload).select('id, public_token').single();
      data = r.data; error = r.error;
    }
    if (error) { console.error('[ensureWebReceiptInvoice]', error.message); return null; }

    // Jejak tanda terima (sama spt jalur CS daily). participant_payments TIDAK
    // disentuh — sudah dicatat oleh fulfillment di atas.
    try {
      await db.from('invoice_payments').insert({
        invoice_id: data.id, amount: Number(amount) || 0,
        payment_date: (paidAt ? new Date(paidAt) : new Date()).toISOString().slice(0, 10),
        payment_method: 'online', status: 'verified',
        verified_by: 'website', verified_at: new Date().toISOString(),
        note_from_customer: `Pembayaran online — order ${booking.order_code}`,
      });
    } catch { /* jejak best-effort */ }

    return data;
  } catch (e) {
    console.error('[ensureWebReceiptInvoice]', e?.message);
    return null;
  }
}
...
export async function fulfillPaidBooking(orderCode, method) {
  ...
  const incEvisaFlag = !!meta.include_evisa;
  ...
  if (customerId && !alreadyCount) {
    for (let idx = 0; idx < paxList.length; idx++) {
      ...
      const { data: p } = await db.from('trip_passengers').insert({
        ...
        include_visa: incVisaFlag, visa_ready: visaReadyFlag, include_asuransi: incAsuransiFlag, include_evisa: incEvisaFlag, visa_type: visaTypeVal,
      }).select('id').single();
      if (p?.id) {
  ...
  // TANDA TERIMA: 1 invoice paid utk SELURUH order (kepala + anggota), dibuat SEBELUM WA.
  // Tanpa ini sendPaymentReceivedWA tak punya invoice -> nominal jatuh ke pembayaran
  // 1 peserta (DP 3,5jt padahal keluarga bayar 7jt) dan link invoice tidak ikut.
  let receiptInvoice = null;
  try {
    // Ambil peserta order ini dari DB (bukan createdPids) supaya jalur idempoten
    // (booking sudah pernah diproses) tetap dapat daftar yg benar.
    const { data: orderPax } = await db.from('trip_passengers')
      .select('id, customer_id, is_family_head, family_group_id')
      .eq('trip_id', booking.trip_id).eq('room_notes', marker).order('id');
    let _pax = orderPax || [];
    // Fallback: kalau re-read kosong (visibilitas read-after-write / pooler), pakai peserta yg
    // BARU dibuat di invocation ini. Tanpa ini invoice tanda terima tak pernah dibuat & WA jatuh
    // ke nominal 1 peserta. (recordMilestonePayment sudah punya fallback serupa; DP tadinya tidak.)
    if (!_pax.length && createdPids.length) {
      _pax = createdPids.map((cp, i) => ({ id: cp.pid, customer_id: cp.cid, is_family_head: i === 0, family_group_id: familyGroupId || null }));
    }
    if (_pax.length) {
      const _head = _pax.find((x) => x.is_family_head) || _pax[0];
      const _pids = _pax.map((x) => x.id);
      // WA konfirmasi harus ke KEPALA (pemilik invoice keluarga). Di jalur idempoten
      // passengerId diambil dgn limit(1) tanpa urutan -> bisa kena anggota, bukan kepala.
      passengerId = _head.id;
      // Nominal tanda terima = pembayaran yg BENAR-BENAR dicatat utk order ini
      // (DP: perPaxDp x jumlah peserta; Pelunasan: harga pokok tiap peserta).
      const { data: _pays } = await db.from('participant_payments')
        .select('passenger_id, amount').eq('notes', booking.order_code).in('passenger_id', _pids);
      const _amounts = {};
      let _total = 0;
      for (const _p of (_pays || [])) {
        const _a = Number(_p.amount) || 0;
        _amounts[String(_p.passenger_id)] = (_amounts[String(_p.passenger_id)] || 0) + _a;
        _total += _a;
      }
      // Cadangan: kalau catatan pembayaran tak terbaca, pakai porsi trip (total bayar - admin).
      if (!(_total > 0)) _total = tripPortion;
      receiptInvoice = await ensureWebReceiptInvoice(db, {
        booking, trip, pids: _pids,
        headPid: _head.id, headCid: _head.customer_id,
        familyGroupId: familyGroupId || _head.family_group_id || null,
        milestone: isFull ? 'Pelunasan' : 'DP',
        amount: _total,
        paidAt: booking.paid_at || new Date().toISOString(),
        passengerAmounts: _amounts,
      });
      if (!receiptInvoice) console.error('[web-receipt] invoice NULL utk order', booking.order_code, 'head', _head.id, 'amount', _total);
    } else {
      console.error('[web-receipt] TIDAK ada peserta utk order', booking.order_code, 'marker', marker);
    }
  } catch (e) { console.error('[web-receipt] gagal utk order', booking.order_code, ':', e?.message, e?.stack); }
  ...
  try {
    if (booking.lead_phone) {
      ...
      // Link tanda terima resmi (invoice). Fallback ke /akun kalau invoice gagal dibuat.
      if (receiptInvoice?.public_token) {
        lines.push('', `Bukti pembayaran (tanda terima resmi) klik link di bawah ini 👇`, `🔗 ${base}/invoice/${receiptInvoice.public_token}`);
      } else {
        lines.push('', `Pantau status & trip kamu di:`, `${base}/akun`);
      }
      lines.push('', `Ada pertanyaan? Chat CS kami di sini.`, '', 'Terima kasih 🙏');
...
export async function recordMilestonePayment(orderCode, milestoneType, method) {
  ...
  const { data: booking } = await db.from('bookings').select('*').eq('order_code', orderCode).maybeSingle();
  if (!booking) return { error: 'booking_not_found' };
  const { data: trip } = await db.from('trips').select('id, name, kode_trip, dp_amount, payment_template').eq('id', booking.trip_id).maybeSingle();
  ...
  // Tanda terima utk milestone ini (P1/P2/P3...) — alasan sama spt DP di
  // fulfillPaidBooking: tanpa invoice ber-milestone ini, sendPaymentReceivedWA
  // menulis nominal 1 peserta, bukan total yg dibayar keluarga.
  let _milestoneInv = null;
  try {
    const { data: orderPax } = await db.from('trip_passengers')
      .select('id, customer_id, is_family_head, family_group_id')
      .eq('trip_id', booking.trip_id).eq('room_notes', 'web:' + booking.order_code).order('id');
    const _pax = (orderPax && orderPax.length) ? orderPax : pids.map((id) => ({ id, customer_id: null, is_family_head: false, family_group_id: null }));
    const _head = _pax.find((x) => x.is_family_head) || _pax[0];
    const _amounts = {};
    for (const _p of _pax) _amounts[String(_p.id)] = perPax;
    _milestoneInv = await ensureWebReceiptInvoice(db, {
      booking, trip, pids: _pax.map((x) => x.id),
      headPid: _head.id, headCid: _head.customer_id,
      familyGroupId: _head.family_group_id || null,
      milestone: milestoneType,
      amount: perPax * _pax.length,
  ...
  try {
    if (booking.lead_phone) {
      ...
      if (_milestoneInv?.public_token) receiptLink = `${base}/invoice/${_milestoneInv.public_token}`;
      else try {
...
export async function invoiceAllInOutstanding(invId) {
  ...
  const _evPrice = Number(tpl.visa_evisa ?? tpl.Visa_Evisa) || Number(bd.visa_evisa) || 0;
  ...
  for (const pid of ids) {
    const { data: pax } = await db.from('trip_passengers').select('id, room_type, age_type, price_paid, discount_amount, include_visa, include_asuransi, include_evisa, visa_ready, visa_type').eq('id', pid).maybeSingle();
    if (!pax) continue;
    ...
    const hasEvisaPay = (pays || []).some((x) => String(x.type).toLowerCase() === 'e-visa');
    ...
    const evisa = _khBrand ? 0 : ((pax.include_evisa && !hasEvisaPay) ? _evPrice : 0);
    members.push({ pid, pokok, visa, asuransi, evisa });
    total += pokok + visa + asuransi + evisa;
  }
