...
export async function buildMonitor(db, { tripFilter = null } = {}) {
  ...
  const fullNoTicket = upcoming
    ...
    .map(tripCard);
  // Siap beli tiket: sudah terisi >= 70% (tapi belum full) & belum ada PNR ke-connect →
  // persiapan beli/kunci tiket sebelum penuh.
  const readyToBuyTicket = upcoming
    .filter((t) => {
      const mt = meta[t.id];
      if (mt.quota <= 0 || mt.full) return false; // full sudah masuk fullNoTicket
      if ((mt.sold / mt.quota) < 0.7) return false;
      return !(ticketByTrip[t.id] && ticketByTrip[t.id].count > 0);
    })
  ...
  return {
    ...
    tripIds: [...tripIds],
    ticketing: { fullNoTicket, readyToBuyTicket, notIssued },
